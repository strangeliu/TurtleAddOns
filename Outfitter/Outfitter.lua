...
local Outfitter_cStatIDItems = {
    ...
    ArgentDawn = Outfitter_cArgentDawnTrinkets,
        Swimming = {
        { Code = 60860, SubCode = 0 },  -- 流动元素之戟
        { Code = 65026, SubCode = 0 },  -- 深渊猎手头盔
        { Code = 9452, SubCode = 0 },  -- 水藤
    },
};
...
local Outfitter_cItemStatInfo = {
    ...
    { ID = "Swimming", Name = Outfitter_cSwimmingStatName, Category = "Trade" },
};
...
local Outfitter_cClassSpecialOutfits = {
    ...
    Hunter = {
        ...
        { Name = Outfitter_cHunterWolf, SpecialID = "Wolf" },
    },
...
local gOutfitter_SpellNameSpecialID = {
    ...
    [Outfitter_cAspectOfTheWolf] = "Wolf",
...
local gOutfitter_AuraIconSpecialID = {
    ...
    -- ["Ability_Mount_WhiteDireWolf"] = "Riding", -- 移除图标映射，改用名称/工具提示识别
...
local Outfitter_cSpecialOutfitDescriptions = {
    ...
    Swimming = Outfitter_cSwimmingOutfitDescription,   -- 新增
};
...
local gOutfitter_IsSwimming = false;   -- 游泳状态缓存
...
function Outfitter_OnShow()
    -- 显示窗口时默认切换到主视图
    Outfitter_ShowPanel(1);
    
    -- 隐藏角色属性统计面板和物品提示检查框，避免重叠
    local bcsFrame = _G["StatCompareSelfFrame"]
    if bcsFrame and bcsFrame:IsShown() then
        bcsFrame:Hide()
    end
    
    -- 隐藏 S_ItemTip_InspectFrame_UI（如果存在）
    local sItemTipFrame = _G["S_ItemTip_InspectFrame_UI"]
    if sItemTipFrame and sItemTipFrame:IsShown() then
        sItemTipFrame:Hide()
    end
end
...
function Outfitter_OnHide()
    ...
    OutfitterFrame:Hide(); -- 这似乎是冗余的，但OnHide处理程序在被调用时
    -- 是为了响应父窗口被隐藏（角色窗口）
    -- 所以在这里调用Hide()确保当角色窗口隐藏时
    -- Outfitter下次打开时不会显示
    -- 恢复显示角色属性统计面板和物品提示检查框
    local bcsFrame = _G["StatCompareSelfFrame"]
    if bcsFrame then
        bcsFrame:Show()
    end
    
    -- 恢复显示 S_ItemTip_InspectFrame_UI（如果存在）
    local sItemTipFrame = _G["S_ItemTip_InspectFrame_UI"]
    if sItemTipFrame then
        sItemTipFrame:Show()
    end
end
...
Outfitter_cCategoryDescriptions = {
    Complete = Outfitter_cCompleteCategoryDescription,
    ...
    while true do
        ...
        elseif not vAuraStates.Dining
            ...
            if vTextLine1 then
                ...
                elseif vTextLine2
                and string.find(vTextLine2, Outfitter_cMountLowFormat) then
                ...
                elseif vTextLine2
                and string.find(vTextLine2, Outfitter_cMountFreeFormat) then
                    vAuraStates.Riding = true;
                end
            end
        end
...
-- 游泳状态更新函数
function Outfitter_UpdateSwimmingState()
    if not gOutfitter_Initialized then return end

    -- 安全调用 PlayerIsSwimming（可能不存在）
    local isSwimming = false
    local success, result = pcall(PlayerIsSwimming)
    if success then
        isSwimming = result
    else
        -- 函数不存在，不做任何处理
    ...
    if gOutfitter_IsSwimming ~= isSwimming then
        gOutfitter_IsSwimming = isSwimming
        Outfitter_SetSpecialOutfitEnabled("Swimming", isSwimming)
    end
end
...
function Outfitter_InitializeSpecialOccassionOutfits()
    ...
    -- 创建游泳套装（自动包含有游泳属性的物品）
    vOutfit = Outfitter_GetSpecialOutfit("Swimming");
    if not vOutfit then
        vOutfit = Outfitter_GenerateSmartOutfit(Outfitter_cSwimmingOutfit, "Swimming", vEquippableItems, true);
        if vOutfit then
            vOutfit.SpecialID = "Swimming";
            Outfitter_AddOutfit(vOutfit);
        else
            Outfitter_CreateEmptySpecialOccassionOutfit("Swimming", Outfitter_cSwimmingOutfit);
        end
    end
end
...
function OutfitterTimer_AdjustTimer()
    local vNeedTimer = true;  -- 始终需要计时器，保证游泳检查运行
...
function OutfitterUpdateFrame_OnUpdate(pElapsed)
    ...
    -- 新增：游泳状态检查（每0.5秒）
    if not OutfitterUpdateFrame.SwimmingCheckElapsed then
        OutfitterUpdateFrame.SwimmingCheckElapsed = 0;
    end
    OutfitterUpdateFrame.SwimmingCheckElapsed = OutfitterUpdateFrame.SwimmingCheckElapsed + pElapsed;
    if OutfitterUpdateFrame.SwimmingCheckElapsed > 0.5 then
        Outfitter_UpdateSwimmingState();
        OutfitterUpdateFrame.SwimmingCheckElapsed = 0;
    end
