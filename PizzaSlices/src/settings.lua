PizzaSlices:RegisterModule('settings', function ()
  ...
  f.header:SetText(PS.Colors.primary .. 'Pizza Slices' .. PS.Colors.secondary .. '   符 文 星 盘')

  f.version = f:CreateFontString('PizzaSlicesSettingsVersion', 'DIALOG', 'GameFontWhite')
  f.version:SetFont(STANDARD_TEXT_FONT, 10, 'OUTLINE')
...
--    frame:SetText(PS.utils.capitalize(tab))
	if tab == 'general' then
	  frame:SetText('通用设置')  -- General → 通用设置
	elseif tab == 'rings' then
	  frame:SetText('星盘设置')  -- Rings → 星盘圆环
	end
  ...
  do -- Dropdown: Ring close rotation animation
    ...
    frame.checkboxRight.label:SetText('右旋')
    ...
    frame.checkboxLeft.label:SetText('左旋')
    ...
    frame.checkboxOff.label:SetText('关闭')
  ...
  rings.bind.hint:SetText('或按ESC取消')
  rings.bind:Hide()
  ...
  function showKeybindOverlay(ringIdx)
    ...
    rings.bind.text:SetText('按下按键或鼠标按钮绑定圆环"' .. ring.name .. '"')
    rings.bind:Show()
    ...
    for i, ring in PizzaSlices_rings do
      ...
      if not f then
        ...
        function f.bind.update()
          local binding = GetBindingKey('PIZZASLICES_RING'..idx) or '快捷键'
  ...
  function PS.settings.editRing(ringIdx)
    ...
    local ring = ringIdx and PS.utils.clone(PizzaSlices_rings[ringIdx]) or { name = '新建快捷星盘', slices = {} }
    if not ring then
      PS:PrintError('索引处的快捷星盘' .. ringIdx .. '不存在！')
      return
    end
