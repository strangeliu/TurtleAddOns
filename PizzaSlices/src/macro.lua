PizzaSlices:RegisterModule('macro', function ()
  ...
  function PS.macro.getRunMacro(name)
    if name and name ~= '<name>' then
      local macroId = GetMacroIndexByName(name)

      if (not macroId or macroId == 0)
        and type(GetNumSuperMacros) == "function"
        and type(RunSuperMacro) == "function" then
        return RunSuperMacro
      end
    end
    ...
    if not runMacro and RunMacro then
      runMacro = RunMacro
    end
    if not runMacro then
      runMacro = PS.macro.run
    end
  ...
  function PS.macro.opensRing(macroName, ringIdx)
    ...
    local lines = PS.utils.strSplit(body, '\n', true)
    for _, line in pairs(lines) do
      if line == ('/ps open ' .. ringIdx)
        or line == ('/slices open ' .. ringIdx)
        or line == ('/pizzaslices open ' .. ringIdx) then
        return true
      end
    end
    return false
