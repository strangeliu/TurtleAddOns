PizzaSlices:RegisterModule('frame', function ()
  ...
  function PS.frame.open(ring)
    ...
    for idx, slice in ring.slices do
      ...
      -- 分隔线特殊处理
      if slice.isSeparator then
        f.tex:SetTexture(nil)
        f.borderlow:SetTexture(nil)
        f.borderhigh:SetTexture(nil)
        f.oglow:SetTexture(nil)
        f.iglow:SetTexture(nil)
        f.cd:Hide()
        f.cdtext:Hide()
      ...
      -- 处理分隔线样式
      if slice.isSeparator then
        f.tex:SetTexture('Interface\\AddOns\\PizzaSlices\\img\\glow')
        f.tex:SetVertexColor(0.5, 0.5, 0.5, 0.7)
        f:SetWidth(100)
        f:SetHeight(2)
        f.borderlow:SetTexture(nil)
        f.borderhigh:SetTexture(nil)
