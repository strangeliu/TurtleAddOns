...
function BGAssist_WSGFlags_OnShow()
	...
	if (not BGAssist_Config.noflags) then
		...
		for i = 1, 2, 1 do
			-- [调试] 显示BGAssist获取旗手信息
			local flagCarrier = Glory:GetFlagCarrier(i)
	...
	if (players > 0) then
		...
		BGAssist_CountedPlayers = {
			["Horde"] = {
				...
				[CLASS_SHAMAN] = 0, [CLASS_WARRIOR] = 0, [CLASS_WARLOCK] = 0,
			},
			["Alliance"] = {
				["count"] = 0,
				[CLASS_DRUID] = 0, [CLASS_HUNTER] = 0, [CLASS_MAGE] = 0,
...
--			Debug_msg("ttv team"..team..": "..  formatTime(Glory:GetTeamTTV(team+1)));
		...
		--[[if (basediff == 1) then
			...
			if selfTeam == team then
				...
				if (timediff > 0) then
					-- loosing
					SendChatMessage("<战场助手> 我方攻占了一个建筑, 但我方还是会在 "..formatTime(Glory:GetTeamTTV(otherTeam+1)).." 后输掉!", msgTarget);
				else
					-- winning
					SendChatMessage("<战场助手> 我方攻占了一个建筑, 并且我方会在 "..formatTime(Glory:GetTeamTTV(selfTeam+1)).." 后胜利!", msgTarget);
				end
			else
				-- enemy did it
				if (timediff > 0) then
					-- loosing
					SendChatMessage("<战场助手> 敌方攻占了一个建筑, 我方会在 "..formatTime(Glory:GetTeamTTV(otherTeam+1)).." 后输掉!", msgTarget);
				else
					-- winning
					SendChatMessage("<战场助手> 敌方攻占了一个建筑, 但我方还是会在 "..formatTime(Glory:GetTeamTTV(selfTeam+1)).." 后胜利!", msgTarget);
				end
			end]]
