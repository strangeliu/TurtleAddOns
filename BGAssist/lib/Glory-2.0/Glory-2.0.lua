...
if locale ~= "deDE" and locale ~= "zhCN" then
	locale = "enUS"
end

if locale == "enUS" then
	PATTERN_HORDE_FLAG_PICKED_UP = "The Horde [Ff]lag was picked up by ([^!]+)!"
	PATTERN_HORDE_FLAG_DROPPED = "The Horde [Ff]lag was dropped by ([^!]+)!"
	PATTERN_HORDE_FLAG_CAPTURED = "([^!]+) captured the Horde [Ff]lag!"
	PATTERN_ALLIANCE_FLAG_PICKED_UP = "The Alliance [Ff]lag was picked up by ([^!]+)!"
	PATTERN_ALLIANCE_FLAG_DROPPED = "The Alliance [Ff]lag was dropped by ([^!]+)!"
	PATTERN_ALLIANCE_FLAG_CAPTURED = "([^!]+) captured the Alliance [Ff]lag!"
...
elseif locale == "zhCN" then
	...
	-- 部落旗帜（联盟玩家捡起）
	PATTERN_HORDE_FLAG_PICKED_UP = "部落的?旗帜被 *([^ !]+) *捡起了?"
	PATTERN_HORDE_FLAG_DROPPED = "部落的?旗帜被 *([^ !]+) *丢下了?"
	PATTERN_HORDE_FLAG_DROPPED_ALT = "部落的?旗帜由 *([^ !]+) *失落了?"
	PATTERN_HORDE_FLAG_CAPTURED = "([^ !]+) *夺取了?部落的?旗帜!"

	-- 联盟旗帜（部落玩家捡起）
	PATTERN_ALLIANCE_FLAG_PICKED_UP = "联盟旗帜被 *([^ !]+) *捡起[!！]"
	PATTERN_ALLIANCE_FLAG_DROPPED = "联盟的?旗帜被 *([^ !]+) *丢下了?"
	PATTERN_ALLIANCE_FLAG_DROPPED_ALT = "联盟的?旗帜由 *([^ !]+) *失落了?"
	PATTERN_ALLIANCE_FLAG_CAPTURED = "([^ !]+) *夺取了?联盟的?旗帜!"

	FACTION_DEFILERS = "污染者"
	FACTION_FROSTWOLF_CLAN = "霜狼氏族"
	FACTION_WARSONG_OUTRIDERS = "战歌侦察骑兵"

	FACTION_LEAGUE_OF_ARATHOR = "阿拉索联军"
	FACTION_STORMPIKE_GUARD = "雷矛卫队"
	FACTION_SILVERWING_SENTINELS = "银翼哨兵"
	...
	PATTERN_GWSUII_SCORE = "(%d+/%d+)"
	PATTERN_GWSUII_BASES = "Bases: (%d+)"
	PATTERN_GWSUII_RESOURCES = "Resources: (%d+)"
	PATTERN_OBJECTIVE_HOLDER = "([%w ]+) Controlled"
end
...
function events:CHAT_MSG_BG_SYSTEM_HORDE(text)
	if Glory:IsInWarsongGulch() then
		...
		local _, _, hordeFC = string.find(cleanText, PATTERN_ALLIANCE_FLAG_PICKED_UP)
		...
		if hordeFC then
			...
			local altPattern = "联盟旗帜被%s*(.-)%s*捡起[!！]"
			local _, _, altName = string.find(cleanText, altPattern)
			if altName then
				...
				local dfName = deformat(cleanText, "联盟旗帜被 %s 捡起!") or deformat(cleanText, "联盟旗帜被 %s 捡起！")
				if dfName then
		...
		if string.find(cleanText, PATTERN_ALLIANCE_FLAG_CAPTURED) then
			local allianceFC = Glory.allianceFC
			Glory.allianceFC = nil
			Glory.hordeFC = nil
			events:TriggerEvent("Glory_AllianceFlagCaptured", allianceFC)
			if not isHorde then
		...
		if string.find(text, PATTERN_HORDE_FLAG_DROPPED) or string.find(text, PATTERN_HORDE_FLAG_DROPPED_ALT) then
			local allianceFC = Glory.allianceFC
			Glory.allianceFC = nil
...
function events:CHAT_MSG_BG_SYSTEM_ALLIANCE(text)
	if Glory:IsInWarsongGulch() then
		...
		if string.find(text, PATTERN_HORDE_FLAG_CAPTURED) then
			...
			Glory.allianceFC = nil
			Glory.hordeFC = nil
			events:TriggerEvent("Glory_HordeFlagCaptured", hordeFC)
			if isHorde then
		...
		if string.find(text, PATTERN_ALLIANCE_FLAG_DROPPED) or string.find(text, PATTERN_ALLIANCE_FLAG_DROPPED_ALT) then
			local hordeFC = Glory.hordeFC
			Glory.hordeFC = nil
