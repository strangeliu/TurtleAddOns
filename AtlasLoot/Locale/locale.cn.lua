local AL = AceLibrary("AceLocale-2.2"):new("AtlasLoot");

local GREY = "|cff999999";
local RED = "|cffff0000";
local WHITE = "|cffFFFFFF";
local GREEN = "|cff1eff00";
local PURPLE = "|cff9F3FFF";
local BLUE = "|cff0070dd";
local ORANGE = "|cffFF8400";
local DEFAULT = "|cffFFd200";

AL:RegisterTranslations("zhCN", function() return {
    [" added to the WishList."] = " 被添加到装备需求表",
    [" already in the WishList!"] = " 已经在装备需求表里了",
    [" could not be accessed, the following module may be out of date: "] = "无法进行操作，以下模块可能已过期：",
    [" deleted from the WishList."] = " 已从装备需求表删除",
    [" is safe."] = "的连接是安全的。",
    [" is unavailable, the following load on demand module is required: "] = "不可用，需要以下需求时载入型模块：",
    [" not listed in loot table registry, please report this message to the AtlasLoot forums at http://www.atlasloot.net"] = "没有被列出，请将该错误信息发送到 AtlasLoot 官方论坛：http://www.atlasloot.net。",
    ["(has random enchantment)"] = "(随机附魔)",
    [").  Depending on changes, there may be the occasional error, so please visit http://www.atlasmod.com as soon as possible to update."] = "）不符。该情况下可能会频繁出现插件错误信息。鉴于此，请您立刻访问 http://www.atlasmod.com 或 http://www.dreamgen.cn 更新您的 Atlas 版本。",
    [".  Right click on any other item to refresh the loot page."] = "。右键点击其他物品可刷新物品列表。",
    ["10 Slot"] = "10格",
    ["16 Slot"] = "16格",
    ["18 Slot"] = "20格",
    ["20 Slot"] = "22格",
    ["28 Slot Soul Shard"] = "28格灵魂袋",
    ["AQ Enchants"] = "安其拉掉落的附魔公式",
    ["AQ Opening Quest Chain"] = "安其拉之门任务奖励",
    ["AQ20 Class Sets"] = "安其拉废墟职业套装",
    ["AQ40 Class Sets"] = "安其拉神殿职业套装",
    ["Abyssal Council"] = "深渊议会",
    ["Accesories and Weapons"] = "杂物与武器",
    ["Accessories"] = "杂物",
    ["Adamantite Battlegear"] = "精金战甲",
    ["Add to QuickLooks:"] = "添加到快捷浏览",
    ["Aether-tech Adept"] = "资深以太技师",
    ["Aether-tech Assistant"] = "以太技师助理",
    ["Aether-tech Master"] = "高级以太技师",
    ["Aggem Thorncurse"] = "阿格姆",
    ["All Available Modules Loaded"] = "所有可用数据模块已载入",
    ["All modules"] = "全部模块",
    ["Anvilrage Captain"] = "铁怒上尉",
    ["Anvilrage Marshal"] = "铁怒队长",
    ["Arathi Basin Sets"] = "阿拉希盆地套装",
    ["Arcane Resistance Gear"] = "奥术抗性装备",
    ["Arcanist Regalia"] = "奥术师",
    ["Arcanoweave Vestments"] = "奥法交织套装",
    ["Arcatraz Sentinel"] = "禁魔监狱斥候",
    ["Armbreaker Huffaz"] = "断臂者霍法斯",
    ["Armor and Weapons"] = "护甲与武器",
    ["Assign this loot table\n to QuickLook"] = "将该掉落表添加到快捷浏览中",
    ["Atal'ai Witch Doctor"] = "阿塔莱巫医",
    ["AtlasLoot Error!"] = "AtlasLoot 发生错误！",
    ["Atlasloot Options"] = "Atlasloot 设置",
    ["Augur's Regalia"] = "预言者套装",
    ["AutoQuery items on loot tables |cffff0000(disconnection risk)|r"] = "打开掉落表后自动进行查询 |cffff0000(有掉线风险)|r",
    ["Avenger's Battlegear"] = "复仇者",
    ["Azure Templar"] = "碧蓝圣殿骑士",
    ["Back"] = "背部",
    ["Baelog's Chest"] = "巴尔洛戈的箱子",
    ["Balzaphon"] = "巴尔萨冯",
    ["Banner"] = "旗帜",
    ["Barleybrew Brewery"] = "麦酒佳酿",
    ["Bash'ir Landing Stasis Chambers"] = "巴什伊尔码头静止间",
    ["Battlecast Garb"] = "战斗施法套装",
    ["Battlegear of Eternal Justice"] = "永恒的公正",
    ["Battlegear of Heroism"] = "英勇",
    ["Battlegear of Might"] = "力量",
    ["Battlegear of Undead Slaying"] = "亡灵毁灭者",
    ["Battlegear of Unyielding Strength"] = "坚定的力量",
    ["Battlegear of Valor"] = "勇气",
    ["Battlegear of Wrath"] = "愤怒",
    ["Beastmaster Armor"] = "兽王",
    ["Beaststalker Armor"] = "野兽追猎者",
    ["Black Dragon Mail"] = "黑龙锁甲",
    ["Blackhand Assassin"] = "黑手刺客",
    ["Blackhand Elite"] = "黑手精英",
    ["Blood Tiger Harness"] = "血虎",
    ["Bloodfang Armor"] = "血牙",
    ["Bloodmail Regalia"] = "血链",
    ["Bloodsoul Embrace"] = "血魂的拥抱",
    ["Bloodvine Garb"] = "血藤",
    ["Blue Dragon Mail"] = "蓝龙锁甲",
    ["Blue Scepter Shard"] = "蓝色节杖碎片",
    ["BoE World Epics"] = "世界掉落的史诗装备",
    ["Bone Witch"] = "骨巫",
    ["Bonescythe Armor"] = "骨镰",
    ["Book"] = "书籍",
    ["Box of Chocolates"] = "一盒巧克力",
    ["Brightly Colored Egg"] = "复活节彩蛋",
    ["Burning Rage"] = "钢铁之怒",
    ["Cadaverous Garb"] = "苍白",
    ["Carapace of the Old God"] = "上古之神的甲壳",
    ["Card Game Item"] = "卡片游戏奖品",
    ["Carefully Wrapped Present"] = "精心包裹的礼物",
    ["Cenarion Raiment"] = "塞纳里奥",
    ["Chain of the Scarlet Crusade"] = "血色十字军链甲",
    ["Champion's Arcanum"] = "勇士的秘密",
    ["Champion's Battlearmor"] = "勇士的战铠",
    ["Champion's Dreadgear"] = "勇士的鬼纹",
    ["Champion's Guard"] = "勇士的套装",
    ["Champion's Investiture"] = "勇士的授权",
    ["Champion's Pursuance"] = "勇士的职责",
    ["Champion's Redoubt"] = "勇士的壁垒",
    ["Champion's Refuge"] = "勇士的庇护",
    ["Champion's Stormcaller"] = "勇士的风暴",
    ["Children's Week"] = "儿童周",
    ["Choose Table ..."] = "请选择……",
    ["Class Books"] = "职业书籍",
    ["Class Set Pieces"] = "职业套装部件",
    ["Classic Instances"] = "旧世界副本",
    ["Clear"] = "重置",
    ["Click boss name to view loot."] = "点击首领名以浏览掉落",
    ["Cloak"] = "披风",
    ["Close Menu"] = "关闭",
    ["Cluster Rocket Recipes"] = "烟花束设计图",
    ["Coilfang Oracle"] = "盘牙先知",
    ["Coilfang Sorceress"] = "盘牙巫师",
    ["Collections"] = "套装/收藏",
    ["Combat Pet"] = "战斗宠物",
    ["Confessor's Raiment"] = "忏悔者衣饰",
    ["Conjured Item"] = "魔法制造的物品",
    ["Conqueror's Battlegear"] = "征服者",
    ["Conspicuous Urn"] = "显眼的石罐",
    ["Coren Direbrew"] = "科伦·恐酒",
    ["Crafted Epic Weapons"] = "制造出的史诗武器",
    ["Crafted Sets"] = "制造出的套装",
    ["Crafting"] = "制造的物品",
    ["Crafting Reagent"] = "商业原料",
    ["Crimson Battle Mage"] = "红衣战斗法师",
    ["Crimson Inquisitor"] = "红衣审查者",
    ["Crimson Sorcerer"] = "红衣法术师",
    ["Crimson Templar"] = "赤红圣殿骑士",
    ["Cryptstalker Armor"] = "地穴追猎者",
    ["DKP"] = "DKP",
    ["Tribute Chest"] = "厄运之槌北区贡品",
    ["Dal'Rend's Arms"] = "雷德双刀",
    ["Dark Coffer"] = "黑暗宝箱",
    ["Darkmantle Armor"] = "暗幕",
    ["Darkmoon Faire Card"] = "暗月马戏团卡片",
    ["Darkscreecher Akkarai"] = "黑暗尖啸者阿克卡莱",
    ["Deathbone Guardian"] = "亡者之骨",
    ["Deathdealer's Embrace"] = "死亡执行者的拥抱",
    ["Deathmist Raiment"] = "死雾",
    ["Default Tooltips"] = "默认提示样式",
    ["Defias Leather"] = "迪菲亚皮甲",
    ["Defias Strip Miner"] = "迪菲亚赤膊矿工",
    ["Demoniac's Threads"] = "恶魔师护甲",
    ["Devilsaur Armor"] = "魔暴龙护甲",
    ["Dire Maul Books"] = "厄运之槌书籍",
    ["Doan's Strongbox"] = "杜安的保险箱",
    ["Don Carlos"] = "唐·卡洛斯",
    ["Done"] = "确定",
    ["D2 Summonable"] = "D2召唤物",
    ["Doomcaller's Attire"] = "厄运召唤者",
    ["Doomforge Arcanasmith"] = "厄炉魔匠",
    ["Doomforge Craftsman"] = "厄炉工匠",
    ["Dragonstalker Armor"] = "巨龙追猎者",
    ["Dreadmist Raiment"] = "鬼雾",
    ["Dreadnaught's Battlegear"] = "无畏",
    ["Dreamwalker Raiment"] = "梦游者",
    ["Drohn's Distillery"] = "德罗恩的佳酿",
    ["Drop Rate: "] = "掉落率: ",
    ["Druid of the Fang"] = "尖牙德鲁伊",
    ["Dukes"] = "公爵",
    ["Dungeon 1/2 Sets"] = "地下城套装1/2",
    ["Dungeon Set 1"] = "地下城套装1",
    ["Dungeon Set 2 Summonable"] = "地下城套装2任务首领",
	["Dungeons & Raids"] = "地下城&团队副本",
    ["Durnholde Rifleman"] = "敦霍尔德火枪手",
    ["Earth"] = "地",
    ["Earthen Templar"] = "土色圣殿骑士",
    ["Elemental Invasion"] = "元素入侵",
    ["Emblems of Veiled Shadows"] = "笼罩的阴影",
    ["Embrace of the Viper"] = "毒蛇的拥抱",
    ["Enable all Chat Links |cffff0000(use at own risk)|r"] = "使用所有连接 |cffff0000(有掉线风险)|r",
    ["Enchant"] = "附魔",
    ["Enchanted Adamantite Armor"] = "魔化精金套装",
    ["Enigma Vestments"] = "神秘套装",
    ["Epic Rewards"] = "史诗级别奖励",
    ["Epic Set"] = "史诗级套装",
    ["Eredar Deathbringer"] = "艾瑞达死亡使者",
    ["Eric 'The Swift'"] = "艾瑞克",
    ["Ethereal Priest"] = "虚灵牧师",
    ["Factions"] = "阵营",
    ["Faith in Felsteel"] = "魔钢的信仰",
    ["Father Flame"] = "烈焰之父",
    ["Fathom Core"] = "深渊之核",
    ["Feast of Winter Veil"] = "冬幕节",
    ["Fel Iron Chain"] = "魔铁链甲",
    ["Fel Iron Plate"] = "魔铁板甲",
    ["Fel Skin"] = "魔能之肤",
    ["Fel Tinkerer Zortan"] = "魔能工匠索尔坦",
    ["Felheart Raiment"] = "恶魔之心",
    ["Felscale Armor"] = "魔鳞套装",
    ["Felstalker Armor"] = "魔能猎手",
    ["Felvine Shard"] = "魔藤碎片",
    ["Fengus's Chest"] = "芬古斯的箱子",
    ["Feralheart Raiment"] = "狂野之心",
    ["Festive Gift"] = "节日礼物",
    ["Field Marshal's Aegis"] = "元帅的庇护",
    ["Field Marshal's Battlegear"] = "元帅的战甲",
    ["Field Marshal's Earthshaker"] = "元帅的震撼",
    ["Field Marshal's Pursuit"] = "元帅的猎装",
    ["Field Marshal's Raiment"] = "元帅的神服",
    ["Field Marshal's Regalia"] = "元帅的法衣",
    ["Field Marshal's Sanctuary"] = "元帅的圣装",
    ["Field Marshal's Threads"] = "元帅的魔装",
    ["Field Marshal's Vestments"] = "元帅的制服",
    ["Finery of Infinite Wisdom"] = "无尽的智慧",
    ["Fire"] = "火",
    ["Fire Resistance Gear"] = "火焰抗性装备",
    ["Firebrand Grunt"] = "火印步兵",
    ["Firebrand Invoker"] = "火印祈求者",
    ["Firebrand Legionnaire"] = "火印军团战士",
    ["Firebrand Pyromancer"] = "火印炎术师",
    ["Fireworks"] = "焰火",
    ["First Prize"] = "第一名奖励",
    ["Fish"] = "鱼",
    ["Flame Guard"] = "烈焰卫士",
    ["Forgosh"] = "弗尔高什",
    ["Freethinker's Armor"] = "思考者护甲",
    ["Frost Resistance Gear"] = "冰霜抗性装备",
    ["Frostfire Regalia"] = "霜火",
    ["Frostwhisper's Embalming Fluid"] = "莱斯·霜语的防腐液",
    ["Fury of the Nether"] = "虚空之怒",
    ["Gaily Wrapped Present"] = "微微震动的礼物",
    ["Garb of the Undead Slayer"] = "亡灵屠戮者的装备",
    ["Gargantuan Abyssal"] = "巨型深渊火魔",
    ["Garments of the Oracle"] = "神谕者",
    ["Garrett Family Chest"] = "加勒特的宝箱",
    ["Genesis Raiment"] = "起源套装",
    ["Gently Shaken Gift"] = "精美的礼品",
    ["Gezzarak the Huntress"] = "猎手吉萨拉克",
    ["Ghoul Ravener"] = "食尸抢夺者",
    ["Giantstalker Armor"] = "巨人追猎者",
    ["Gift of Adoration"] = "爱慕的礼物",
    ["Gift of the Gathering Storm"] = "聚集的风暴",
    ["Gordok Brewery"] = "戈多克佳酿",
    ["Gorgolon the All-seeing"] = "全视者格苟尔隆",
    ["Green Dragon Mail"] = "绿龙锁甲",
    ["Green Scepter Shard"] = "绿色节杖碎片",
    ["Gul'bor"] = "古尔博",
    ["Gurubashi Arena"] = "古拉巴什竞技场",
    ["Hallow's End"] = "万圣节",
    ["Haruspex's Garb"] = "占卜师套装",
    ["Harvest Festival"] = "收获节",
    ["Headless Horseman"] = "无头骑士",
    ["Henry Stern"] = "亨利·斯特恩",
    ["Herbalism Knife"] = "采药刀",
    ["Herod/Mograine"] = "赫洛德/莫格莱尼",
    ["Heroic"] = "英雄模式",
    ["Hide AtlasLoot Panel"] = "隐藏 AtlasLoot 面板",
    ["High Council"] = "议会高层",
    ["Hoary Templar"] = "苍白圣殿骑士",
    ["Husk of the Old God"] = "上古之神的外鞘",
    ["If checked, AtlasLoot will load and search across all the modules."] = "如果选中，AtlasLoot会载入并扫描所有的模块。",
    ["Illusionist's Attire"] = "幻术师套装",
    ["Imbued Netherweave"] = "魔化灵纹套装",
    ["Imperial Plate"] = "君王板甲",
    ["Implements of Unspoken Names"] = "禁断的邪语",
    ["Ironfeather Armor"] = "铁羽护甲",
    ["Ironweave Battlesuit"] = "铁纹作战套装",
    ["It has been detected that your version of Atlas does not match the version that Atlasloot is tuned for ("] = "AtlasLoot 检测到您正在使用的 Atlas 插件的版本与 AtlasLoot 需要的版本（",
    ["Item Unavailable"] = "物品不可用",
    ["ItemSync Tooltips"] = "ItemSync 提示样式",
    ["Judgement Armor"] = "审判",
    ["Kalldan Felmoon"] = "卡尔丹·暗月",
    ["Karrog"] = "卡尔洛格",
    ["Khorium Ward"] = "氪金套装",
    ["King Dorfbruiser"] = "多弗布鲁瑟国王",
    ["Knot Thimblejack"] = "诺特·希姆加克",
    ["Krom Stoutarm's Chest"] = "克罗姆·粗臂的箱子",
    ["Lady Falther'ess"] = "法瑟蕾丝夫人",
    ["Large Cluster Rocket Recipes"] = "大型烟花束设计图",
    ["Large Rocket Recipes"] = "大型烟花设计图",
    ["Last Result"] = "上次搜索",
    ["Lawbringer Armor"] = "秩序之源",
    ["Left Half"] = "左半部分",
    ["Legendary Items"] = "传奇物品",
    ["Level 10-19 Rewards"] = "等级10-19奖励",
    ["Level 20-29 Rewards"] = "等级20-29奖励",
    ["Level 30-39"] = "等级 30-39",
    ["Level 30-39 Rewards"] = "等级30-39奖励",
    ["Level 40-49"] = "等级 40-49",
    ["Level 40-49 Rewards"] = "等级40-49奖励",
    ["Level 50-59 Rewards"] = "等级50-59奖励",
    ["Level 50-60"] = "等级 50-60",
    ["Level 60"] = "等级 60",
    ["Level 60 Rewards"] = "等级60奖励",
    ["Lieutenant Commander's Arcanum"] = "少校的秘密",
    ["Lieutenant Commander's Battlearmor"] = "少校的战铠",
    ["Lieutenant Commander's Dreadgear"] = "少校的鬼纹",
    ["Lieutenant Commander's Guard"] = "少校的护卫",
    ["Lieutenant Commander's Investiture"] = "少校的授权",
    ["Lieutenant Commander's Pursuance"] = "少校的职责",
    ["Lieutenant Commander's Redoubt"] = "少校的壁垒",
    ["Lieutenant Commander's Refuge"] = "少校的庇护",
    ["Lieutenant Commander's Stormcaller"] = "少校的震撼暴",
    ["Lightforge Armor"] = "光铸护甲",
    ["Load Modules"] = "载入所有数据",
    ["Loading available tables for searching"] = "在已载入的物品数据中进行搜索",
    ["Lootlink Tooltips"] = "Lootlink 提示样式",
    ["Lord Ahune"] = "埃霍恩",
    ["Lord Blackwood"] = "布莱克伍德公爵",
    ["Lorgalis Manuscript"] = "洛迦里斯手稿",
    ["Love is in the Air"] = "情人节",
    ["Lucky Red Envelope"] = "红包",
    ["Lumbering Horror"] = "笨拙的憎恶",
    ["Lunar Festival"] = "春节",
    ["Lunar Festival Fireworks Pack"] = "春节烟花包",
    ["Madcap's Outfit"] = "狂妄者套装",
    ["Magister's Regalia"] = "博学者的徽记",
    ["Magregan Deepshadow"] = "马格雷甘·深影",
    ["Major Mojo Infusion"] = "极效魔精套装",
    ["Make Loot Table Opaque"] = "禁用物品列表背景透明",
    ["Malevus the Mad"] = "疯狂的玛尔弗斯",
    ["Malor's Strongbox"] = "玛洛尔的保险箱",
    ["Master Angler"] = "钓鱼大师",
    ["Matron Li-sahar"] = "里萨哈",
    ["Midsummer Fire Festival"] = "仲夏火焰节",
    ["Misc"] = "其他",
    ["Misc. Rewards"] = "其他奖励",
    ["Mount"] = "坐骑",
    ["Murk Worm"] = "黑暗虫",
    ["Namdo Bizzfizzle"] = "纳姆杜",
    ["Nature Resistance Gear"] = "自然抗性装备",
    ["Necropile Raiment"] = "骨堆",
    ["Nemesis Raiment"] = "复仇",
    ["Netherscale Armor"] = "虚空之鳞",
    ["Netherstrike Armor"] = "虚空打击",
    ["Netherweave Vestments"] = "灵纹套装",
    ["Netherwind Regalia"] = "灵风",
    ["New feature in 4.02.01: Type '/atlasloot options' to bring up the options menu and '/atlasloot reset' to reset AtlasLoot after a disconnect."] = "在4.02.01以上版本: 输入 '/atlasloot options'可以打开设置菜单，输入'/atlasloot reset'可以在登出游戏后重置AtlasLoot",
    ["New feature in 4.03.00: Introducing the Wishlist!  Simply alt-click on any item to add it to the wishlist.  To delete an item from the wishlist, open up your wishlist and alt-click the item to remove it.  It's that simple.  Buttons to view the wishlist have been added to the Atlas interface and the loot browser."] = "在4.03.00版本中添加了装备需求表！你可以按住Alt并点击任何物品的链接来添加它到装备需求表中。打开装备需求表，按住Alt并点击物品链接可以将它从表中删除。你可以点击Atlas界面内掉落浏览器上的相应按钮来打开装备需求表。",
    ["New feature in 4.05.00: Advanced searching functionality is now available. You can type in a partial item name, for example typing 'elixir' gives all items in the database with 'elixir' in the name.  Big thanks to Kurax for his help."] = "4.05.00新功能：高级搜索功能。现在可以使用模糊搜索方式，如在搜索框中输入“药剂”即可搜索艘有物品名中带有“药剂”字样的物品。非常感谢 Kurax 的帮助。",
    ["New feature in 4.05.00: All professions are now included in the AtlasLoot_Crafting module."] = "4.05.00新功能：所有专业技能信息现在都包含在 AtlasLoot_Crafting 模块中。",
    ["Nightslayer Armor"] = "夜幕杀手",
    ["No match found for"] = "未找到物品",
    ["Noblegarden"] = "彩蛋节",
    ["OK"] = "确定",
    ["Ogre Tannin Basket"] = "食人魔鞣酸篮",
    ["Olaf"] = "奥拉夫",
    ["Options"] = "设置",
    ["Ouro's Intact Hide"] = "奥罗的外皮",
    ["Overlord's Resolution"] = "督军的决心",
    ["Overmaster Pyron"] = "征服者派隆",
    ["Partial matching"] = "部分匹配",
    ["Path of the Conqueror"] = "征服者之路",
    ["Path of the Invoker"] = "祈求者之路",
    ["Path of the Protector"] = "保护者之路",
    ["Pet"] = "宠物",
    ["Phase 1"] = "第一阶段",
    ["Phase 2"] = "第二阶段",
    ["Phase 3"] = "第三阶段",
    ["Plagueheart Raiment"] = "瘟疫之心",
    ["Porfus the Gem Gorger"] = "掘钻者波弗斯",
    ["Prayer of the Primal"] = "远古祷言",
    ["Predator's Armor"] = "捕猎者套装",
    ["Primal Batskin"] = "原始蝙蝠皮套装",
    ["Primal Blessing"] = "原始祝福",
    ["Primal Hakkari Aegis"] = "原始哈卡莱之盾",
    ["Primal Hakkari Armsplint"] = "原始哈卡莱护臂",
    ["Primal Hakkari Bindings"] = "原始哈卡莱护腕",
    ["Primal Hakkari Girdle"] = "原始哈卡莱束带",
    ["Primal Hakkari Kossack"] = "原始哈卡莱套索",
    ["Primal Hakkari Sash"] = "原始哈卡莱腰带",
    ["Primal Hakkari Shawl"] = "原始哈卡莱披肩",
    ["Primal Hakkari Stanchion"] = "原始哈卡莱直柱",
    ["Primal Hakkari Tabard"] = "原始哈卡莱徽章",
    ["Primal Intent"] = "原始打击",
    ["Primal Mooncloth"] = "原始月布",
    ["Priority:"] = "优先: ",
    ["PvP Accessories"] = "PvP奖励杂物",
    ["PvP Accessories (Level 60)"] = "PvP奖励杂物 (等级 60)",
    ["PvP Armor Sets"] = "PvP奖励套装",
    ["PvP Honor System"] = "PvP荣誉系统",
    ["PvP Reputation Sets"] = "PvP声望套装",
    ["PvP Rewards"] = "PvP奖励",
    ["PvP Rewards (Level 60)"] = "PvP奖励 (等级 60)",
    ["PvP Weapons"] = "PvP奖励武器",
    ["PvP Weapons (Level 60)"] = "PvP奖励武器 (等级 60)",
    ["Qiraji Bindings of Command"] = "其拉命令腕轮",
    ["Qiraji Bindings of Dominance"] = "其拉统御腕轮",
    ["Qiraji Ceremonial Ring"] = "其拉典礼戒指",
    ["Qiraji Magisterial Ring"] = "其拉将领戒指",
    ["Qiraji Martial Drape"] = "其拉军用披风",
    ["Qiraji Ornate Hilt"] = "其拉装饰刀柄",
    ["Qiraji Regal Drape"] = "其拉帝王披风",
    ["Qiraji Spiked Hilt"] = "其拉尖刺刀柄",
    ["Queries the server for all items"] = "向服务器查询本页",
    ["Query Server"] = "查询物品",
    ["Quest Item"] = "任务物品",
    ["Quest Reward"] = "任务奖励",
    ["QuickLook"] = "快捷浏览",
    ["Raging Skeleton"] = "暴怒的骷髅",
    ["Raid"] = "团队",
    ["Rajaxx's Captains"] = "拉贾克斯的副官",
    ["Random"] = "随机",
    ["Random Boss Loot"] = "首领随机掉落物品",
    ["Rare"] = "稀有",
    ["Rare Fish"] = "稀有鱼种",
    ["Rare Fish Rewards"] = "稀有鱼种奖励",
    ["Rare Mounts"] = "稀有坐骑",
    ["Rare Set"] = "精良级套装",
    ["Razorfen Spearhide"] = "剃刀沼泽刺鬃守卫",
    ["Red Scepter Shard"] = "红色节杖碎片",
    ["Redemption Armor"] = "救赎",
    ["Regalia of Undead Cleansing"] = "亡灵净化者",
    ["Reinforced Fel Iron Chest"] = "强化魔铁箱",
    ["Reputation Factions"] = "声望阵营",
    ["Reset complete!"] = "重置完成",
    ["Rethilgore"] = "雷希戈尔",
    ["Revanchion"] = "雷瓦克安",
    ["Rewards"] = "奖励",
    ["Rift Keeper/Rift Lord"] = "裂隙守卫者/领主",
    ["Right Half"] = "右半部分",
    ["Roogug"] = "鲁古格",
    ["Ruins of Ahn'Qiraj Sets"] = "安其拉废墟职业套装",
    ["Safe Chat Links |cff1eff00(recommended)|r"] = "使用安全物品连接 |cff1eff00(推荐)|r",
    ["Scaled Draenic Armor"] = "缀鳞德拉诺套装",
    ["Scarlet Protector/Guardsman"] = "血色保卫者/卫兵",
    ["Scarlet Trainee"] = "血色预备兵",
    ["Scarshield Quartermaster"] = "裂盾军需官",
    ["Scepter of the Shifting Sands"] = "流沙节杖",
    ["Scholomance Adept"] = "通灵学院专家",
    ["Scholomance Dark Summoner"] = "通灵学院黑暗召唤师",
    ["Scope"] = "瞄准镜",
    ["Scorn"] = "瑟克恩",
    ["Scourge Invasion"] = "天灾入侵",
    ["Search"] = "搜索",
    ["Search Result: %s"] = "搜索结果：%s",
    ["Search on"] = "搜索于",
    ["Search options"] = "搜索选项",
    ["Select Loot Table"] = "选择掉落表",
    ["Select Sub-Table"] = "选择二级表",
    ["Server queried for "] = "已向服务器申请查询",
    ["Set"] = "套装",
    ["Set Minimap Button Position"] = "设置小地图图标位置",
    ["Set: Bloodmail Regalia"] = "套装：血链",
    ["Set: Cadaverous Garb"] = "套装：苍白",
    ["Set: Chain of the Scarlet Crusade"] = "套装：血色十字军链甲",
    ["Set: Dal'Rend's Arms"] = "套装：雷德双刀",
    ["Set: Deathbone Guardian"] = "套装：亡者之骨",
    ["Set: Defias Leather"] = "套装：迪菲亚皮甲",
    ["Set: Embrace of the Viper"] = "套装：毒蛇的拥抱",
    ["Set: Necropile Raiment"] = "套装：骨堆",
    ["Set: Spider's Kiss"] = "套装：蜘蛛之吻",
    ["Set: The Gladiator"] = "套装：角斗士",
    ["Set: The Postmaster"] = "套装：邮差",
    ["Sethekk Ravenguard"] = "塞泰克鸦人卫士",
    ["Sets/Collections"] = "套装/收藏",
    ["Setup"] = "设置",
    ["Sever"] = "塞沃尔",
    ["Shadow Resistance Gear"] = "暗影抗性装备",
    ["Shadow of Doom"] = "末日之影",
    ["Shadow's Embrace"] = "暗影之拥",
    ["Shadowcraft Armor"] = "迅影",
    ["Shadowforge Flame Keeper"] = "暗炉持火者",
    ["Shard of the Gods"] = "天神碎片",
    ["Shared"] = "共享掉落",
    ["Shartuul"] = "沙图尔",
    ["Shattered Hand Centurion"] = "碎手百夫长",
    ["Shen'dralar Provisioner"] = "辛德拉圣职者",
    ["Show Minimap Button"] = "显示小地图图标",
    ["Skin of the Great Sandworm"] = "巨型沙虫的皮",
    ["Skinning Knife"] = "剥皮刀",
    ["Small Rocket Recipes"] = "小型烟花设计图",
    ["Smokywood Pastures Extra-Special Gift"] = "烟林牧场的超级特殊礼物",
    ["Smokywood Pastures Vendor"] = "烟林牧场商人",
    ["Solus the Eternal"] = "永恒者索鲁斯",
    ["Sorcerer's Regalia"] = "巫师",
    ["Sothos and Jarien"] = "索托斯/亚雷恩",
    ["Soulcloth Embrace"] = "灵魂布之拥",
    ["Soulforge Armor"] = "魂铸",
    ["Spectral Citizen"] = "鬼魂市民",
    ["Spectral Researcher"] = "鬼灵研究员",
    ["Spellstrike Infusion"] = "法术打击",
    ["Spider's Kiss"] = "蜘蛛之吻",
    ["Spirestone Mystic"] = "尖石秘法师",
    ["Spirestone Warlord"] = "尖石军阀",
    ["Spirit of Eskhandar"] = "艾斯卡达尔之魂",
    ["Status of the following module could not be determined: "] = "以下模块的类型不可被确定：",
    ["Stormcaller's Garb"] = "风暴召唤者",
    ["Stormrage Raiment"] = "怒风",
    ["Stranglethorn Fishing Extravaganza"] = "荆棘谷钓鱼大赛",
    ["Strength of the Clefthoof"] = "裂蹄之力",
    ["Striker's Garb"] = "攻击者",
    ["Summon"] = "召唤",
    ["Sunseeker Astromage"] = "寻日者星术师",
    ["Sunseeker Botanist"] = "寻日者植物学家",
    ["Superior Rewards"] = "精良级别奖励",
    ["Suppress text spam when querying items"] = "查询物品时不显示提示信息",
    ["Symbols of Unending Life"] = "不灭的生命",
    ["T'chali's Voodoo Brewery"] = "塔卡里的佳酿",
    ["Tabards"] = "徽章",
    ["Templars"] = "圣殿骑士",
    ["Temple of Ahn'Qiraj Sets"] = "安其拉神殿职业套装",
    ["Terokk"] = "泰罗克",
    ["The Darksoul"] = "黑暗之魂",
    ["The Defiler's Determination"] = "污染者的果断",
    ["The Defiler's Fortitude"] = "污染者的坚韧",
    ["The Defiler's Intent"] = "污染者的专注",
    ["The Defiler's Purpose"] = "污染者的毅力",
    ["The Defiler's Resolution"] = "污染者的决心",
    ["The Defiler's Will"] = "污染者的意志",
    ["The Duke of Cynders"] = "灰烬公爵",
    ["The Duke of Fathoms"] = "深渊公爵",
    ["The Duke of Shards"] = "碎石公爵",
    ["The Duke of Zephyrs"] = "微风公爵",
    ["The Earthfury"] = "大地之怒",
    ["The Earthshatterer"] = "碎地者",
    ["The Elements"] = "元素",
    ["The Five Thunders"] = "五雷",
    ["The Gladiator"] = "角斗士",
    ["The Highlander's Determination"] = "高地人的果断",
    ["The Highlander's Fortitude"] = "高地人的坚韧",
    ["The Highlander's Intent"] = "高地人的专注",
    ["The Highlander's Purpose"] = "高地人的毅力",
    ["The Highlander's Resolution"] = "高地人的决心",
    ["The Highlander's Resolve"] = "高地人的执着",
    ["The Highlander's Will"] = "高地人的意志",
    ["The Nameles Prophet"] = "无名预言者",
    ["The Postmaster"] = "邮差",
    ["The Prince's Chest"] = "王子的箱子",
    ["The Secret Safe"] = "秘密保险箱",
    ["The Talon King's Coffer"] = "利爪之王的宝箱",
    ["The Ten Storms"] = "无尽风暴",
    ["The Twin Blades of Hakkari"] = "哈卡莱双刃",
    ["The Unyielding"] = "不屈的力量",
    ["The Vault"] = "黑色宝库",
    ["Theldren"] = "塞尔德林",
    ["Thick Draenic Armor"] = "厚重德莱尼套装",
    ["This Item Begins a Quest"] = "将触发一个任务",
    ["This item is not available on your server or your battlegroup yet."] = "该物品尚未在你所在的服务器或战场组中出现。",
    ["This item is unsafe.  To view this item without the risk of disconnection, you need to have first seen it in the game world. This is a restriction enforced by Blizzard since Patch 1.10."] = "该物品连接不安全。若想得知此物品的属性又想避免掉线问题，您需要在游戏内见过一次该物品。这是暴雪在1.10版本中做出的强制性改动。",
    ["Thomas Yance"] = "托马斯·杨斯",
    ["Thunderbrew Brewery"] = "雷酒佳酿",
    ["Thuzadin Shadowcaster"] = "图萨丁暗影法师",
    ["Ticking Present"] = "条纹礼物盒",
    ["Tier 1"] = "T1",
    ["Tier 2"] = "T2",
    ["Tier 3"] = "T3",
    ["Time-Lost Shadowmage"] = "迷失的暗影法师",
    ["Timed Reward Chest"] = "限时击杀首领奖励",
    ["Timed Reward Chest 1"] = "限时击杀首领奖励1",
    ["Timed Reward Chest 2"] = "限时击杀首领奖励2",
    ["Timed Reward Chest 3"] = "限时击杀首领奖励3",
    ["Timed Reward Chest 4"] = "限时击杀首领奖励4",
    ["Toggle AL Panel"] = "AtlasLoot 面板",
    ["Token"] = "兑换物",
    ["Token Hand-Ins"] = "需要兑换的奖励",
    ["Trade Goods"] = "杂货",
    ["Trappings of Vaulted Secrets"] = "魔法的秘密",
    ["Trappings of the Unseen Path"] = "隐秘的通途",
    ["Trash Mobs"] = "普通怪物",
    ["Treat Bag"] = "糖果包",
    ["Trelopades"] = "特雷洛帕兹",
    ["Tribute Run"] = "贡品",
    ["Unchained Doombringer"] = "摆脱束缚的厄运使者",
    ["Undead Slayer's Armor"] = "亡灵屠戮者的护甲",
    ["Unfinished Painting"] = "未完成的油画",
    ["Unforged Rune Covered Breastplate"] = "未铸造的符文覆饰胸甲",
    ["Unique"] = "唯一",
    ["Unknown"] = "未知",
    ["Unsafe Item"] = "不安全的物品",
    ["Use EquipCompare"] = "使用装备对比",
    ["Use to purchase rewards"] = "用以购买奖励",
    ["Use to purchase rewards (Alliance)"] = "用以购买奖励 (联盟)",
    ["Use to purchase rewards (Horde)"] = "用以购买奖励 (部落)",
    ["Used to summon boss"] = "用以召唤首领",
    ["Vakkiz the Windrager"] = "风怒者瓦克奇斯",
    ["Various Locations"] = "多个位置",
    ["Vek'lor's Diadem"] = "维克洛尔的王冠",
    ["Vek'nilash's Circlet"] = "维克尼拉斯的头饰",
    ["Vestments of Faith"] = "信仰",
    ["Vestments of Prophecy"] = "预言",
    ["Vestments of Transcendence"] = "卓越",
    ["Vestments of the Devout"] = "虔诚",
    ["Vestments of the Virtuous"] = "坚贞",
    ["Vindicator's Battlegear"] = "辩护者重甲",
    ["Volcanic Armor"] = "火山",
    ["Warlord's Aegis"] = "督军的庇护",
    ["Warlord's Battlegear"] = "督军的战甲",
    ["Warlord's Earthshaker"] = "督军的震撼",
    ["Warlord's Pursuit"] = "督军的猎装",
    ["Warlord's Raiment"] = "督军的神服",
    ["Warlord's Regalia"] = "督军的法衣",
    ["Warlord's Sanctuary"] = "督军的圣装",
    ["Warlord's Threads"] = "督军的魔装",
    ["Warlord's Vestments"] = "督军的制服",
    ["Water"] = "水",
    ["Weapon Technician"] = "武器技师",
    ["Weapons"] = "武器",
    ["Welcome to Atlasloot Enhanced.  Please take a moment to set your preferences for tooltips and links in the chat window.\n\n  This options screen can be reached again at any later time by typing '/atlasloot'."] = "欢迎使用 Atlasloot Enhanced。请花少许时间进来设置提示与物品连接的方式。\n\n  以后可以输入“/atlasloot”再次显示该设置窗口。",
    ["Welcome to Atlasloot Enhanced.  Please take a moment to set your preferences."] = "欢迎使用 Atlasloot Enhanced，请花少许时间进行参数设置",
    ["Whitemend Wisdom"] = "白色治愈",
    ["Wild Draenish Armor"] = "野性德莱尼套装",
    ["Wildheart Raiment"] = "野性之心",
    ["Wind"] = "风",
    ["Windhawk Armor"] = "风鹰",
    ["Winter Veil Gift"] = "冬幕节礼物",
    ["WishList"] = "装备需求表",
    ["WishList Full!"] = "装备需求表已满！",
    ["World Bosses"] = "世界首领",
    ["World Drop"] = "世界掉落",
    ["World Epics"] = "世界掉落的史诗装备",
    ["World Events"] = "世界事件",
    ["Wrath of Spellfire"] = "魔焰之怒",
    ["Wrathbringer Laz-tarash"] = "天罚使者拉塔莱什",
    ["You can right-click to attempt to query the server.  You may be disconnected."] = "您可以右键点击该物品以向服务器查询，但这样做有可能会与服务器断开连接。",
    ["You don't have any module selected to search on!"] = "你没有选择任何用于搜索的模块。",
    ["ZG Class Sets"] = "祖尔格拉布职业套装",
    ["ZG Enchants"] = "祖尔格拉布附魔",
    ["Enchants"] = "附魔",
    ["Zanzil's Concentration"] = "赞吉尔的专注",
    ["Zelemar the Wrathful"] = "愤怒者塞雷玛尔",
    ["Zul'Gurub Rings"] = "祖尔格拉布戒指",
    ["Zul'Gurub Sets"] = "祖尔格拉布职业套装",
    ["on this page. The items will be"] = "中的所有物品链接",
    ["options"] = "options",
    ["over them."] = "下次滑过时刷新",
    ["refreshed when you next mouse"] = "物品将会在鼠标",
    ["reset"] = "reset",
    ["sucessfully loaded."] = "已成功载入",
    ["|cff1eff00Left-Click|r Browse Loot Tables"] = "|cff1eff00单击|r 浏览掉落表",
    ["|cff9d9d9dItemSync Tooltips|r"] = "|cff9d9d9dItemSync 提示样式|r",
    ["|cff9d9d9dLootlink Tooltips|r"] = "|cff9d9d9dLootlink 提示样式",
    ["|cff9d9d9dUse EquipCompare|r"] = "|cff9d9d9d使用装备对比|r",
    ["|cffff0000(unsafe)"] = "|cffff0000(不安全)",
    ["|cffff0000Right-Click|r View Options"] = "|cffff0000右键点击|r 进行设置",
    ["|cffff0000Shift-Click|r View Options"] = "|cffff0000Shift-单击|r 进行设置",
	
	["A Small Seaforium charge which is enough to blast open some doors."] = "足以炸开某些门的小型爆盐炸弹。",
	["Accurate Scope"] = "精确瞄准镜",
	["Admiral's Hat"] = "将军之帽",
	["Advanced Target Dummy"] = "高级假人",
	["Agatestone Crown"] = "玛瑙头冠",
	["Agitating Poison"] = "煽动毒药I",
	["Alarm-O-Bot"] = "报警机器人",
	["Alchemist's Stone"] = "炼金石",
	["Allows opening of hard locks.  The skeleton key is consumed in the process."] = "可以打开复杂的锁，需消耗万能钥匙。",
	["Allows the miner to smelt a chunk of copper ore into a copper bar.  Smelting copper requires a forge."] = "使矿工可以将一块铜矿石熔炼成一块铜锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of gold ore into a gold bar.  Smelting gold requires a forge."] = "使矿工可以将一块金矿石熔炼成一块金锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of iron ore and a lump of coal together into a steel bar.  Smelting steel requires a forge."] = "使矿工可以将一块铁矿石和一份煤熔炼成一块钢锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of iron ore into an iron bar.  Smelting iron requires a forge."] = "使矿工可以将一块铁矿石熔炼成一块铁锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of mithril ore into a mithril bar.  Smelting mithril requires a forge."] = "使矿工可以将一块秘银矿石熔炼成一块秘银锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of silver ore into a silver bar.  Smelting silver requires a forge."] = "使矿工可以将一块银矿石熔炼成一块银锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of thorium ore into a thorium bar.  Smelting thorium requires a forge."] = "使矿工可以将一块瑟银矿石熔炼成一块瑟银锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of tin ore into a tin bar.  Smelting tin requires a forge."] = "使矿工可以将一块锡矿石熔炼成一块锡锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a chunk of truesilver ore into a truesilver bar.  Smelting truesilver requires a forge."] = "使矿工可以将一块真银矿石熔炼成一块真银锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a dreamsteel bar. Smelting dreamsteel requires a forge."] = "使矿工可以熔炼梦境钢锭。需要有熔炉才能进行。",
	["Allows the miner to smelt a tin bar and a copper bar together into two bronze bars.  Smelting bronze requires a forge."] = "使矿工可以将一块锡锭和一块铜锭熔炼成两块青铜锭。需要有熔炉才能进行。",
	["Allows the miner to smelt chunks of dark iron ore into a dark iron bar.  Smelting Dark Iron can only be done at the Black Forge in Blackrock Depths."] = "使矿工可以将黑铁矿石熔炼成黑铁锭。必须在黑石深渊中的黑铁熔炉中进行。",
	["Alluring Citrine Choker"] = "迷人的黄玉项链",
	["Amber Orb"] = "琥珀宝珠",
	["Amber Ring"] = "琥珀指环",
	["Ambersap Glazed Boar Ribs"] = "蜜汁烤猪排",
	["Amberstone Pendant"] = "琥珀石吊坠",
	["An improved scope that increases the damage done with your bow or gun."] = "改进型的瞄准镜，可以提高你的弓箭或枪械所造成的伤害。",
	["Ancient Dwarven Gemstone"] = "古代矮人宝石",
	["Annihilator"] = "歼灭者",
	["Anti-Venom"] = "抗毒药剂",
	["Aquadynamic Fish Attractor"] = "水下诱鱼器",
	["Aquamarine Pendant"] = "青绿石坠饰",
	["Arcane Bomb"] = "奥术炸弹",
	["Arcane Elixir"] = "奥法药剂",
	["Arcane Emerald Gemstone"] = "奥术翡翠",
	["Arcanite Belt Buckle"] = "奥金带扣",
	["Arcanite Champion"] = "奥金圣剑",
	["Arcanite Dragonling"] = "奥金幼龙",
	["Arcanite Reaper"] = "奥金斧",
	["Arcanite Rod"] = "奥金棒",
	["Arcanite Skeleton Key"] = "奥金万能钥匙",
	["Arcanum Baton"] = "奥秘之杖",
	["Arclight Spanner"] = "扳手",
	["Argent Boots"] = "银色长靴",
	["Argent Shoulders"] = "银色护肩",
	["Astral Amulet"] = "星界护符",
	["Astronomer Raiments 	"] = "观星者法袍",
	["Augerer's Boots"] = "预言家靴子",
	["Augerer's Gloves"] = "预言家手套",
	["Augerer's Hat"] = "预言家帽子",
	["Augerer's Mantle"] = "预言家护肩",
	["Augerer's Robe"] = "预言家长袍",
	["Augerer's Trousers"] = "预言家裤子",
	["Azerothian Ruby Gemstone"] = "艾泽拉斯红玉",
	["Azure Ring"] = "碧蓝戒指",
	["Azure Shoulders"] = "碧蓝护肩",
	["Azure Silk Belt"] = "碧蓝丝质腰带",
	["Azure Silk Cloak"] = "碧蓝丝质披风",
	["Azure Silk Gloves"] = "碧蓝丝质手套",
	["Azure Silk Hood"] = "碧蓝丝质头巾",
	["Azure Silk Pants"] = "碧蓝丝质短裤",
	["Azure Silk Vest"] = "碧蓝丝质外衣",
	["Baked Salmon"] = "烤鲑鱼",
	["Barbaric Belt"] = "野人腰带",
	["Barbaric Bracers"] = "野人护腕",
	["Barbaric Gloves"] = "野人手套",
	["Barbaric Harness"] = "野人背心",
	["Barbaric Iron Boots"] = "野人铁靴",
	["Barbaric Iron Breastplate"] = "野人铁质胸甲",
	["Barbaric Iron Gloves"] = "野人铁手套",
	["Barbaric Iron Helm"] = "野人铁盔",
	["Barbaric Iron Shoulders"] = "野人铁护肩",
	["Barbaric Leggings"] = "野人护腿",
	["Barbaric Linen Vest"] = "野人亚麻外衣",
	["Barbaric Shoulders"] = "野人护肩",
	["Barbecued Buzzard Wing"] = "烧烤秃鹰翅膀",
	["Basic Campfire"] = "基础营火",
	["Battery-Powered Crowd Pummeler"] = "电动惩戒器",
	["Beautiful Diamond Gemstone"] = "绚丽宝钻",
	["Beer Basted Boar Ribs"] = "啤酒烤猪排",
	["Belt of the Archmage"] = "大法师腰带",
	["Big Bag of Enchantment"] = "大附魔袋",
	["Big Bear Steak"] = "大块的熊排",
	["Big Black Mace"] = "巨型黑色锤",
	["Big Bronze Bomb"] = "重磅青铜炸弹",
	["Big Bronze Knife"] = "青铜匕首",
	["Big Iron Bomb"] = "重磅铁制炸弹",
	["Big Voodoo Cloak"] = "巫毒披风",
	["Big Voodoo Mask"] = "巫毒面具",
	["Big Voodoo Pants"] = "巫毒短裤",
	["Big Voodoo Robe"] = "巫毒长袍",
	["Binding Signet"] = "禁锢徽记",
	["Bindings of Luminance"] = "光亮护腕",
	["Biznicks 247x128 Accurascope"] = "比兹尼克247x128精确瞄准镜",
	["Black Amnesty"] = "野蛮狂怒",
	["Black Dragonscale Boots"] = "黑色龙鳞战靴",
	["Black Dragonscale Breastplate"] = "黑色龙鳞胸甲",
	["Black Dragonscale Leggings"] = "黑色龙鳞护腿",
	["Black Dragonscale Shoulders"] = "黑色龙鳞护肩",
	["Black Grasp of the Destroyer"] = "毁灭者的黑暗之握",
	["Black Mageweave Boots"] = "黑色魔纹之靴",
	["Black Mageweave Gloves"] = "黑色魔纹手套",
	["Black Mageweave Headband"] = "黑色魔纹头带",
	["Black Mageweave Leggings"] = "黑色魔纹短裤",
	["Black Mageweave Robe"] = "黑色魔纹长袍",
	["Black Mageweave Shoulders"] = "黑色魔纹护肩",
	["Black Mageweave Vest"] = "黑色魔纹外衣",
	["Black Silk Pack"] = "黑色丝质背包",
	["Black Swashbuckler's Shirt"] = "黑色冒险者衬衣",
	["Black Whelp Cloak"] = "黑色雏龙披风",
	["Black Whelp Tunic"] = "黑色雏龙外衣",
	["Blackfury"] = "黑色怒火",
	["Blackguard"] = "黑色卫士",
	["Blackmouth Oil"] = "黑口鱼油",
	["Blackrock Ironclamps"] = "黑石铁夹",
	["Blackwing Signet of Command"] = "支配之龙印",
	["Blast Shield"] = "爆炸护盾",
	["Blazefury Circlet"] = "光芒之怒指环",
	["Blazing Rapier"] = "闪耀轻剑",
	["Bleakwood Hew"] = "冷木斧",
	["Blight"] = "荒芜",
	["Blinding Powder"] = "致盲粉",
	["Blood Sausage"] = "血肠",
	["Blood Talon"] = "血爪",
	["Blood Tiger Breastplate"] = "血虎胸甲",
	["Blood Tiger Shoulders"] = "血虎护肩",
	["Bloodfire Circlet"] = "血火头饰",
	["Bloodsoul Breastplate"] = "血魂胸甲",
	["Bloodsoul Gauntlets"] = "血魂护手",
	["Bloodsoul Shoulders"] = "血魂护肩",
	["Bloodstone Warblade"] = "血石战刃",
	["Bloodvine Boots"] = "血藤长靴",
	["Bloodvine Goggles"] = "血藤护目镜",
	["Bloodvine Leggings"] = "血藤护腿",
	["Bloodvine Lens"] = "血藤透镜",
	["Bloodvine Vest"] = "血藤外套",
	["Bloody Belt Buckle"] = "血腥带扣",
	["Blue Dragonscale Boots"] = "蓝色龙鳞长靴",
	["Blue Dragonscale Breastplate"] = "蓝龙鳞片胸甲",
	["Blue Dragonscale Leggings"] = "蓝龙鳞片护腿",
	["Blue Dragonscale Shoulders"] = "蓝龙鳞片护肩",
	["Blue Firework"] = "蓝色焰火",
	["Blue Glittering Axe"] = "蓝光斧",
	["Blue Linen Robe"] = "蓝色亚麻长袍",
	["Blue Linen Shirt"] = "蓝色亚麻衬衣",
	["Blue Linen Vest"] = "蓝色亚麻外衣",
	["Blue Overalls"] = "蓝色罩衫",
	["Blue Rocket Cluster"] = "蓝色烟花束",
	["Blue Starfire"] = "天蓝星火",
	["Boiled Clams"] = "水煮蚌肉",
	["Bolt of Linen Cloth"] = "亚麻布卷",
	["Bolt of Mageweave"] = "魔纹布卷",
	["Bolt of Runecloth"] = "符文布卷",
	["Bolt of Silk Cloth"] = "丝绸卷",
	["Bolt of Woolen Cloth"] = "毛布卷",
	["Boots of Darkness"] = "黑暗之靴",
	["Boots of the Enchanter"] = "附魔师长靴",
	["Boots of the Wind"] = "风之靴",
	["Bottomless Bag"] = "无底包",
	["Bramblewood Belt"] = "荆木腰带",
	["Bramblewood Boots"] = "荆木长靴",
	["Bramblewood Helm"] = "荆木头盔",
	["Breastplate of the Earth"] = "大地胸甲",
	["Bright Copper Necklace"] = "亮铜项链",
	["Bright Copper Ring"] = "明亮的铜戒指",
	["Bright Yellow Shirt"] = "淡黄色衬衣",
	["Bright-Eye Goggles"] = "增亮护目镜",
	["Brightcloth Cloak"] = "亮布披风",
	["Brightcloth Gloves"] = "亮布手套",
	["Brightcloth Pants"] = "亮布短裤",
	["Brightcloth Robe"] = "亮布长袍",
	["Brilliant Opal Gemstone"] = "闪耀白玉",
	["Brilliant Smallfish"] = "美味小鱼",
	["Bristle Whisker Catfish"] = "刺须鲶鱼",
	["Bronze Axe"] = "青铜斧",
	["Bronze Battle Axe"] = "青铜战斧",
	["Bronze Belt Buckle"] = "青铜带扣",
	["Bronze Bruiser"] = "青铜荣耀者",
	["Bronze Cuffed Bangles"] = "青铜手镯",
	["Bronze Framework"] = "青铜框架",
	["Bronze Greatsword"] = "青铜巨剑",
	["Bronze Mace"] = "青铜钉锤",
	["Bronze Scepter"] = "青铜权杖",
	["Bronze Shortsword"] = "青铜短剑",
	["Bronze Tube"] = "青铜管",
	["Bronze Warhammer"] = "青铜战锤",
	["Brown Linen Pants"] = "棕色亚麻短裤",
	["Brown Linen Robe"] = "棕色亚麻长袍",
	["Brown Linen Shirt"] = "棕色亚麻衬衣",
	["Brown Linen Vest"] = "棕色亚麻外衣",
	["Build a Goblin Land Mine."] = "制造地精暗雷。",
	["Builds a campfire that increases the spirits of those nearby by 4 and allows cooking."] = "点起一堆营火，使附近的冒险者们的精神提高4点，并可以在营火旁烹饪。",
	["Bulky Copper Ring"] = "笨重的铜戒指",
	["Burning Star Gemstone"] = "燃星宝石",
	["Carrion Surprise"] = "腐肉大餐",
	["Catseye Elixir"] = "猫眼药剂",
	["Catseye Ultra Goggles"] = "猫眼超级护目镜",
	["Cenarion Herb Bag"] = "塞纳里奥草药包",
	["Centaur Battle Harness"] = "半人马战甲",
	["Centaur Hoof Circlet"] = "半人马指环",
	["Charred Wolf Meat"] = "烧烤狼肉",
	["Chimeric Boots"] = "奇美拉长靴",
	["Chimeric Gloves"] = "奇美拉手套",
	["Chimeric Leggings"] = "奇美拉护腿",
	["Chimeric Vest"] = "奇美拉外衣",
	["Chromatic Cloak"] = "多彩披风",
	["Chromatic Gauntlets"] = "多彩护手",
	["Chromatic Leggings"] = "多彩护腿",
	["Cindercloth Boots"] = "灰布长靴",
	["Cindercloth Cloak"] = "灰布披风",
	["Cindercloth Gloves"] = "灰布手套",
	["Cindercloth Pants"] = "灰布短裤",
	["Cindercloth Robe"] = "灰布长袍",
	["Cindercloth Vest"] = "灰布外衣",
	["Cinderfall Band"] = "灰烬堕落戒指",
	["Circlet of Dampening"] = "抑制头环",
	["Clam Chowder"] = "杂烩蚌肉",
	["Cleaning Cloth"] = "清洁布",
	["Cloak of Fire"] = "火焰披风",
	["Cloak of Warding"] = "护卫披风",
	["Coarse Blasting Powder"] = "粗制火药粉",
	["Coarse Dynamite"] = "粗制炸药",
	["Coarse Gemstone Cluster"] = "粗糙的宝石串",
	["Coarse Grinding Stone"] = "粗制砂轮",
	["Coarse Gritted Paper"] = "粗糙砂纸",
	["Coarse Sharpening Stone"] = "粗制磨刀石",
	["Coarse Weightstone"] = "粗制平衡石",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 112 to 148 Nature damage.  115 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即对其造成112到148点自然伤害。可使用115次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 19 to 25 Nature damage.  40 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即对其造成19到25点自然伤害。可使用40次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 30 to 38 Nature damage.  55 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即对其造成30到38点自然伤害。可使用55次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 44 to 56 Nature damage.  70 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即对其造成44到56点自然伤害。可使用70次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 67 to 85 Nature damage, and causes additional threat.  115 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即造成67到85点自然伤害，并产生额外的威胁值。可使用115次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 67 to 85 Nature damage.  85 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即对其造成67到85点自然伤害。可使用85次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy which instantly inflicts 92 to 118 Nature damage.  100 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，立即对其造成92到118点自然伤害。可使用100次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy, increasing their casting time by 40% for 10 sec.  50 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，使其施法时间延长40%，持续10秒。可使用50次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy, increasing their casting time by 50% for 12 sec.  75 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，使其施法时间延长50%，持续12秒。可使用75次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 20% chance of poisoning the enemy, increasing their casting time by 60% for 14 sec.  100 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有20%的几率令敌人中毒，使其施法时间延长60%，持续14秒。可使用100次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy for 108 Nature damage over 12 sec.  Stacks up to 5 times on a single target.  105 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成108点自然伤害，对单一目标最多可以叠加5次。可使用105次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy for 136 Nature damage over 12 sec.  Stacks up to 5 times on a single target.  105 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成136点自然伤害，对单一目标最多可以叠加5次。可使用105次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy for 36 Nature damage over 12 sec.  Stacks up to 5 times on a single target.  60 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成36点自然伤害，对单一目标最多可以叠加5次。可使用60次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy for 52 Nature damage over 12 sec.  Stacks up to 5 times on a single target.  75 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成52点自然伤害，对单一目标最多可以叠加5次。可使用75次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy for 80 Nature damage over 12 sec.  Stacks up to 5 times on a single target.  90 charges."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成80点自然伤害，对单一目标最多可以叠加5次。可使用90次。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy, reducing all healing effects used on them by 105 for 15 sec."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，使其在受到治疗时效果降低105点，持续15秒。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy, reducing all healing effects used on them by 135 for 15 sec."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，使其在受到治疗时效果降低135点，持续15秒。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy, reducing all healing effects used on them by 5% for 15 sec."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，使其在受到治疗时效果降低5点，持续15秒。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy, reducing all healing effects used on them by 75 for 15 sec."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，使其在受到治疗时效果降低75点，持续15秒。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy, slowing their movement speed by 50% for 12 sec."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，移动速度降低50%，持续12秒。",
	["Coats a weapon with poison that lasts for 30 minutes.\nEach strike has a 30% chance of poisoning the enemy, slowing their movement speed by 60% for 12 sec."] = "给一件武器浸毒，效果持续30分钟。\n每次攻击都有30%的几率令敌人中毒，移动速度降低60%，持续12秒。",
	["Coats a weapon with poison that lasts for 60 minutes.\nEach strike has a 20% chance to poison the enemy, instantly inflicting 106 to 142 damage. This damage ignores armor. 115 charges."] = "给一件武器浸毒，效果持续60分钟。\n每次攻击都有20%的几率使敌人中毒，并立即造成106到142点物理伤害。此伤害无视护甲，可使用115次。",
	["Coats a weapon with poison that lasts for 60 minutes.\nEach strike has a 20% chance to poison the enemy, instantly inflicting 88 to 113 damage. This damage ignores armor. 100 charges."] = "给一件武器浸毒，效果持续60分钟。\n每次攻击都有20%的几率使敌人中毒，并立即造成88到113点物理伤害。此伤害无视护甲，可使用100次。",
	["Coats a weapon with poison that lasts for 60 minutes. \nEach strike has a 30% chance of poisoning the enemy for 100 damage over 12 sec.  Stacks up to 5 times on a single target.  105 charges."] = "给一件武器浸毒，效果持续60分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成100点伤害，对单一目标最多可以叠加5次。可使用105次。",
	["Coats a weapon with poison that lasts for 60 minutes. \nEach strike has a 30% chance of poisoning the enemy for 128 damage over 12 sec.  Stacks up to 5 times on a single target.  105 charges."] = "给一件武器浸毒，效果持续60分钟。\n每次攻击都有30%的几率令敌人中毒，在12秒内对其造成128点伤害，对单一目标最多可以叠加5次。可使用105次。",
	["Colorful Kilt"] = "多彩褶裙",
	["Comfortable Leather Hat"] = "舒适的皮帽",
	["Compact Harvest Reaper Kit"] = "联合收割机组件",
	["Concoction of the Arcane Giant"] = "奥法巨人药剂",
	["Concoction of the Dreamwater"] = "梦酊火酒药剂",
	["Concoction of the Emerald Mongoose"] = "梦精猫鼬药剂",
	["Cooked Crab Claw"] = "煮蟹爪",
	["Cooked Glossy Mightfish"] = "煮熟的光滑大鱼",
	["Copper Axe"] = "铜斧",
	["Copper Bangle"] = "铜手镯",
	["Copper Battle Axe"] = "铜质战斧",
	["Copper Belt Buckle"] = "铜带扣",
	["Copper Bracers"] = "铜质护腕",
	["Copper Chain Belt"] = "铜质链甲腰带",
	["Copper Chain Boots"] = "铜质链甲战靴",
	["Copper Chain Pants"] = "铜质链甲短裤",
	["Copper Chain Vest"] = "铜质链甲外衣",
	["Copper Claymore"] = "铜质双刃刀",
	["Copper Dagger"] = "铜质匕首",
	["Copper Knuckles"] = "铜指虎",
	["Copper Mace"] = "铜质钉锤",
	["Copper Modulator"] = "铜质调节器",
	["Copper Shortsword"] = "铜质短剑",
	["Copper Staff"] = "铜杖",
	["Copper Tube"] = "铜管",
	["Core Armor Kit"] = "熔火护甲片",
	["Core Felcloth Bag"] = "熔火恶魔布包",
	["Core Marksman Rifle"] = "火核狙击步枪",
	["Corehound Belt"] = "熔火犬皮腰带",
	["Corehound Boots"] = "熔岩犬皮靴",
	["Corehound Gloves"] = "熔火猎犬手套",
	["Corrosive Poison"] = "腐蚀毒药",
	["Corrosive Poison II"] = "腐蚀毒药 II",
	["Corruption"] = "腐蚀术",
	["Cosmic Headdress"] = "宇宙头饰",
	["Cosmic Leggings"] = "宇宙护腿",
	["Cosmic Mantle"] = "宇宙衬肩",
	["Cosmic Vest"] = "宇宙外衣",
	["Coyote Steak"] = "山狗肉排",
	["Crab Cake"] = "蟹肉蛋糕",
	["Crafted Heavy Shot"] = "精制重弹丸",
	["Crafted Light Shot"] = "精制轻弹丸",
	["Crafted Solid Shot"] = "精制实心弹丸",
	["Craftsman's Monocle"] = "工匠眼镜",
	["Create the reagent for the Blind ability."] = "制造供致盲技能使用的材料。",
	["Creates 3 Vials of Anti-Venom."] = "制造3瓶抗毒抗毒药剂。",
	["Creates 3 Vials of Strong Anti-Venom."] = "制造3瓶强力抗毒药剂。",
	["Creates a Vial of Powerful Anti-Venom."] = "制造一瓶特效抗毒药剂。",
	["Creates two Iron Buckles that are used by other trade skills."] = "制造两块供其它商业技能使用的铁扣环。",
	["Crimson Silk Belt"] = "深红丝质腰带",
	["Crimson Silk Cloak"] = "深红丝质披风",
	["Crimson Silk Gloves"] = "深红丝质手套",
	["Crimson Silk Pantaloons"] = "深红丝质马裤",
	["Crimson Silk Robe"] = "深红丝质长袍",
	["Crimson Silk Shoulders"] = "深红丝质护肩",
	["Crimson Silk Vest"] = "深红丝质外衣",
	["Crippling Poison"] = "致残毒药",
	["Crippling Poison II"] = "致残毒药 II",
	["Crispy Bat Wing"] = "香脆蝙蝠翅",
	["Crispy Lizard Tail"] = "脆炸蜥蜴尾",
	["Crocolisk Gumbo"] = "鳄鱼浓汤",
	["Crocolisk Steak"] = "鳄鱼肉排",
	["Crown of Elegance"] = "高雅之冠",
	["Crown of Molten Ascension"] = "熔火升华之冠",
	["Crown of the Illustrious Queen"] = "辉煌女王的皇冠",
	["Crude Scope"] = "粗制瞄准镜",
	["Crystal Earring"] = "水晶耳环",
	["Crystalfire Armlets"] = "水晶火焰护腕",
	["Crystalweft Bracers"] = "水晶护腕",
	["Cured Heavy Hide"] = "熟化重毛皮",
	["Cured Light Hide"] = "熟化轻毛皮",
	["Cured Medium Hide"] = "熟化中毛皮",
	["Cured Rugged Hide"] = "熟化毛皮",
	["Cured Thick Hide"] = "熟化厚毛皮",
	["Curiously Tasty Omelet"] = "美味煎蛋卷",
	["Danonzo's Tel'Abim Delight"] = "达农佐的泰拉比姆趣味",
	["Danonzo's Tel'Abim Medley"] = "达农佐的泰拉比姆情调",
	["Danonzo's Tel'Abim Surprise"] = "达农佐的泰拉比姆惊喜",
	["Dark Iron Belt Buckle"] = "黑铁带扣",
	["Dark Iron Bomb"] = "黑铁炸弹",
	["Dark Iron Boots"] = "黑铁长靴",
	["Dark Iron Bracers"] = "黑铁护腕",
	["Dark Iron Destroyer"] = "黑铁战斧",
	["Dark Iron Gauntlets"] = "黑铁护手",
	["Dark Iron Helm"] = "黑铁头盔",
	["Dark Iron Leggings"] = "黑铁护腿",
	["Dark Iron Mail"] = "黑铁锁甲",
	["Dark Iron Plate"] = "黑铁板甲",
	["Dark Iron Pulverizer"] = "黑铁粉碎者",
	["Dark Iron Reaver"] = "黑铁利剑",
	["Dark Iron Rifle"] = "黑铁步枪",
	["Dark Iron Shoulders"] = "黑铁护肩",
	["Dark Iron Signet Ring"] = "黑铁徽记之戒",
	["Dark Iron Sunderer"] = "黑铁斩碎者",
	["Dark Leather Belt"] = "黑皮腰带",
	["Dark Leather Boots"] = "黑皮战靴",
	["Dark Leather Cloak"] = "黑皮披风",
	["Dark Leather Gloves"] = "黑皮手套",
	["Dark Leather Pants"] = "黑皮短裤",
	["Dark Leather Shoulders"] = "黑皮护肩",
	["Dark Leather Tunic"] = "黑皮外套",
	["Dark Silk Shirt"] = "黑丝衬衣",
	["Darkrune Breastplate"] = "黑暗符文胸甲",
	["Darkrune Gauntlets"] = "黑暗符文护手",
	["Darkrune Helm"] = "黑暗符文头盔",
	["Darksoul Breastplate"] = "黑暗之魂胸甲",
	["Darksoul Leggings"] = "黑暗之魂护腿",
	["Darksoul Shoulders"] = "黑暗之魂护肩",
	["Darkspear"] = "黑暗之矛",
	["Dawn Treaders"] = "黎明皮靴",
	["Dawn's Edge"] = "黎明之刃",
	["Dawnbright Cuffs"] = "曙光护腕",
	["Dawnbringer Shoulders"] = "黎明使者护肩",
	["Dawnstone Hammer"] = "黎明石锤",
	["Dazzling Aquamarine Loop"] = "闪亮青绿石戒指",
	["Dazzling Mithril Rapier"] = "秘银细剑",
	["Dazzling Moonstone Band"] = "闪耀月亮石指环",
	["Deadly Blunderbuss"] = "致命的短枪",
	["Deadly Bronze Poniard"] = "致命的青铜短剑",
	["Deadly Poison"] = "致命毒药",
	["Deadly Poison II"] = "致命毒药 II",
	["Deadly Poison III"] = "致命毒药 III",
	["Deadly Poison IV"] = "致命毒药 IV",
	["Deadly Poison V"] = "致命毒药 V",
	["Deadly Scope"] = "致命瞄准镜",
	["Deep Sapphire Circlet"] = "蓝玉指环",
	["Deepdive Helmet"] = "潜水头盔",
	["Deepmist Choker"] = "深海项链",
	["Delicate Arcanite Converter"] = "精密奥金转换器",
	["Delicate Mithril Amulet"] = "精致秘银护符",
	["Demon Forged Breastplate"] = "魔铸胸甲",
	["Dense Blasting Powder"] = "致密炸药粉",
	["Dense Dynamite"] = "致密炸药",
	["Dense Gemstone Cluster"] = "致密的宝石串",
	["Dense Grinding Stone"] = "致密砂轮",
	["Dense Gritted Paper"] = "致密砂纸",
	["Dense Sharpening Stone"] = "致密磨刀石",
	["Dense Weightstone"] = "致密平衡石",
	["Depthstalker Helmet"] = "深渊猎手头盔",
	["Deviate Scale Belt"] = "蛇鳞腰带",
	["Deviate Scale Cloak"] = "蛇鳞披风",
	["Deviate Scale Gloves"] = "蛇鳞手套",
	["Devilsaur Gauntlets"] = "魔暴龙皮手套",
	["Devilsaur Leggings"] = "魔暴龙皮护腿",
	["Dig Rat Stew"] = "掘地鼠炖肉",
	["Dim Torch"] = "劣质火炬",
	["Dimensional Ripper - Everlook"] = "空间撕裂器 - 永望镇",
	["Dirge's Kickin' Chimaerok Chops"] = "迪尔格的超美味奇美拉肉片",
	["Discolored Healing Potion"] = "透明治疗药水",
	["Discombobulator Ray"] = "退化射线",
	["Dissolvent Poison"] = "溶解毒药",
	["Dissolvent Poison II"] = "溶解毒药 II",
	["Diviner's Boots"] = "占卜者鞋子",
	["Diviner's Cowl"] = "占卜者兜帽",
	["Diviner's Epaulets"] = "占卜者护肩",
	["Diviner's Mitts"] = "占卜者手套",
	["Diviner's Pantaloons"] = "占卜者裤子",
	["Diviner's Robes"] = "占卜者长袍",
	["Double-stitched Woolen Shoulders"] = "双线毛纺护肩",
	["Draenethyst Baton"] = "德莱尼水晶魔棒",
	["Dragonbreath Chili"] = "龙息红椒",
	["Dragonmaw Armor Kit"] = "龙喉护甲片",
	["Dragonmaw Gloves"] = "龙喉手套",
	["Dragonscale Belt Buckle"] = "龙鳞带扣",
	["Dragonscale Breastplate"] = "龙鳞胸甲",
	["Dragonscale Gauntlets"] = "龙鳞护手",
	["Dragonscale Leggings"] = "龙鳞护腿",
	["Dream's Herald"] = "梦境使者",
	["Dreamhide"] = "梦境皮革",
	["Dreamhide Belt"] = "梦境皮革腰带",
	["Dreamhide Bracers"] = "梦境皮革护腕",
	["Dreamhide Leggings"] = "梦境皮革护腿",
	["Dreamhide Mantle"] = "梦境皮革披肩",
	["Dreamless Sleep Potion"] = "昏睡药水",
	["Dreamscale Breastplate"] = "梦幻龙鳞胸甲",
	["Dreamshard Elixir"] = "梦境精华药剂",
	["Dreamsteel Belt Buckle"] = "梦钢带扣",
	["Dreamsteel Boots"] = "梦境钢铁长靴",
	["Dreamsteel Bracers"] = "梦境钢铁护腕",
	["Dreamsteel Leggings"] = "梦境钢铁护腿",
	["Dreamsteel Mantle"] = "梦境钢铁护肩",
	["Dreamthread"] = "梦境丝线",
	["Dreamthread Bracers"] = "梦境丝线护腕",
	["Dreamthread Gloves"] = "梦境丝线手套",
	["Dreamthread Kilt"] = "梦境丝线裙裤",
	["Dreamthread Mantle"] = "梦境丝线披肩",
	["Dreamweave Circlet"] = "梦纹头饰",
	["Dreamweave Gloves"] = "梦纹手套",
	["Dreamweave Vest"] = "梦纹外衣",
	["Dreary Opal Gemstone"] = "深沉白玉",
	["Driftwood Fishing Pole"] = "流木鱼竿",
	["Dry Pork Ribs"] = "猪肉干",
	["Dusky Belt"] = "暗色皮带",
	["Dusky Boots"] = "暗色长靴",
	["Dusky Bracers"] = "暗色护腕",
	["Dusky Leather Armor"] = "暗色皮甲",
	["Dusky Leather Leggings"] = "暗色皮护腿",
	["Dustguider Sash"] = "导尘者腰带",
	["EZ-Thro Dynamite"] = "简易投掷炸弹",
	["EZ-Thro Dynamite II"] = "简易投掷炸弹II",
	["Earthen Leather Shoulders"] = "土灵皮护肩",
	["Earthen Silk Belt"] = "土灵丝质腰带",
	["Earthen Vest"] = "土灵外衣",
	["Earthguard Tunic"] = "大地卫士外套",
	["Earthrock Loop"] = "岩石戒指",
	["Ebon Hand"] = "黑手",
	["Ebon Ring"] = "乌木戒指",
	["Ebon Shiv"] = "乌木刀",
	["Edge of Winter"] = "寒冬之刃",
	["Egg Nog"] = "蛋奶酒",
	["Elaborate Golden Bracelets"] = "精致的金手镯",
	["Elegant Emerald Gemstone"] = "优雅翡翠",
	["Elemental Sharpening Stone"] = "元素磨刀石",
	["Elixir of Agility"] = "敏捷药剂",
	["Elixir of Brute Force"] = "蛮力药剂",
	["Elixir of Defense"] = "防御药剂",
	["Elixir of Demonslaying"] = "屠魔药剂",
	["Elixir of Detect Demon"] = "侦测恶魔药剂",
	["Elixir of Detect Lesser Invisibility"] = "侦测次级隐形药剂",
	["Elixir of Detect Undead"] = "侦测亡灵药剂",
	["Elixir of Dream Vision"] = "梦境药剂",
	["Elixir of Firepower"] = "火力药剂",
	["Elixir of Fortitude"] = "坚韧药剂",
	["Elixir of Frost Power"] = "冰霜之力药剂",
	["Elixir of Giant Growth"] = "增长药剂",
	["Elixir of Giants"] = "巨人药剂",
	["Elixir of Greater Agility"] = "强效敏捷药剂",
	["Elixir of Greater Arcane Power"] = "强效奥术之力药剂",
	["Elixir of Greater Defense"] = "强效防御药剂",
	["Elixir of Greater Firepower"] = "强效火力药剂",
	["Elixir of Greater Frost Power"] = "强效冰霜之力药剂",
	["Elixir of Greater Intellect"] = "强效聪颖药剂",
	["Elixir of Greater Nature Power"] = "强效自然力量药剂",
	["Elixir of Greater Water Breathing"] = "强效水下呼吸药剂",
	["Elixir of Lesser Agility"] = "次级敏捷药剂",
	["Elixir of Lion's Strength"] = "狮王之力药剂",
	["Elixir of Minor Agility"] = "初级敏捷药剂",
	["Elixir of Minor Defense"] = "初级防御药剂",
	["Elixir of Minor Fortitude"] = "初级坚韧药剂",
	["Elixir of Ogre's Strength"] = "食人魔力量药剂",
	["Elixir of Poison Resistance"] = "抗毒药水",
	["Elixir of Rapid Growth"] = "急速生长药剂",
	["Elixir of Shadow Power"] = "暗影之力药剂",
	["Elixir of Superior Defense"] = "超强防御药剂",
	["Elixir of Water Breathing"] = "水下呼吸药剂",
	["Elixir of Wisdom"] = "智慧药剂",
	["Elixir of the Mongoose"] = "猫鼬药剂",
	["Elixir of the Sages"] = "先知药剂",
	["Embergem Cuffs"] = "灰烬宝石护腕",
	["Emberstone Idol"] = "灰烬石神像",
	["Emberstone Studded Ring"] = "灰烬石镶嵌指环",
	["Embossed Leather Boots"] = "雕花皮靴",
	["Embossed Leather Cloak"] = "雕花皮质披风",
	["Embossed Leather Gloves"] = "雕花皮手套",
	["Embossed Leather Pants"] = "雕花皮短裤",
	["Embossed Leather Vest"] = "雕花皮外衣",
	["Emerald Monarch's Glow"] = "翡翠帝王石",
	["Empowered Domination Rod"] = "支配之权杖",
	["Empowering Herbal Salad"] = "强能草药沙拉",
	["Enchanted Armor Kit"] = "魔化护甲片",
	["Enchanted Battlehammer"] = "魔化战锤",
	["Enchanted Bracelets"] = "魔化手镯",
	["Enchanted Emerald Gemstone"] = "魔化翡翠",
	["Enchanted Mageweave Pouch"] = "魔化魔纹布包",
	["Enchanted Runecloth Bag"] = "魔化符文布包",
	["Enchanted Thorium Belt Buckle"] = "魔化瑟银带扣",
	["Enchanted Thorium Breastplate"] = "魔化瑟银胸甲",
	["Enchanted Thorium Helm"] = "魔化瑟银头盔",
	["Enchanted Thorium Leggings"] = "魔化瑟银护腿",
	["Enchanter's Cowl"] = "巫术师兜帽",
	["Encrusted Bronze Staff"] = "硬化青铜法杖",
	["Encrusted Copper Bangle"] = "硬化铜手镯",
	["Encrusted Gemstone Ring"] = "镶嵌宝石之戒",
	["Essence Infused Leather Gloves"] = "灌注精华手套",
	["Ethereal Frostspark Crown"] = "缥缈的霜花皇冠",
	["Ethereal Helmet"] = "虚灵头饰",
	["Ethereal Leggings"] = "虚灵护腿",
	["Ethereal Shoulder Pads"] = "虚灵护肩",
	["Ethereal Tunic"] = "虚灵外套",
	["Explosive Sheep"] = "自爆绵羊",
	["Facetted Moonstone Brooch"] = "泛光月亮石胸针",
	["Fangclaw Relic"] = "牙爪圣物",
	["Farraki Ceremony Totem"] = "法拉基祭祀图腾",
	["Feathered Breastplate"] = "羽饰胸甲",
	["Felcloth Bag"] = "恶魔布包",
	["Felcloth Boots"] = "恶魔布靴",
	["Felcloth Gloves"] = "恶魔布手套",
	["Felcloth Hood"] = "恶魔布帽",
	["Felcloth Pants"] = "恶魔布短裤",
	["Felcloth Robe"] = "恶魔布袍",
	["Felcloth Shoulders"] = "恶魔布护肩",
	["Festive Red Dress"] = "红色节庆长裙",
	["Festive Red Pant Suit"] = "红色节庆裤装",
	["Field Repair Bot 74A"] = "修理机器人74A型",
	["Fiery Chain Breastplate"] = "火链胸甲",
	["Fiery Chain Girdle"] = "炽热链甲束带",
	["Fiery Chain Shoulders"] = "炽热链甲护肩",
	["Fiery Plate Gauntlets"] = "炽热板甲护手",
	["Filet of Redgill"] = "油炸红腮鱼",
	["Fillet of Frenzy"] = "狂鱼肉片",
	["Fine Leather Belt"] = "优质皮带",
	["Fine Leather Boots"] = "优质皮靴",
	["Fine Leather Cloak"] = "优质皮披风",
	["Fine Leather Gloves"] = "优质皮手套",
	["Fine Leather Pants"] = "优质皮裤",
	["Fine Leather Tunic"] = "优质皮外套",
	["Fire Goggles"] = "火焰护目镜",
	["Fire Oil"] = "火焰之油",
	["Fire Protection Potion"] = "火焰防护药水",
	["Firework Cluster Launcher"] = "烟花束发射器",
	["Firework Launcher"] = "烟花发射器",
	["Fishing Boat"] = "垂钓小舟",
	["Flame Deflector"] = "火焰偏斜器",
	["Flamewrath Leggings"] = "火怒护腿",
	["Flarecore Boots"] = "烈焰核心靴子",
	["Flarecore Gloves"] = "光芒手套",
	["Flarecore Leggings"] = "光芒护腿",
	["Flarecore Mantle"] = "光芒衬肩",
	["Flarecore Robe"] = "光芒长袍",
	["Flarecore Wraps"] = "光芒护腕",
	["Flash Bomb"] = "闪光雷",
	["Flask of Chromatic Resistance"] = "多重抗性合剂",
	["Flask of Distilled Wisdom"] = "精炼智慧合剂",
	["Flask of Petrification"] = "化石合剂",
	["Flask of Supreme Power"] = "超级能量合剂",
	["Flask of the Titans"] = "泰坦合剂",
	["Flawless Arcanite Rifle"] = "完美的奥金步枪",
	["Flawless Black Gemstone"] = "漆黑曜石",
	["Fletcher's Gloves"] = "造弓师手套",
	["Flying Tiger Goggles"] = "飞虎护目镜",
	["Force Reactive Disk"] = "力反馈盾牌",
	["Formal White Shirt"] = "体面的白衬衣",
	["Free Action Potion"] = "自由行动药剂",
	["Frost Leather Cloak"] = "冰霜皮质披风",
	["Frost Oil"] = "冰霜之油",
	["Frost Protection Potion"] = "冰霜防护药水",
	["Frost Tiger Blade"] = "霜虎之刃",
	["Frostbound Slasher"] = "冰霜缚斩者",
	["Frostguard"] = "寒冰护卫者",
	["Frostsaber Boots"] = "霜刃长靴",
	["Frostsaber Gloves"] = "霜刃手套",
	["Frostsaber Leggings"] = "霜刃护腿",
	["Frostsaber Tunic"] = "霜刃外套",
	["Frostweave Gloves"] = "霜纹手套",
	["Frostweave Pants"] = "霜纹短裤",
	["Frostweave Robe"] = "霜纹长袍",
	["Frostweave Tunic"] = "霜纹外套",
	["Fury of the Timbermaw"] = "木喉之怒",
	["Gaea's Embrace"] = "盖亚的拥抱",
	["Garnet Guardian Staff"] = "深红护卫法杖",
	["Gauntlets of the Sea"] = "深海护手",
	["Gem Encrusted Choker"] = "宝石项圈",
	["Gem-studded Leather Belt"] = "宝石皮带",
	["Gemkeeper's Folio"] = "宝石商的魔棒",
	["Gemmed Citrine Pendant"] = "黄玉坠饰",
	["Gemmed Copper Gauntlets"] = "铜质宝石手套",
	["Gemstone Compendium"] = "宝石纲要",
	["Ghost Dye"] = "幻象染料",
	["Ghostweave Belt"] = "鬼纹腰带",
	["Ghostweave Gloves"] = "鬼纹手套",
	["Ghostweave Pants"] = "鬼纹短裤",
	["Ghostweave Vest"] = "鬼纹外衣",
	["Giant Clam Scorcho"] = "烧烤巨蚌",
	["Gift of Arthas"] = "阿尔萨斯的礼物",
	["Giga-Charged Arcane Reflector"] = "究极充能奥术反射器",
	["Gilneas Hot Stew"] = "吉尔尼斯热炖菜",
	["Gingerbread Cookie"] = "姜饼",
	["Girdle of Insight"] = "洞察束带",
	["Girdle of the Dawn"] = "黎明束腰",
	["Glacial Cloak"] = "冰川披风",
	["Glacial Gloves"] = "冰川手套",
	["Glacial Vest"] = "冰川外衣",
	["Glacial Wrists"] = "冰川护腕",
	["Gleaming Chain"] = "闪光项链",
	["Gleaming Jade Gemstone"] = "圆润翡翠",
	["Gleaming Silver Necklace"] = "闪光的银项链",
	["Glinting Steel Dagger"] = "亮闪闪的钢匕首",
	["Glittering Sapphire Gemstone"] = "辉光蓝宝石",
	["Gloomweed Bindings"] = "暗草护腕",
	["Gloomy Diamond Gemstone"] = "幽影墨玉",
	["Gloves of Manathirst"] = "渴魔手套",
	["Gloves of Meditation"] = "冥想手套",
	["Gloves of Spell Mastery"] = "法术掌握手套",
	["Gloves of Unwinding Mystery"] = "解谜手套",
	["Gloves of the Dawn"] = "黎明手套",
	["Gloves of the Greatfather"] = "冬天爷爷的手套",
	["Glowing Ruby Gemstone"] = "炽热红宝石",
	["Glyph Codex"] = "铭文圣典",
	["Gnomish Battle Chicken"] = "侏儒作战小鸡",
	["Gnomish Cloaking Device"] = "侏儒隐形装置",
	["Gnomish Death Ray"] = "侏儒死亡射线",
	["Gnomish Goggles"] = "侏儒护目镜",
	["Gnomish Harm Prevention Belt"] = "侏儒防护腰带",
	["Gnomish Mind Control Cap"] = "侏儒洗脑帽",
	["Gnomish Net-o-Matic Projector"] = "侏儒撒网器",
	["Gnomish Rocket Boots"] = "侏儒火箭靴",
	["Gnomish Shrink Ray"] = "侏儒缩小射线",
	["Gnomish Universal Remote"] = "侏儒通用遥控器",
	["Goblin Bomb Dispenser"] = "地精炸弹箱",
	["Goblin Construction Helmet"] = "地精施工头盔",
	["Goblin Deviled Clams"] = "地精芥末蘸蚌肉",
	["Goblin Dragon Gun"] = "地精龙枪",
	["Goblin Jumper Cables"] = "地精起搏器",
	["Goblin Jumper Cables XL"] = "地精起搏器XL型",
	["Goblin Land Mine"] = "地精暗雷",
	["Goblin Mining Helmet"] = "地精采矿头盔",
	["Goblin Mortar"] = "地精迫击炮",
	["Goblin Radio"] = "地精无线电",
	["Goblin Rocket Boots"] = "地精火箭靴",
	["Goblin Rocket Fuel"] = "地精火箭燃油",
	["Goblin Rocket Fuel Recipe"] = "地精火箭燃油配方",
	["Goblin Rocket Helmet"] = "地精火箭头盔",
	["Goblin Sapper Charge"] = "地精工兵炸药",
	["Gold Belt Buckle"] = "设计图：金带扣",
	["Gold Power Core"] = "黄金能量核心",
	["Goldcrest Amulet"] = "金色徽记护符",
	["Golden Iron Destroyer"] = "碎铁金锤",
	["Golden Jade Ring"] = "金玉戒指",
	["Golden Mantle of the Dawn"] = "金色黎明衬肩",
	["Golden Rod"] = "金棒",
	["Golden Runed Ring"] = "黄金符文戒指",
	["Golden Scale Boots"] = "金鳞战靴",
	["Golden Scale Bracers"] = "金鳞护腕",
	["Golden Scale Coif"] = "金鳞罩盔",
	["Golden Scale Cuirass"] = "金鳞胸甲",
	["Golden Scale Gauntlets"] = "金鳞护手",
	["Golden Scale Leggings"] = "金鳞护腿",
	["Golden Scale Shoulders"] = "金鳞护肩",
	["Golden Scepter of Authority"] = "黄金权杖",
	["Golden Skeleton Key"] = "黄金万能钥匙",
	["Goldenshade Quartz Crown"] = "黄金石英头冠",
	["Goldfire Crystal Bracelet"] = "金焰水晶手镯",
	["Goldthorn Tea"] = "金棘茶",
	["Gooey Spider Cake"] = "蜘蛛蛋糕",
	["Gordok Ogre Suit"] = "戈多克食人魔装",
	["Goretusk Liver Pie"] = "猪肝馅饼",
	["Gorgeous Mountain Gemstone"] = "华丽山地宝石",
	["Graceful Agate Gemstone"] = "璀璨玛瑙",
	["Grail of Forgotten Memories"] = "忘却的圣杯",
	["Grandstaff of the Shen'dralar Elder"] = "辛德拉长老的高贵法杖",
	["Gray Woolen Robe"] = "灰色毛纺长袍",
	["Gray Woolen Shirt"] = "灰色毛纺衬衣",
	["Great Rage Potion"] = "暴怒药水",
	["Greater Adept's Robe"] = "大师长袍",
	["Greater Arcane Elixir"] = "强效奥法药剂",
	["Greater Arcane Protection Potion"] = "强效奥术防护药水",
	["Greater Binding Signet"] = "强效禁锢徽记",
	["Greater Dreamless Sleep Potion"] = "强效昏睡药水",
	["Greater Fire Protection Potion"] = "强效火焰防护药水",
	["Greater Frost Protection Potion"] = "强效冰霜防护药水",
	["Greater Healing Potion"] = "强效治疗药水",
	["Greater Holy Protection Potion"] = "强效神圣防护药水",
	["Greater Mana Potion"] = "强效法力药水",
	["Greater Nature Protection Potion"] = "强效自然防护药水",
	["Greater Shadow Protection Potion"] = "强效暗影防护药水",
	["Greater Stoneshield Potion"] = "强效石盾药水",
	["Green Dragonscale Breastplate"] = "绿龙鳞片胸甲",
	["Green Dragonscale Gauntlets"] = "绿色龙鳞护手",
	["Green Dragonscale Leggings"] = "绿龙鳞片护腿",
	["Green Firework"] = "绿色焰火",
	["Green Holiday Shirt"] = "绿色节日衬衣",
	["Green Iron Boots"] = "绿铁战靴",
	["Green Iron Bracers"] = "绿铁护腕",
	["Green Iron Gauntlets"] = "绿铁护手",
	["Green Iron Hauberk"] = "绿铁锁甲",
	["Green Iron Helm"] = "绿铁头盔",
	["Green Iron Leggings"] = "绿铁护腿",
	["Green Iron Shoulders"] = "绿铁护肩",
	["Green Leather Armor"] = "绿色皮甲",
	["Green Leather Belt"] = "绿色皮带",
	["Green Leather Bracers"] = "绿色皮护腕",
	["Green Lens"] = "绿色透镜",
	["Green Linen Bracers"] = "绿色亚麻护腕",
	["Green Linen Shirt"] = "绿色亚麻衬衣",
	["Green Rocket Cluster"] = "绿色烟花束",
	["Green Silk Armor"] = "绿色丝甲",
	["Green Silk Pack"] = "绿色丝质包",
	["Green Silken Shoulders"] = "绿色丝质护肩",
	["Green Tinted Goggles"] = "绿色护目镜",
	["Green Whelp Armor"] = "绿色幼龙护甲",
	["Green Whelp Bracers"] = "绿色幼龙护腕",
	["Green Woolen Bag"] = "绿色毛纺包",
	["Green Woolen Vest"] = "绿色毛纺外衣",
	["Grifter's Belt"] = "骗子腰带",
	["Grifter's Boots"] = "骗子靴子",
	["Grifter's Cover"] = "骗子兜帽",
	["Grifter's Gauntlets"] = "骗子手套",
	["Grifter's Leggings"] = "骗子护腿",
	["Grifter's Tunic"] = "骗子外衣",
	["Grilled Squid"] = "烤鱿鱼",
	["Guardbreaker Charm"] = "破碎守护者的护符",
	["Guardian Armor"] = "守护之甲",
	["Guardian Belt"] = "守护腰带",
	["Guardian Cloak"] = "守护披风",
	["Guardian Gloves"] = "守护手套",
	["Guardian Leather Bracers"] = "守护腕甲",
	["Guardian Pants"] = "守护短裤",
	["Gurubashi Gumbo"] = "古拉巴什浓汤",
	["Gurubashi Mojo Madness"] = "古拉巴什魔精",
	["Gyrochronatom"] = "发条式同步协调陀螺仪",
	["Gyrofreeze Ice Reflector"] = "寒冰偏斜器",
	["Gyromatic Micro-Adjustor"] = "侏儒微调器",
	["Hammer of the Titans"] = "泰坦之锤",
	["Handful of Copper Bolts"] = "一把螺栓",
	["Hands of Darkness"] = "黑暗之手",
	["Handstitched Leather Belt"] = "手工皮带",
	["Handstitched Leather Boots"] = "手工皮靴",
	["Handstitched Leather Bracers"] = "手工皮护腕",
	["Handstitched Leather Cloak"] = "手工皮披风",
	["Handstitched Leather Pants"] = "手工皮短裤",
	["Handstitched Leather Vest"] = "手工皮外衣",
	["Handstitched Linen Britches"] = "手工亚麻裤",
	["Hardened Iron Shortsword"] = "硬铁短剑",
	["Harness of the High Thane"] = "崇高领主甲胄",
	["Harpy Talon Ring"] = "翼爪戒指",
	["Hateforge Belt"] = "仇恨熔炉腰带",
	["Hateforge Boots"] = "仇恨熔炉靴子",
	["Hateforge Cuirass"] = "仇恨熔炉胸甲",
	["Hateforge Grips"] = "仇恨熔炉护手",
	["Hateforge Helmet"] = "仇恨熔炉头盔",
	["Hateforge Leggings"] = "仇恨熔炉护腿",
	["Healing Potion"] = "治疗药水",
	["Heart of the Sea"] = "海洋之心",
	["Heartseeker"] = "寻心者",
	["Heavy Armor Kit"] = "重型护甲片",
	["Heavy Blasting Powder"] = "烈性火药",
	["Heavy Bronze Mace"] = "青铜重锤",
	["Heavy Copper Broadsword"] = "铜质重剑",
	["Heavy Copper Maul"] = "铜质大锤",
	["Heavy Crocolisk Stew"] = "鳄鱼炖肉",
	["Heavy Dynamite"] = "烈性炸药",
	["Heavy Earthen Gloves"] = "重型土灵手套",
	["Heavy Gemstone Cluster"] = "厚重的宝石串",
	["Heavy Grinding Stone"] = "重砂轮",
	["Heavy Gritted Paper"] = "厚重砂纸",
	["Heavy Kodo Stew"] = "科多肉杂烩",
	["Heavy Leather"] = "重皮",
	["Heavy Leather Ammo Pouch"] = "重皮弹药包",
	["Heavy Leather Ball"] = "重皮球",
	["Heavy Linen Bandage"] = "厚亚麻绷带",
	["Heavy Linen Gloves"] = "高级亚麻手套",
	["Heavy Mageweave Bandage"] = "厚魔纹绷带",
	["Heavy Mithril Axe"] = "秘银重斧",
	["Heavy Mithril Boots"] = "秘银重靴",
	["Heavy Mithril Breastplate"] = "重型秘银胸甲",
	["Heavy Mithril Gauntlet"] = "重型秘银手套",
	["Heavy Mithril Helm"] = "重型秘银头盔",
	["Heavy Mithril Pants"] = "秘银短裤",
	["Heavy Mithril Shoulder"] = "重型秘银护肩",
	["Heavy Obsidian Belt"] = "重型黑曜石腰带",
	["Heavy Quiver"] = "重型箭袋",
	["Heavy Runecloth Bandage"] = "厚符文布绷带",
	["Heavy Scorpid Belt"] = "重型蝎壳腰带",
	["Heavy Scorpid Bracers"] = "重型蝎壳护腕",
	["Heavy Scorpid Gauntlets"] = "重型蝎壳护手",
	["Heavy Scorpid Helm"] = "重型蝎壳头盔",
	["Heavy Scorpid Leggings"] = "重型蝎壳护腿",
	["Heavy Scorpid Shoulders"] = "重型蝎壳护肩",
	["Heavy Scorpid Vest"] = "重型蝎壳外衣",
	["Heavy Sharpening Stone"] = "重磨刀石",
	["Heavy Silk Bandage"] = "厚丝质绷带",
	["Heavy Timbermaw Belt"] = "重型木喉腰带",
	["Heavy Timbermaw Boots"] = "重型木喉长靴",
	["Heavy Weightstone"] = "重平衡石",
	["Heavy Wool Bandage"] = "厚绒线绷带",
	["Heavy Woolen Cloak"] = "高级毛纺披风",
	["Heavy Woolen Gloves"] = "高级毛纺手套",
	["Heavy Woolen Pants"] = "高级毛纺短裤",
	["Helm of Fire"] = "火焰头盔",
	["Helm of the Great Chief"] = "大酋长头盔",
	["Herb Baked Egg"] = "草药烘蛋",
	["Herbalist's Gloves"] = "采药人手套",
	["Hi-Explosive Bomb"] = "高爆炸弹",
	["Hi-Impact Mithril Slugs"] = "高速秘银弹头",
	["Hide of the Wild"] = "野性之皮",
	["Hillman's Belt"] = "山地腰带",
	["Hillman's Cloak"] = "山地披风",
	["Hillman's Leather Gloves"] = "山地皮手套",
	["Hillman's Leather Vest"] = "山地皮外衣",
	["Hillman's Shoulders"] = "山地护肩",
	["Holy Protection Potion"] = "神圣防护药水",
	["Hot Lion Chops"] = "烤狮排",
	["Hot Smoked Bass"] = "烟熏鲈鱼",
	["Hot Wolf Ribs"] = "热狼排",
	["Huge Thorium Battleaxe"] = "巨型瑟银战斧",
	["Hydrathorn Bracers"] = "刺花护腕",
	["Hyper-Radiant Flame Reflector"] = "高辐射烈焰反射器",
	["Hypertech Battery Pack"] = "超能电池组",
	["Ice Deflector"] = "寒冰偏斜器",
	["Icebane Bracers"] = "破冰护腕",
	["Icebane Breastplate"] = "破冰胸甲",
	["Icebane Gauntlets"] = "破冰护手",
	["Icy Cloak"] = "冰覆披风",
	["Icy Scale Bracers"] = "寒鳞护腕",
	["Icy Scale Breastplate"] = "寒鳞胸甲",
	["Icy Scale Gauntlets"] = "寒鳞护手",
	["Illuminated Gemstone"] = "幻光宝玉",
	["Imperial Plate Belt"] = "君王板甲腰带",
	["Imperial Plate Boots"] = "君王板甲战靴",
	["Imperial Plate Bracers"] = "君王板甲护腕",
	["Imperial Plate Chest"] = "君王板甲护胸",
	["Imperial Plate Gauntlets"] = "帝国板甲护手",
	["Imperial Plate Helm"] = "君王板甲头盔",
	["Imperial Plate Leggings"] = "君王板甲护腿",
	["Imperial Plate Shoulders"] = "君王板甲护肩",
	["Inferno Gloves"] = "地狱火手套",
	["Inlaid Copper Ring"] = "镶饰的铜戒指",
	["Inlaid Mithril Cylinder"] = "秘银杆",
	["Inlaid Mithril Cylinder Plans"] = "秘银杆设计图",
	["Inlaid Thorium Hammer"] = "镶饰瑟银战锤",
	["Inscribed Runic Bracers"] = "魔符雕刻护腕",
	["Instant Poison"] = "速效毒药",
	["Instant Poison II"] = "速效毒药 II",
	["Instant Poison III"] = "速效毒药 III",
	["Instant Poison IV"] = "速效毒药 IV",
	["Instant Poison V"] = "速效毒药 V",
	["Instant Poison VI"] = "速效毒药 VI",
	["Intricate Gyroscope Goggles"] = "精密陀螺仪护目镜",
	["Invisibility Potion"] = "隐形药水",
	["Invulnerable Mail"] = "免伤锁甲",
	["Iridescent Hammer"] = "彩虹之锤",
	["Iron Belt Buckle"] = "铁带扣",
	["Iron Counterweight"] = "铁质平衡锤",
	["Iron Grenade"] = "铁皮手雷",
	["Iron Lantern"] = "铁质灯笼",
	["Iron Shield Spike"] = "铁质盾刺",
	["Iron Strut"] = "铁棒",
	["Ironbloom Ring"] = "铁花戒指",
	["Ironfeather Breastplate"] = "铁羽胸甲",
	["Ironfeather Shoulders"] = "铁羽护肩",
	["Ironforge Breastplate"] = "铁炉堡胸甲",
	["Ironforge Chain"] = "铁炉堡链甲",
	["Ironforge Gauntlets"] = "铁炉堡护手",
	["Ironsun Citrine Ring"] = "烈日黄玉戒指",
	["Ironvine Belt"] = "铁藤腰带",
	["Ironvine Breastplate"] = "铁藤胸甲",
	["Ironvine Gloves"] = "铁藤手套",
	["Jade Harmony Circlet"] = "翡翠和谐指环",
	["Jade Serpentblade"] = "玉蛇刀",
	["Jagged Obsidian Shield"] = "碎裂黑曜石盾牌",
	["Jewelry Lens"] = "宝石透镜",
	["Jewelry Scope"] = "宝石瞄准镜",
	["Jungle Stew"] = "丛林大杂烩",
	["Kaldorei Spider Kabob"] = "卡多雷蜘蛛烤肉",
	["Kodo Hide Bag"] = "科多兽皮包",
	["Large Blue Rocket"] = "大型蓝色烟花",
	["Large Blue Rocket Cluster"] = "大型蓝色烟花束",
	["Large Copper Bomb"] = "大型铜壳炸弹",
	["Large Green Rocket"] = "大型绿色烟花",
	["Large Green Rocket Cluster"] = "大型绿色烟花束",
	["Large Red Rocket"] = "大型红色烟花",
	["Large Red Rocket Cluster"] = "大型红色烟花束",
	["Large Seaforium Charge"] = "大型爆盐炸弹",
	["Lava Belt"] = "熔岩腰带",
	["Lavender Mageweave Shirt"] = "紫色魔纹衬衣",
	["Lavish Gemmed Necklace"] = "奢华宝石项链",
	["Le Fishe Au Chocolat"] = "巧克力鱼",
	["Lean Venison"] = "瘦鹿肉",
	["Lean Wolf Steak"] = "瘦狼排",
	["Lesser Fortification Ring"] = "小型强固戒指",
	["Lesser Healing Potion"] = "次级治疗药水",
	["Lesser Invisibility Potion"] = "次级隐形药水",
	["Lesser Mana Potion"] = "次级法力药水",
	["Lesser Stoneshield Potion"] = "次级石盾药水",
	["Lesser Wizard's Robe"] = "次级巫师袍",
	["Ley-Kissed Drape"] = "魔网亲和披风",
	["Lifelike Mechanical Toad"] = "仿真机械蛙",
	["Light Armor Kit"] = "轻型护甲片",
	["Light Leather"] = "轻皮",
	["Light Leather Bracers"] = "轻皮护腕",
	["Light Leather Pants"] = "轻皮短裤",
	["Light Leather Quiver"] = "轻皮箭袋",
	["Light Obsidian Belt"] = "轻型黑曜石腰带",
	["Lil' Smoky"] = "发条娃娃",
	["Limited Invulnerability Potion"] = "有限无敌药水",
	["Linen Bag"] = "亚麻包",
	["Linen Bandage"] = "亚麻绷带",
	["Linen Belt"] = "亚麻腰带",
	["Linen Boots"] = "亚麻靴",
	["Linen Cloak"] = "亚麻披风",
	["Lionheart Helm"] = "狮心头盔",
	["Living Action Potion"] = "活力行动药水",
	["Living Breastplate"] = "生命胸甲",
	["Living Leggings"] = "生命护腿",
	["Living Shoulders"] = "生命护肩",
	["Lobster Stew"] = "炖龙虾",
	["Loch Frenzy Delight"] = "洛克湖狂鱼",
	["Long Silken Cloak"] = "丝质长披风",
	["Longjaw Mud Snapper"] = "长嘴泥鳅",
	["Lordaeron Breastplate"] = "洛丹伦胸甲",
	["Lovingly Crafted Boomstick"] = "精致手工火枪",
	["Lucidity Potion"] = "清醒药水",
	["Lynxstep Boots"] = "山猫步靴",
	["Mageblood Potion"] = "魔血药水",
	["Mageweave Bag"] = "魔纹包",
	["Mageweave Bandage"] = "魔纹绷带",
	["Magic Resistance Potion"] = "抗魔药水",
	["Major Healing Potion"] = "特效治疗药水",
	["Major Mana Potion"] = "特效法力药水",
	["Major Recombobulator"] = "强力净化器",
	["Major Rejuvenation Potion"] = "特效活力药水",
	["Major Troll's Blood Potion"] = "特效巨魔之血药水",
	["Malachite Ring"] = "孔雀石戒指",
	["Mana Binding Signet"] = "魔法禁锢徽记",
	["Mana Potion"] = "法力药水",
	["Mantle of Centaur Authority"] = "半人马权威护肩",
	["Mantle of the Timbermaw"] = "木喉衬肩",
	["Marine Root"] = "海洋之根",
	["Marine's Demise"] = "水手的陨落",
	["Maritime Gumbo"] = "海员香辣炖菜",
	["Massive Iron Axe"] = "巨型铁斧",
	["Massive Jewel Circlet"] = "巨型宝石指环",
	["Master Engineer's Goggles"] = "高级技师护目镜",
	["Mastercrafted Diamond Bangles"] = "精制钻石手镯",
	["Mastercrafted Diamond Crown"] = "精致钻石宝冠",
	["Masterwork Stormhammer"] = "精工风暴战锤",
	["Masterwork Target Dummy"] = "高级活动假人",
	["Mechanical Dragonling"] = "机械幼龙",
	["Mechanical Repair Kit"] = "机械修理包",
	["Mechanical Squirrel"] = "机械松鼠",
	["Medallion of Flame"] = "火焰勋章",
	["Medium Armor Kit"] = "中型护甲片",
	["Medium Leather"] = "中皮",
	["Might of the Timbermaw"] = "木喉之力",
	["Mightfish Steak"] = "大鱼片",
	["Mighty Iron Hammer"] = "巨型铁锤",
	["Mighty Rage Potion"] = "强效怒气药水",
	["Mighty Troll's Blood Potion"] = "超强巨魔之血药水",
	["Mind-numbing Poison"] = "麻痹毒药",
	["Mind-numbing Poison II"] = "麻痹毒药 II",
	["Mind-numbing Poison III"] = "麻痹毒药 III",
	["Minor Healing Potion"] = "初级治疗药水",
	["Minor Magic Resistance Potion"] = "初级抗魔药水",
	["Minor Mana Potion"] = "初级法力药水",
	["Minor Recombobulator"] = "自动净化装置",
	["Minor Rejuvenation Potion"] = "初级活力药水",
	["Minor Trollblood Ring"] = "小型巨魔血戒",
	["Mistwood Tiara"] = "雾林头冠",
	["Mithril Belt Buckle"] = "秘银带扣",
	["Mithril Blackstone Necklace"] = "秘银黑石项链",
	["Mithril Blunderbuss"] = "秘银火枪",
	["Mithril Casing"] = "秘银外壳",
	["Mithril Coif"] = "秘银罩帽",
	["Mithril Frag Bomb"] = "秘银破片炸弹",
	["Mithril Gyro-Shot"] = "秘银螺旋弹",
	["Mithril Headed Trout"] = "银头鲑鱼",
	["Mithril Heavy-bore Rifle"] = "大口径秘银步枪",
	["Mithril Mechanical Dragonling"] = "秘银机械幼龙",
	["Mithril Scale Bracers"] = "秘银鳞片护腕",
	["Mithril Scale Gloves"] = "秘银鳞片手套",
	["Mithril Scale Pants"] = "秘银鳞片短裤",
	["Mithril Scale Shoulders"] = "秘银鳞片护肩",
	["Mithril Shield Spike"] = "秘银盾刺",
	["Mithril Spurs"] = "秘银马刺",
	["Mithril Tube"] = "秘银管",
	["Molten Belt"] = "熔火腰带",
	["Molten Helm"] = "熔铸头盔",
	["Molten Leggings"] = "熔岩护腿",
	["Monastery Emberbrace"] = "修道院灰烬护腕",
	["Mongoose Boots"] = "猫鼬长靴",
	["Monster Omelet"] = "超级煎蛋卷",
	["Mooncloth"] = "月布",
	["Mooncloth Bag"] = "月布包",
	["Mooncloth Boots"] = "月布长靴",
	["Mooncloth Circlet"] = "月布头饰",
	["Mooncloth Gloves"] = "月布手套",
	["Mooncloth Leggings"] = "月布护腿",
	["Mooncloth Robe"] = "月布长袍",
	["Mooncloth Shoulders"] = "月布护肩",
	["Mooncloth Vest"] = "月布外衣",
	["Moonglow Vest"] = "月光外衣",
	["Moonlight Staff"] = "月光法杖",
	["Moonlit Charm"] = "月光咒符",
	["Moonsight Rifle"] = "夜视步枪",
	["Moonsteel Broadsword"] = "月钢宽剑",
	["Murloc Fin Soup"] = "鱼人鳍汤",
	["Murloc Scale Belt"] = "鱼人鳞片腰带",
	["Murloc Scale Bracers"] = "鱼人鳞片护腕",
	["Murloc Scale Breastplate"] = "鱼人鳞片胸甲",
	["Murloc's Flippers"] = "鱼人脚蹼",
	["Mystery Stew"] = "神秘杂烩",
	["Nature Protection Potion"] = "自然防护药水",
	["Netherbane Rod"] = "地狱灾祸手杖",
	["Nightfall"] = "夜幕",
	["Nightfin Soup"] = "夜鳞鱼汤",
	["Nightscape Boots"] = "夜色长靴",
	["Nightscape Cloak"] = "夜色披风",
	["Nightscape Headband"] = "夜色头带",
	["Nightscape Pants"] = "夜色短裤",
	["Nightscape Shoulders"] = "夜色护肩",
	["Nightscape Tunic"] = "夜色外套",
	["Nimble Leather Gloves"] = "轻巧的皮手套",
	["Obsidian Belt Buckle"] = "黑曜石带扣",
	["Obsidian Brooch"] = "黑曜石胸针",
	["Obsidian Mail Tunic"] = "黑曜石锁甲",
	["Ocean's Gaze"] = "深海的凝视",
	["Ocean's Wrath"] = "海洋之怒",
	["Ogre Bone Band"] = "食人魔白骨指环",
	["Oil of Immolation"] = "献祭之油",
	["Onyxia Scale Cloak"] = "奥妮克希亚鳞片披风",
	["Opal Guided Bangles"] = "白玉手镯",
	["Opaline Illuminator"] = "柔光照明法杖",
	["Opalstone Circle"] = "白玉指环",
	["Orange Mageweave Shirt"] = "橙色魔纹衬衣",
	["Orange Martial Shirt"] = "橙色军用衬衣",
	["Orb of Clairvoyance"] = "预视之眼",
	["Orcish War Leggings"] = "兽人护腿",
	["Ornament of Restraint"] = "封印雕饰",
	["Ornate Bloodstone Dagger"] = "华丽血石匕首",
	["Ornate Mithril Boots"] = "精制秘银战靴",
	["Ornate Mithril Bracelets"] = "华丽秘银手镯",
	["Ornate Mithril Breastplate"] = "精制秘银胸甲",
	["Ornate Mithril Crown"] = "精致秘银头冠",
	["Ornate Mithril Gloves"] = "精制秘银手套",
	["Ornate Mithril Helm"] = "精制秘银头盔",
	["Ornate Mithril Pants"] = "精制秘银短裤",
	["Ornate Mithril Scepter"] = "华丽的秘银权杖",
	["Ornate Mithril Shoulders"] = "精制秘银护肩",
	["Ornate Spyglass"] = "精制望远镜",
	["Ornate Thorium Handaxe"] = "华丽瑟银手斧",
	["Otherworldly Breastplate"] = "超凡胸甲",
	["Otherworldly Coif"] = "超凡罩帽",
	["Otherworldly Leggings"] = "超凡护腿",
	["Otherworldly Spaulders"] = "超凡肩甲",
	["Parachute Cloak"] = "降落伞披风",
	["Patterned Bronze Bracers"] = "青铜花纹护腕",
	["Pauldron of Deflection"] = "偏斜护肩",
	["Pauldrons of the Timbermaw"] = "木喉肩铠",
	["Pearl-clasped Cloak"] = "珍珠披风",
	["Pearl-handled Dagger"] = "珍珠匕首",
	["Pendant of Arcane Radiance"] = "奥术光辉吊坠",
	["Pendant of Instability"] = "幻彩吊坠",
	["Pendant of Midnight"] = "午夜吊坠",
	["Permanently increase the intellect by 8 and spell power by 6 of an item worn on the chest, legs, hands or feet.  Only usable on items level 60 and above."] = "永久性地为一件胸部、腿部、手部或靴子装备附魔，使其获得额外的8点智力和5点法术伤害和治疗效果。无法与其它同位置的附魔共存。只能对60级或以上的物品使用。",
	["Persuader"] = "说服者",
	["Pet Bombling"] = "炸弹宠物",
	["Phantom Blade"] = "幻影之刃",
	["Philosophers' Stone"] = "点金石",
	["Phoenix Gloves"] = "凤凰手套",
	["Phoenix Pants"] = "凤凰短裤",
	["Pilferer's Gloves"] = "窃贼手套",
	["Pillager's Amice"] = "掠夺者披肩",
	["Pillager's Grips"] = "掠夺者手套",
	["Pillager's Hood"] = "掠夺者兜帽",
	["Pillager's Pantaloons"] = "掠夺者裤子",
	["Pillager's Robe"] = "掠夺者长袍",
	["Pillager's Shoes"] = "掠夺者鞋子",
	["Pink Mageweave Shirt"] = "粉色魔纹衬衣",
	["Poached Sunscale Salmon"] = "水煮阳鳞鲑鱼",
	["Polar Bracers"] = "北极护腕",
	["Polar Gloves"] = "北极手套",
	["Polar Tunic"] = "北极外套",
	["Polished Steel Boots"] = "精钢战靴",
	["Portable Bronze Mortar"] = "便携式青铜迫击炮",
	["Portable Wormhole Generator - Orgrimmar"] = "便携式虫洞发生器 - 奥格瑞玛",
	["Portable Wormhole Generator - Stormwind"] = "便携式虫洞发生器 - 暴风城",
	["Potion of Quickness"] = "加速药水",
	["Powerful Anti-Venom"] = "特效抗毒药剂",
	["Powerful Citrine Pendant"] = "强能黄水晶吊坠",
	["Powerful Seaforium Charge"] = "强力爆盐炸弹",
	["Powerful Smelling Salts"] = "强力嗅盐",
	["Practice Lock"] = "练习锁",
	["Precision Jewelry Kit"] = "精密珠宝工具包",
	["Primal Batskin Bracers"] = "原始蝙蝠皮护腕",
	["Primal Batskin Gloves"] = "原始蝙蝠皮手套",
	["Primal Batskin Jerkin"] = "原始蝙蝠皮外套",
	["Primalist's Boots"] = "首领长靴",
	["Primalist's Gloves"] = "首领手套",
	["Primalist's Headdress"] = "首领头饰",
	["Primalist's Pants"] = "首领短裤",
	["Primalist's Shoulders"] = "首领护肩",
	["Primalist's Vest"] = "首领马甲",
	["Prism Amulet"] = "棱光护符",
	["Prismatic Scale Barbute"] = "棱光鳞甲头盔",
	["Pristine Crystal Gemstone"] = "原始水晶石",
	["Pure Gold Ring"] = "纯金戒指",
	["Pure Shining Moonstone"] = "无暇月亮石",
	["Purification Potion"] = "净化药水",
	["Purifies Essence of Undeath turning it into Water."] = "将死灵精华转化为水之精华。",
	["Quartz Halo"] = "石英指环",
	["Quickdraw Quiver"] = "快捷箭袋",
	["Quicksilver Whirl"] = "水银环",
	["Rare Spawns"] = "稀有精英",
	["Radiant Belt"] = "辐光腰带",
	["Radiant Boots"] = "辐光长靴",
	["Radiant Breastplate"] = "辐光胸甲",
	["Radiant Circlet"] = "辐光头饰",
	["Radiant Ember Gemstone"] = "明亮灰烬石",
	["Radiant Gloves"] = "辐光手套",
	["Radiant Leggings"] = "辐光护腿",
	["Radiant Thorium Twilight"] = "明亮瑟银微光",
	["Rage Potion"] = "怒气药水",
	["Rainbow Fin Albacore"] = "彩鳍鱼",
	["Raptor Hide Belt"] = "迅猛龙皮腰带",
	["Raptor Hide Harness"] = "迅猛龙皮背心",
	["Red Dragonscale Boots"] = "红龙鳞片长靴",
	["Red Dragonscale Breastplate"] = "红龙鳞片胸甲",
	["Red Dragonscale Leggings"] = "红龙鳞片护腿",
	["Red Dragonscale Shoulders"] = "红龙鳞片护肩",
	["Red Firework"] = "红色焰火",
	["Red Linen Bag"] = "红色亚麻包",
	["Red Linen Robe"] = "红色亚麻长袍",
	["Red Linen Shirt"] = "红色亚麻衬衣",
	["Red Linen Vest"] = "红色亚麻外衣",
	["Red Mageweave Bag"] = "红色魔纹包",
	["Red Mageweave Gloves"] = "红色魔纹手套",
	["Red Mageweave Headband"] = "红色魔纹头带",
	["Red Mageweave Pants"] = "红色魔纹短裤",
	["Red Mageweave Shoulders"] = "红色魔纹护肩",
	["Red Mageweave Vest"] = "红色魔纹外衣",
	["Red Rocket Cluster"] = "红色烟花束",
	["Red Swashbuckler's Shirt"] = "红色冒险者衬衣",
	["Red Whelp Gloves"] = "红色雏龙手套",
	["Red Woolen Bag"] = "红色毛纺包",
	["Red Woolen Boots"] = "红色毛纺靴",
	["Redridge Goulash"] = "赤脊山炖肉",
	["Refined Dwarven Necklace"] = "精致的矮人项链",
	["Reflective Breastplate"] = "炫光胸甲",
	["Reflective Helmet"] = "炫光头盔",
	["Reflective Leggings"] = "炫光护腿",
	["Reflective Pauldrons"] = "炫光肩铠",
	["Regal Twilight Staff"] = "皇家曙光权杖",
	["Reinforced Linen Cape"] = "强化亚麻斗篷",
	["Reinforced Woolen Shoulders"] = "强化毛纺护肩",
	["Reloads an empty Goblin mortar."] = "重新装载地精迫击炮。",
	["Repaired Electro-Lantern"] = "修复的电灯",
	["Resilient Arcane Gemstone"] = "柔韧奥术石",
	["Restorative Potion"] = "滋补药剂",
	["Resurged Topaz Gemstone"] = "苏生黄玉",
	["Rich Purple Silk Shirt"] = "紫色丝质衬衣",
	["Ring of Midnight"] = "午夜之戒",
	["Ring of Purified Silver"] = "纯银戒指",
	["Ring of The Turtle"] = "乌龟戒指",
	["Ring of Unleashed Potential"] = "激发潜能之戒",
	["Roast Raptor"] = "烤迅猛龙肉",
	["Roasted Boar Meat"] = "烤野猪肉",
	["Roasted Kodo Meat"] = "烤科多肉",
	["Robe of Power"] = "力量法袍",
	["Robe of Sacrifice"] = "牺牲长袍",
	["Robe of Winter Night"] = "冬夜法袍",
	["Robe of the Archmage"] = "大法师之袍",
	["Robe of the Void"] = "虚空法袍",
	["Robes of Arcana"] = "奥法之袍",
	["Rockscale Cod"] = "石鳞鳕鱼",
	["Rose Colored Goggles"] = "玫瑰色护目镜",
	["Rough Blasting Powder"] = "劣质火药",
	["Rough Boomstick"] = "劣质火枪",
	["Rough Bronze Boots"] = "劣质青铜战靴",
	["Rough Bronze Bracers"] = "劣质青铜护腕",
	["Rough Bronze Cuirass"] = "劣质青铜胸甲",
	["Rough Bronze Leggings"] = "劣质青铜护腿",
	["Rough Bronze Ring"] = "粗糙青铜戒指",
	["Rough Bronze Shoulders"] = "劣质青铜护肩",
	["Rough Copper Bomb"] = "劣质铜壳炸弹",
	["Rough Copper Ring"] = "劣质的铜戒指",
	["Rough Copper Vest"] = "劣质铜外衣",
	["Rough Dynamite"] = "劣质炸药",
	["Rough Gemstone Cluster"] = "劣质的宝石串",
	["Rough Gold Ring"] = "粗糙的金戒指",
	["Rough Grinding Stone"] = "劣质砂轮",
	["Rough Gritted Paper"] = "劣质砂纸",
	["Rough Iron Ring"] = "粗糙的铁戒指",
	["Rough Mithril Ring"] = "劣质秘银戒指",
	["Rough Sharpening Stone"] = "劣质磨刀石",
	["Rough Silver Ring"] = "劣质瑟银戒指",
	["Rough Thorium Ring"] = "劣质瑟银戒指",
	["Rough Truesilver Ring"] = "劣质真银戒指",
	["Rough Weightstone"] = "劣质平衡石",
	["Royal Gemstone Staff"] = "皇家宝石法杖",
	["Ruby Ring of Ruin"] = "毁灭红玉指环",
	["Rudeus' Focusing Cane"] = "粗野的聚焦手杖",
	["Rugged Armor Kit"] = "毛皮护甲片",
	["Rugged Leather"] = "硬甲皮",
	["Rugged Leather Pants"] = "皱褶皮短裤",
	["Rune Edge"] = "符文之刃",
	["Rune-Etched Breastplate"] = "符文蚀刻胸甲",
	["Rune-Etched Crown"] = "符文蚀刻王冠",
	["Rune-Etched Greaves"] = "符文蚀刻胫甲",
	["Rune-Etched Grips"] = "符文蚀刻之握",
	["Rune-Etched Legplates"] = "符文蚀刻腿甲",
	["Rune-Etched Mantle"] = "符文蚀刻护肩",
	["Rune-Inscribed Plate Leggings"] = "铭文板甲护腿",
	["Runebound Amulet"] = "符文护符",
	["Runecloth Bag"] = "符文布背包",
	["Runecloth Bandage"] = "符文布绷带",
	["Runecloth Belt"] = "符文布腰带",
	["Runecloth Boots"] = "符文布靴",
	["Runecloth Cloak"] = "符文布披风",
	["Runecloth Gloves"] = "符文布手套",
	["Runecloth Headband"] = "符文布头带",
	["Runecloth Pants"] = "符文布短裤",
	["Runecloth Robe"] = "符文布袍",
	["Runecloth Shoulders"] = "符文布护肩",
	["Runecloth Tunic"] = "符文布外套",
	["Runed Copper Belt"] = "铜质符文腰带",
	["Runed Copper Bracers"] = "铜质符文护腕",
	["Runed Copper Breastplate"] = "铜质符文胸甲",
	["Runed Copper Gauntlets"] = "铜质符文护手",
	["Runed Copper Pants"] = "铜质符文短裤",
	["Runed Mithril Hammer"] = "秘银符文战锤",
	["Runed Stygian Belt"] = "符文冥河腰带",
	["Runed Stygian Boots"] = "符文冥河长靴",
	["Runed Stygian Leggings"] = "符文冥河护腿",
	["Runed Truesilver Ring"] = "符文真银戒指",
	["Runic Breastplate"] = "符文板甲",
	["Runic Leather Armor"] = "符文皮甲",
	["Runic Leather Belt"] = "符文皮甲腰带",
	["Runic Leather Bracers"] = "符文皮甲护腕",
	["Runic Leather Gauntlets"] = "符文皮甲护手",
	["Runic Leather Headband"] = "符文皮甲头环",
	["Runic Leather Pants"] = "符文皮甲短裤",
	["Runic Leather Shoulders"] = "符文皮甲护肩",
	["Runic Plate Boots"] = "符文板甲战靴",
	["Runic Plate Helm"] = "符文板甲头盔",
	["Runic Plate Leggings"] = "符文板甲护腿",
	["Runic Plate Shoulders"] = "符文板甲护肩",
	["Runn Tum Tuber Surprise"] = "洛恩塔姆薯块",
	["Sageblade"] = "先知之刃",
	["Sagefish Delight"] = "美味鼠尾鱼",
	["Salt Shaker"] = "筛盐器",
	["Sandstalker Bracers"] = "沙行者护腕",
	["Sandstalker Breastplate"] = "沙行者胸甲",
	["Sandstalker Gauntlets"] = "沙行者护手",
	["Sapphire Luminescence"] = "光辉蓝宝石戒指",
	["Satchel of Cenarius"] = "塞纳留斯之袋",
	["Savory Deviate Delight"] = "美味风蛇",
	["Scorpid Surprise"] = "蝎肉大餐",
	["Searing Golden Blade"] = "灼热金剑",
	["Seasoned Wolf Kabob"] = "干烤狼肉串",
	["Serenity"] = "平静",
	["Serpent's Coil Staff"] = "蛇盘之杖",
	["Sets up a fishing boat. While on your boat you gain +50 fishing skill. Can only be used outdoors near a deep enough body of water."] = "放置一艘垂钓小舟。在船上你钓鱼技能提高50点。只能在户外足够深的水域使用。",
	["Sets up a simple wooden planter. Planters allow you to plant seeds and cultivate crops."] = "设置一个简易的木制花盆。允许你种植植物。",
	["Sets up a traveler's tent. While under the cover of the tent you are resting and may gain rested experience. Can only be used outdoors."] = "放置一个旅行帐篷。在帐篷下你将处于休息状态并且可以获得休息经验。只能在户外使用。",
	["Shadow Crescent Axe"] = "月牙斧",
	["Shadow Goggles"] = "暗影护目镜",
	["Shadow Hood"] = "暗影头巾",
	["Shadow Oil"] = "暗影之油",
	["Shadow Protection Potion"] = "暗影防护药水",
	["Shadoweave Boots"] = "暗纹之靴",
	["Shadoweave Gloves"] = "暗纹手套",
	["Shadoweave Mask"] = "暗纹面罩",
	["Shadoweave Pants"] = "暗纹短裤",
	["Shadoweave Robe"] = "暗纹长袍",
	["Shadoweave Shoulders"] = "暗纹护肩",
	["Shadowfall Jewel"] = "暗影宝珠",
	["Shadowforged Eye"] = "影铸之眼",
	["Shadowgem Band"] = "暗影石戒指",
	["Shadowmoon Orb"] = "影月宝珠",
	["Shadowskin Boots"] = "暗影皮靴",
	["Shadowskin Gloves"] = "影皮手套",
	["Sharpened Citrine Gemstone"] = "尖锐黄水晶",
	["Sharpened Claw"] = "锋利之爪",
	["Shifting Cloak"] = "移形披风",
	["Shimmering Aqua Gemstone"] = "闪亮水玉",
	["Shimmering Bronze Ring"] = "闪亮青铜戒指",
	["Shimmering Diamond Band"] = "闪烁的钻戒",
	["Shimmering Gold Necklace"] = "闪光的金项链",
	["Shimmering Moonstone Tablet"] = "闪亮月亮石板",
	["Shining Copper Cuffs"] = "闪亮护腕",
	["Shining Sapphire Gemstone"] = "辉煌蓝宝石",
	["Shining Silver Breastplate"] = "银鳞胸甲",
	["Silk Bandage"] = "丝质绷带",
	["Silk Headband"] = "丝质头带",
	["Silver Contact"] = "银触媒",
	["Silver Medallion"] = "银质勋章",
	["Silver Rod"] = "银棒",
	["Silver Skeleton Key"] = "白银万能钥匙",
	["Silver-plated Shotgun"] = "镀银猎枪",
	["Silvered Bronze Boots"] = "镀银青铜战靴",
	["Silvered Bronze Breastplate"] = "镀银青铜胸甲",
	["Silvered Bronze Gauntlets"] = "镀银青铜护手",
	["Silvered Bronze Leggings"] = "镀银青铜护腿",
	["Silvered Bronze Shoulders"] = "镀银青铜护肩",
	["Simple Black Dress"] = "简易的黑裙子",
	["Simple Dress"] = "简易的裙子",
	["Simple Kilt"] = "简易的褶裙",
	["Simple Linen Boots"] = "简易的亚麻靴",
	["Simple Linen Pants"] = "简易亚麻短裤",
	["Simple Wooden Planter"] = "简易的木制花盆",
	["Skyfire Jewel"] = "天火宝石戒指",
	["Slitherskin Mackerel"] = "滑皮鲭鱼",
	["Small Blue Rocket"] = "小型蓝色烟花",
	["Small Bronze Bomb"] = "小型青铜炸弹",
	["Small Green Rocket"] = "小型绿色烟花",
	["Small Leather Ammo Pouch"] = "皮质小弹药包",
	["Small Pearl Ring"] = "小型珍珠戒指",
	["Small Pearlstone Staff"] = "小珍珠法杖",
	["Small Red Rocket"] = "小型红色烟花",
	["Small Seaforium Charge"] = "小型爆盐炸弹",
	["Small Silk Pack"] = "丝绸小包",
	["Smelt Bronze"] = "熔炼青铜",
	["Smelt Copper"] = "熔炼铜锭",
	["Smelt Dark Iron"] = "熔炼黑铁",
	["Smelt Dreamsteel"] = "熔炼梦境钢锭",
	["Smelt Elementium"] = "熔炼源质",
	["Smelt Gold"] = "熔炼金锭",
	["Smelt Iron"] = "熔炼铁锭",
	["Smelt Mithril"] = "熔炼秘银",
	["Smelt Silver"] = "熔炼银锭",
	["Smelt Steel"] = "熔炼钢锭",
	["Smelt Thorium"] = "熔炼瑟银",
	["Smelt Tin"] = "熔炼锡锭",
	["Smelt Truesilver"] = "熔炼真银",
	["Smoked Bear Meat"] = "熏熊肉",
	["Smoked Desert Dumplings"] = "沙漠肉丸子",
	["Smoked Sagefish"] = "烤鼠尾鱼",
	["Smoldering Brooch"] = "烟熏胸针",
	["Snake Burst Firework"] = "长蛇焰火",
	["Sniper Scope"] = "狙击瞄准镜",
	["SnowMaster 9000"] = "雪王9000型",
	["Soft-soled Linen Boots"] = "软底亚麻靴",
	["Softglow Ring"] = "柔光戒指",
	["Solid Blasting Powder"] = "实心炸药",
	["Solid Dynamite"] = "实心炸弹",
	["Solid Gemstone Cluster"] = "精致的宝石串",
	["Solid Grinding Stone"] = "坚固的砂轮",
	["Solid Gritted Paper"] = "坚固砂纸",
	["Solid Iron Maul"] = "结实的铁锤",
	["Solid Sharpening Stone"] = "坚固的磨刀石",
	["Solid Weightstone"] = "坚固的平衡石",
	["Soothing Turtle Bisque"] = "海龟汤",
	["Soul Pouch"] = "灵魂袋",
	["Specter's Shade Ring"] = "幽灵戒指",
	["Spellpower Goggles Xtreme"] = "法术能量护目镜超级版",
	["Spellpower Goggles Xtreme Plus"] = "法术能量护目镜超级改良版",
	["Spellweaver Pendant"] = "织法者坠饰",
	["Spellweaver Rod"] = "织法者之杖",
	["Sphinx's Wisdom Staff"] = "古代法老智慧之杖",
	["Spiced Chili Crab"] = "辣椒蟹肉",
	["Spiced Wolf Meat"] = "香辣狼肉",
	["Spider Belt"] = "蜘蛛腰带",
	["Spider Sausage"] = "蜘蛛肉肠",
	["Spider Silk Slippers"] = "蛛丝便鞋",
	["Spidersilk Boots"] = "蛛丝之靴",
	["Spire of Channeled Power"] = "魔导力量之杖",
	["Spitfire Bracers"] = "飞火护腕",
	["Spitfire Breastplate"] = "飞火胸甲",
	["Spitfire Gauntlets"] = "飞火护手",
	["Spotted Yellowtail"] = "斑点黄尾鱼",
	["Staff of Blossomed Jade"] = "绽放翡翠之杖",
	["Staff of Gallitrea"] = "加列拉之杖",
	["Standard Scope"] = "普通瞄准镜",
	["Star Belt"] = "星辰腰带",
	["Starforge Amulet"] = "星形护符",
	["Starry Thorium Band"] = "星形瑟银指环",
	["Steel Belt Buckle"] = "钢带扣",
	["Steel Breastplate"] = "钢质胸甲",
	["Steel Plate Armor"] = "钢铁板甲胸甲",
	["Steel Plate Barbute"] = "钢铁板甲头盔",
	["Steel Plate Boots"] = "钢铁板甲长靴",
	["Steel Plate Gauntlets"] = "钢铁板甲护手",
	["Steel Plate Helm"] = "钢质头盔",
	["Steel Plate Legguards"] = "钢铁板甲护腿",
	["Steel Plate Pauldrons"] = "钢铁板甲护肩",
	["Steel Weapon Chain"] = "钢质武器链",
	["Stellar Gemguards"] = "星型宝石护腕",
	["Stellar Ruby Ring"] = "星形红玉戒指",
	["Stonescale Oil"] = "石鳞鱼油",
	["Storm Gauntlets"] = "风暴护手",
	["Stormcloth Boots"] = "雷织长靴",
	["Stormcloth Gloves"] = "雷织手套",
	["Stormcloth Headband"] = "雷织头带",
	["Stormcloth Pants"] = "雷织短裤",
	["Stormcloth Shoulders"] = "雷织护肩",
	["Stormcloth Vest"] = "雷织外衣",
	["Stormcloud Shackles"] = "雷云护腕",
	["Stormcloud Sigil"] = "雷云魔印",
	["Stormcloud Signet"] = "雷云徽记",
	["Stormscale Leggings"] = "风暴鳞片护腿",
	["Stormshroud Armor"] = "雷暴",
	["Stormshroud Gloves"] = "雷暴手套",
	["Stormshroud Pants"] = "雷暴短裤",
	["Stormshroud Shoulders"] = "雷暴护肩",
	["Strider Stew"] = "炖陆行鸟",
	["Strong Anti-Venom"] = "强力抗毒药剂",
	["Strong Troll's Blood Potion"] = "强力巨魔之血药水",
	["Stronghold Gauntlets"] = "要塞护手",
	["Stunning Imperial Gemstone"] = "惊艳帝王石",
	["Sturdy Copper Ring"] = "坚硬的铜戒指",
	["Stylish Blue Shirt"] = "漂亮的蓝衬衣",
	["Stylish Green Shirt"] = "漂亮的绿衬衣",
	["Stylish Red Shirt"] = "漂亮的红衬衣",
	["Succulent Pork Ribs"] = "多汁猪排",
	["Sulfuron Hammer"] = "萨弗隆战锤",
	["Sunburst Tiara"] = "旭日皇冠",
	["Superior Healing Potion"] = "优质治疗药水",
	["Superior Mana Potion"] = "优质法力药水",
	["Survivalist's Skinning Knife"] = "生存剥皮刀",
	["Swift Boots"] = "迅捷之靴",
	["Swift Flight Bracers"] = "迅行护腕",
	["Swiftness Potion"] = "迅捷药水",
	["Swim Speed Potion"] = "速游药水",
	["Sylvan Crown"] = "林栖者头冠",
	["Sylvan Shoulders"] = "林栖者护肩",
	["Sylvan Vest"] = "林栖者外衣",
	["Taints Essence of Water turning it into Undeath."] = "将水之精华转化为死灵精华。",
	["Talisman of Hinderance"] = "守御护符",
	["Talisman of Stone"] = "石之护符",
	["Target Dummy"] = "活动假人",
	["Tasty Lion Steak"] = "香烤狮肉",
	["Tempered Azerothian Gemstone"] = "猝火艾泽拉斯钻石",
	["Tender Wolf Steak"] = "嫩狼肉排",
	["The Big One"] = "大炸弹",
	["The Golden Goblet"] = "黄金酒杯",
	["The King's Conviction"] = "王之决断",
	["The Mortar: Reloaded"] = "迫击炮：重载",
	["The Shatterer"] = "粉碎者",
	["The purification of tainted Felcloth into Mooncloth can only be done at a moonwell, and only ocassionally will the waters permit themselves to be used in such a way."] = "将被玷污的恶魔布净化为月布的过程只能在月亮井中进行。",
	["Thick Armor Kit"] = "厚重护甲片",
	["Thick Leather"] = "厚皮",
	["Thick Leather Ammo Pouch"] = "厚皮弹药包",
	["Thick Murloc Armor"] = "厚鱼人皮甲",
	["Thick Obsidian Breastplate"] = "厚重黑曜石胸甲",
	["Thick War Axe"] = "厚重战斧",
	["Thistle Tea"] = "菊花茶",
	["Thorium Armor"] = "瑟银护甲",
	["Thorium Belt"] = "瑟银腰带",
	["Thorium Belt Buckle"] = "瑟银带扣",
	["Thorium Boots"] = "瑟银长靴",
	["Thorium Bracers"] = "瑟银护腕",
	["Thorium Greatsword"] = "瑟银巨剑",
	["Thorium Grenade"] = "瑟银手榴弹",
	["Thorium Helm"] = "瑟银头盔",
	["Thorium Leggings"] = "瑟银护腿",
	["Thorium Rifle"] = "瑟银火枪",
	["Thorium Shells"] = "瑟银弹",
	["Thorium Shield Spike"] = "瑟银盾刺",
	["Thorium Spurs"] = "瑟银马刺",
	["Thorium Tube"] = "瑟银管",
	["Thorium Widget"] = "瑟银零件",
	["Tigercrest Ring"] = "虎纹戒指",
	["Timbermaw Brawlers"] = "木喉作战手套",
	["Titanic Leggings"] = "泰坦护腿",
	["Topaz Studded Ring"] = "缀玉戒指",
	["Totem of Self Preservation"] = "守护图腾",
	["Tough Scorpid Boots"] = "硬化蝎壳战靴",
	["Tough Scorpid Bracers"] = "硬化蝎壳护腕",
	["Tough Scorpid Breastplate"] = "硬化蝎壳胸甲",
	["Tough Scorpid Gloves"] = "硬化蝎壳手套",
	["Tough Scorpid Helm"] = "硬化蝎壳头盔",
	["Tough Scorpid Leggings"] = "硬化蝎壳护腿",
	["Tough Scorpid Shoulders"] = "硬化蝎壳护肩",
	["Toughened Leather Armor"] = "韧化皮甲",
	["Toughened Leather Gloves"] = "韧化皮手套",
	["Towerforge Breastplate"] = "塔铸胸甲",
	["Towerforge Crown"] = "塔铸皇冠",
	["Towerforge Demolisher"] = "塔铸破坏者",
	["Towerforge Pauldrons"] = "塔铸肩铠",
	["Tranquil Mechanical Yeti"] = "安静的机械雪人",
	["Transmute: Air to Fire"] = "转化：点气成火",
	["Transmute: Arcanite"] = "转化：奥金",
	["Transmute: Earth to Life"] = "转化：土转生命",
	["Transmute: Earth to Water"] = "转化：转土成水",
	["Transmute: Elemental Earth"] = "转化：元素之土",
	["Transmute: Elemental Fire"] = "转化：元素火焰",
	["Transmute: Elemental Water"] = "转化：元素之水",
	["Transmute: Fire to Earth"] = "转化：点火成土",
	["Transmute: Iron to Gold"] = "点铁成金",
	["Transmute: Life to Earth"] = "转化：生命归土",
	["Transmute: Mithril to Truesilver"] = "转化秘银",
	["Transmute: Undeath to Water"] = "转化：死灵化水",
	["Transmute: Water to Air"] = "转化：点水成气",
	["Transmute: Water to Undeath"] = "转化：水转死灵",
	["Transmutes Air into Fire."] = "将空气精华转化为火焰精华。",
	["Transmutes Earth into Water."] = "将大地精华转化为水之精华。",
	["Transmutes Earth to Living Essence."] = "将大地精华转化为生命精华。",
	["Transmutes Fire into Earth."] = "将火焰精华转化为大地精华。",
	["Transmutes Living Essence into Earth."] = "将生命精华转化为大地精华。",
	["Transmutes Water into Air."] = "将水之精华转化为空气精华。",
	["Transmutes a Globe of Water into three Elemental Water."] = "将一份纯水之球转化成为三份元素之水。",
	["Transmutes a Heart of Fire into three Elemental Earth."] = "将一份大地之核转化成为三份元素之土。",
	["Transmutes a Heart of Fire into three Elemental Fires."] = "将一份火焰之心转化成为三份元素火焰。",
	["Traveler's Tent"] = "旅行帐篷",
	["Truefaith Gloves"] = "信念手套",
	["Truefaith Vestments"] = "信念外衣",
	["Truesilver Belt Buckle"] = "真银带扣",
	["Truesilver Breastplate"] = "真银胸甲",
	["Truesilver Champion"] = "真银圣剑",
	["Truesilver Gauntlets"] = "真银护手",
	["Truesilver Rod"] = "真银棒",
	["Truesilver Skeleton Key"] = "真银万能钥匙",
	["Truesilver Transformer"] = "真银变压器",
	["Turns a bar of Iron into a bar of Gold."] = "将一块铁锭变成金锭。",
	["Turns a bar of Mithril into a bar of Truesilver."] = "将一块秘银锭变成真银锭。",
	["Turns a bar of Thorium and an Arcane Crystal into Arcanite."] = "将一块瑟银锭和一块奥术水晶转化为一块奥金。",
	["Turtle Scale Bracers"] = "龟壳护腕",
	["Turtle Scale Breastplate"] = "龟壳胸甲",
	["Turtle Scale Gloves"] = "龟壳手套",
	["Turtle Scale Helm"] = "龟壳头盔",
	["Turtle Scale Leggings"] = "龟壳护腿",
	["Tuxedo Jacket"] = "礼服夹克",
	["Tuxedo Pants"] = "礼服短裤",
	["Tuxedo Shirt"] = "礼服衬衣",
	["Twilight Opal Cascade"] = "微光白玉坠戒",
	["Ultra-Flash Shadow Reflector"] = "快速暗影反射器",
	["Ultrasafe Transporter - Gadgetzan"] = "安全传送器 - 加基森",
	["Undermine Clam Chowder"] = "安德麦蚌肉杂烩",
	["Unstable Arcane Gemstone"] = "不稳定的奥术宝石",
	["Unstable Mining Dynamite"] = "不稳定的采矿炸药",
	["Unstable Trigger"] = "不牢固的扳机",
	["Untempered Runeblade"] = "未淬火的符文剑",
	["Venomspire Diadem"] = "猛毒尖冠",
	["Verdant Dreamer's Breastplate"] = "翡翠梦境者护胸",
	["Vitriol Brooch"] = "天青石胸针",
	["Voice Amplification Modulator"] = "语音增强模组",
	["Voidheart Charm"] = "虚空之心咒符",
	["Volcanic Breastplate"] = "火山胸甲",
	["Volcanic Hammer"] = "火山战锤",
	["Volcanic Leggings"] = "火山护腿",
	["Volcanic Shoulders"] = "火山护肩",
	["Voltage-Neutralizing Nature Reflector"] = "电压中和自然反射器",
	["Warbear Harness"] = "战熊背心",
	["Warbear Woolies"] = "战熊热裤",
	["Weak Troll's Blood Potion"] = "弱效巨魔之血药水",
	["Westfall Stew"] = "杂味炖肉",
	["Whirring Bronze Gizmo"] = "高速青铜齿轮",
	["White Bandit Mask"] = "白色强盗面罩",
	["White Leather Jerkin"] = "白色皮夹克",
	["White Linen Robe"] = "白色亚麻长袍",
	["White Linen Shirt"] = "白色亚麻衬衣",
	["White Swashbuckler's Shirt"] = "白色冒险者衬衣",
	["White Wedding Dress"] = "白色婚纱",
	["White Woolen Dress"] = "白色毛绒裙",
	["Whitesoul Helm"] = "白魂头盔",
	["Wicked Leather Armor"] = "邪恶皮甲",
	["Wicked Leather Belt"] = "邪恶皮甲腰带",
	["Wicked Leather Bracers"] = "邪恶皮甲护腕",
	["Wicked Leather Gauntlets"] = "邪恶皮甲护手",
	["Wicked Leather Headband"] = "邪恶皮甲头环",
	["Wicked Leather Pants"] = "邪恶皮甲短裤",
	["Wicked Mithril Blade"] = "秘银魔剑",
	["Wild Leather Boots"] = "蛮皮战靴",
	["Wild Leather Cloak"] = "蛮皮披风",
	["Wild Leather Helmet"] = "蛮皮头盔",
	["Wild Leather Leggings"] = "蛮皮护腿",
	["Wild Leather Shoulders"] = "蛮皮护肩",
	["Wild Leather Vest"] = "蛮皮外衣",
	["Wildthorn Mail"] = "野刺锁甲",
	["Wildvine Potion"] = "野葡萄药水",
	["Windbinder Gloves"] = "缚风者手套",
	["Windwalker Boots"] = "风行者之靴",
	["Wisdom of the Timbermaw"] = "木喉之智",
	["With  Swiftthistle for speed and crushed Silverleaf for enhancement, a potion to to enchance ones natural agility is brewed."] = "由雨燕草和银叶草精心配置的药水，可以让饮用者变得更加敏捷。",
	["With the mixture of Snakeroot for physical improvement and crushed Silverleaf for enhancement, a potion to to bring out one's inner lion is brewed."] = "由蛇根草和银叶草精心配置的药水，可以让饮用者变得更加强壮。",
	["Wizardweave Leggings"] = "巫纹护腿",
	["Wizardweave Robe"] = "巫纹长袍",
	["Wizardweave Turban"] = "巫纹头巾",
	["Wolfshead Helm"] = "狼头之盔",
	["Wool Bandage"] = "绒线绷带",
	["Woolen Bag"] = "毛纺包",
	["Woolen Boots"] = "毛纺靴",
	["Woolen Cape"] = "毛纺斗篷",
	["World Enlarger"] = "世界放大器",
	["Wound Poison"] = "致伤毒药",
	["Wound Poison II"] = "致伤毒药 II",
	["Wound Poison III"] = "致伤毒药 III",
	["Wound Poison IV"] = "致伤毒药 IV",
	--2025/9/24吊儿啷当添加
	--副本
	["[13-18] Ragefire Chasm"] = WHITE.."[13-18]"..DEFAULT.."  怒焰裂谷",
	["[13-20] Frostmane Hollow"] = WHITE.."[13-20]"..DEFAULT.."  霜鬃谷",
    ["[17-24] The Deadmines"] = WHITE.."[17-24]"..DEFAULT.."  死亡矿井",
    ["[17-24] Wailing Caverns"] = WHITE.."[17-24]"..DEFAULT.."  哀嚎洞穴",
    ["[22-30] Shadowfang Keep"] = WHITE.."[22-30]"..DEFAULT.."  影牙城堡",
    ["[22-30] The Stockade"] = WHITE.."[22-30]"..DEFAULT.."  监狱",
    ["[23-32] Blackfathom Deeps"] = WHITE.."[23-32]"..DEFAULT.."  黑暗深渊",
	["[26-30] Windhorn Canyon"] = WHITE.."[26-30]"..DEFAULT.."  风角峡谷",--1.18.1新增
    ["[26-35] Dragonmaw Retreat"] = WHITE.."[26-35]"..DEFAULT.."  龙喉居所",
    ["[27-36] Scarlet Monastery (Graveyard)"] = WHITE.."[27-36]"..DEFAULT.."  血色修道院（墓地）",
    ["[27-45] Scarlet Monastery"] = WHITE.."[27-45]"..DEFAULT.."  血色修道院",
    ["[28-39] Scarlet Monastery (Library)"] = WHITE.."[28-39]"..DEFAULT.."  血色修道院（图书馆）",
    ["[29-38] Gnomeregan"] = WHITE.."[29-38]"..DEFAULT.."  诺莫瑞根",
    ["[29-38] Razorfen Kraul"] = WHITE.."[29-38]"..DEFAULT.."  剃刀沼泽",
    ["[32-38] The Crescent Grove"] = WHITE.."[32-38]"..DEFAULT.."  新月林地",
    ["[32-41] Scarlet Monastery (Armory)"] = WHITE.."[32-41]"..DEFAULT.."  血色修道院（军械库）",
    ["[32-44] Stormwrought Ruins"] = WHITE.."[32-44]"..DEFAULT.."  风暴废墟",
    ["[35-45] Scarlet Monastery (Cathedral)"] = WHITE.."[35-45]"..DEFAULT.."  血色修道院（大教堂）",
    ["[36-46] Razorfen Downs"] = WHITE.."[36-46]"..DEFAULT.."  剃刀高地",
    ["[40-51] Uldaman"] = WHITE.."[40-51]"..DEFAULT.."  奥达曼",
    ["[42-50] Gilneas City"] = WHITE.."[42-50]"..DEFAULT.."  吉尔尼斯城",
    ["[44-54] Zul'Farrak"] = WHITE.."[44-54]"..DEFAULT.."  祖尔法拉克",
    ["[45-55] Maraudon"] = WHITE.."[45-55]"..DEFAULT.."  玛拉顿",
    ["[50-60] Hateforge Quarry"] = WHITE.."[50-60]"..DEFAULT.."  仇恨熔炉采石场",
    ["[50-60] The Sunken Temple"] = WHITE.."[50-60]"..DEFAULT.."  沉没的神庙",
    ["[52-60] Blackrock Depths"] = WHITE.."[52-60]"..DEFAULT.."  黑石深渊",
    ["[55-60] Dire Maul (East)"] = WHITE.."[55-60]"..DEFAULT.."  厄运之槌 (东)",
    ["[55-60] Dire Maul"] = WHITE.."[55-60]"..DEFAULT.."  厄运之槌",
    ["[55-60] Lower Blackrock Spire"] = WHITE.."[55-60]"..DEFAULT.."  黑石塔下层",
    ["[57-60] Dire Maul (North)"] = WHITE.."[57-60]"..DEFAULT.."  厄运之槌 (北)",
    ["[57-60] Dire Maul (West)"] = WHITE.."[57-60]"..DEFAULT.."  厄运之槌 (西)",
    ["[58-60] Karazhan Crypt"] = WHITE.."[58-60]"..DEFAULT.."  卡拉赞墓穴",
    ["[58-60] Scholomance"] = WHITE.."[58-60]"..DEFAULT.."  通灵学院",
    ["[58-60] Stratholme"] = WHITE.."[58-60]"..DEFAULT.."  斯坦索姆",
    ["[58-60] Upper Blackrock Spire"] = WHITE.."[58-60]"..DEFAULT.."  黑石塔上层",
    ["[60] Caverns of Time: Black Morass"] = WHITE.."[60]"..DEFAULT.."  时光之穴：黑色沼泽",
    ["[60] Stormwind Vault"] = WHITE.."[60]"..DEFAULT.."  暴风城地牢",
    ["[RAID] Blackwing Lair"] = RED.."[团本]"..DEFAULT.."  黑翼之巢",
    ["[RAID] Emerald Sanctum"] = RED.."[团本]"..DEFAULT.."  翡翠圣殿",
    ["[RAID] Lower Karazhan Halls"] = RED.."[团本]"..DEFAULT.."  卡拉赞下层大厅",
    ["[RAID] Molten Core"] = RED.."[团本]"..DEFAULT.."  熔火之心",
    ["[RAID] Naxxramas"] = RED.."[团本]"..DEFAULT.."  纳克萨玛斯",
    ["[RAID] Onyxia's Lair"] = RED.."[团本]"..DEFAULT.."  奥妮克希亚的巢穴",
    ["[RAID] Ruins of Ahn'Qiraj"] = RED.."[团本]"..DEFAULT.."  安其拉废墟",
    ["[RAID] Temple of Ahn'Qiraj"] = RED.."[团本]"..DEFAULT.."  安其拉神殿",
    ["[RAID] Upper Karazhan Halls"] = RED.."[团本]"..DEFAULT.."  卡拉赞之塔",
    ["[RAID] Zul'Gurub"] = RED.."[团本]"..DEFAULT.."  祖尔格拉布",
	["[RAID] Timbermaw Hold"] = RED.."[团本]"..DEFAULT.."  木喉要塞",--1.18.1新增

    ["Ragefire Chasm"] = "怒焰裂谷",
    ["The Deadmines"] = "死亡矿井",
    ["Wailing Caverns"] = "哀嚎洞穴",
    ["Shadowfang Keep"] = "影牙城堡",
    ["The Stockade"] = "监狱",
    ["Blackfathom Deeps"] = "黑暗深渊",
	["Frostmane Hollow"] = "霜鬃谷",--1.18.1新增
	["Windhorn Canyon"] = "风角峡谷",--1.18.1新增
    ["Dragonmaw Retreat"] = "龙喉居所",
    ["Scarlet Monastery (Graveyard)"] = "血色修道院（墓地）",
    ["Scarlet Monastery"] = "血色修道院",
    ["Scarlet Monastery (Library)"] = "血色修道院（图书馆）",
    ["Gnomeregan"] = "诺莫瑞根",
    ["Razorfen Kraul"] = "剃刀沼泽",
    ["The Crescent Grove"] = "新月林地",
    ["Scarlet Monastery (Armory)"] = "血色修道院（军械库）",
    ["Stormwrought Ruins"] = "风暴废墟",
    ["Scarlet Monastery (Cathedral)"] = "血色修道院（大教堂）",
    ["Razorfen Downs"] = "剃刀高地",
    ["Uldaman"] ="奥达曼",
    ["Gilneas City"] = "吉尔尼斯城",
    ["Zul'Farrak"] = "祖尔法拉克",
    ["Maraudon"] = "玛拉顿",
    ["Hateforge Quarry"] = "仇恨熔炉采石场",
    ["The Sunken Temple"] = "沉没的神庙",
    ["Blackrock Depths"] = "黑石深渊",
    ["Dire Maul (East)"] = "厄运之槌 (东)",
    ["Dire Maul"] = "厄运之槌",
    ["Lower Blackrock Spire"] = "黑石塔下层",
    ["Dire Maul (North)"] = "厄运之槌 (北)",
    ["Dire Maul (West)"] = "厄运之槌 (西)",
    ["Karazhan Crypt"] = "卡拉赞墓穴",
    ["Scholomance"] = "通灵学院",
    ["Stratholme"] = "斯坦索姆",
    ["Upper Blackrock Spire"] = "黑石塔上层",
    ["Black Morass"] = "时光之穴：黑色沼泽",
    ["Stormwind Vault"] = "暴风城地牢",
    ["Lower Karazhan Halls"] = "卡拉赞下层",
    ["Upper Karazhan Halls"] = "卡拉赞上层",
    ["Blackwing Lair"] = "黑翼之巢",
    ["Molten Core"] = "熔火之心",
    ["Emerald Sanctum"] = "翡翠圣殿",
    ["Zul'Gurub"] = "祖尔格拉布",
    ["Ruins of Ahn'Qiraj"] = "安其拉废墟",
    ["Temple of Ahn'Qiraj"] = "安其拉神殿",
    ["Naxxramas"] = "纳克萨玛斯",
	-- ["Timbermaw Hold"] = "木喉要塞",--1.18.1新增

	["default"] = "默认",
    ["has been reset!"] = "已重置！",
    ["on this page. The items will be"] = "中的所有物品链接",
    ["options"] = "选项",
    ["over them."] = "下次滑过时刷新",
    ["panel"] = "面板",
    ["random colour"] = "随机颜色",
    ["random resistance"] = "随机抗性",
    ["random stats"] = "随机属性",
    ["refreshed when you next mouse"] = "物品将会在鼠标",
    ["reset"] = "重置",
    ["sucessfully loaded."] = "已成功载入",
    ["|cff1eff00Left-Click|r Browse Loot Tables"] = "|cff1eff00单击|r 浏览掉落表",
    ["|cff9d9d9dItemSync Tooltips|r"] = "|cff9d9d9dItemSync提示样式|r",
    ["|cff9d9d9dLootlink Tooltips|r"] = "|cff9d9d9dLootlink提示样式",
    ["|cff9d9d9dUse EquipCompare|r"] = "|cff9d9d9d使用装备对比|r",
    ["|cffccccccRight-Click + Drag|r Move Minimap Button"] = "|cffcccccc右键点击 + 拖动|r 移动小地图按钮",
    ["|cffff0000(unsafe)"] = "|cffff0000(不安全)",
    ["|cffff0000Right-Click|r View Options"] = "|cffff0000右键点击|r 进行设置",
    ["|cffff0000Shift-Click|r View Options"] = "|cffff0000Shift-单击|r 进行设置",

	--世界boss
	["Azuregos"] = "艾索雷葛斯", 
	["Cla'ckora"] = "|CFF00FF00克拉科拉",
    ["Emeriss"] = "|CFF00FF00艾莫莉丝",
    ["Lethon"] = "|CFF00FF00雷索",
    ["Taerar"] = "|CFF00FF00泰拉尔",
	["Ysondre"] = "|CFF00FF00伊森德雷",
    ["Lord Kazzak"] = "卡扎克",
    ["Nerubian Overseer"] = "|CFF00FF00蛛魔监工",
    ["Dark Reaver of Karazhan"] = "|CFF00FF00卡拉赞黑暗掠夺者",
    ["Ostarius"] = "|CFF00FF00奥兹塔里亚斯-奥丹姆",
    ["Concavius"] = "|CFF00FF00空卡维斯 <虚空之子>",
	["Moo"] = "奶牛之王",
	--区域名称
	["Orgrimmar"] = "|cffff0000奥格瑞玛",
	["Barrens"] = "贫瘠之地",
	["Westfall"] = "西部荒野",
	["Silverpine Forest"] = "银松森林",
	["Ashenvale"] = "灰谷",
	["Stormwind"] = "|cff2773ff暴风城",
	["Wetlands"] = "湿地",
	["Tirisfal Glades"] = "提瑞斯法林地",
	["Thousand Needles"] = "千针石林",
	["Balor"] = "巴洛",
	["Badlands"] = "荒芜之地",
	["Gilneas"] = "吉尔尼斯",
	["Desolace"] = "凄凉之地",
	["Tanaris"] = "塔纳利斯",
	["Swamp of Sorrows"] = "悲伤沼泽",
	["Burning Steppes"] = "燃烧平原",
	["Feralas"] = "菲拉斯",
	["Western Plaguelands"] = "西瘟疫之地",
	["Eastern Plaguelands"] = "东瘟疫之地",
	["Deadwind Pass"] = "逆风小径",
	["Stranglethorn Vale"] = "荆棘谷",
	["Dustwallow Marsh"] = "尘泥沼泽",
	["Hyjal"] = "海加尔山",
	["Silithus"] = "希利苏斯",
	["Stonetalon Mountains"] = "石爪山",
	["Searing Gorge"] = "灼热峡谷",
	["Hinterlands"] = "辛特兰",
	["Gilijim Isle"] = "吉利吉姆岛",
	["Lapidis Isle"] = "拉匹迪斯之岛",
	["Azshara"] = "艾萨拉",
	["Felwood"] = "费伍德森林",
	["Tel'abim"] = "泰拉比姆",
	["Blasted Lands"] = "诅咒之地", 
	["Dun Morogh"] = "丹莫罗",
	["Arathi Highlands"] = "阿拉希高地",
	["Winterspring"] = "冬泉谷",
	["Scarlet Enclave"] = "血色领地",
	["Karazhan"] = "卡拉赞",
    ["Un'Goro Crater"] = "安戈洛环形山",
	--稀有精英
	["Earthcaller Rezengal"] = "唤地者雷森加尔",
	["Shade Mage"] = "暗影法师",
	["Graypaw Alpha"] = "灰爪阿尔法",
	["Blazespark"] = "火星",
	["Witch Doctor Tan'zo"] = "巫医坦佐",
	["Dawnhowl"] = "黎明嚎叫",
	["Maltimor's Prototype"] = "马尔蒂莫的原型",
	["Bonecruncher"] = "碎骨者",
	["Duskskitter"] = "暮掠者",
	["Baron Perenolde"] = "男爵匹瑞诺德",
	["Grug'thok the Seer"] = "先知格鲁格索克",
	["Explorer Ashbeard"] = "探索者尘须",
	["Jal'akar"] = "嘉卡尔",
	["Embereye"] = "烬眼",
	["Ruk'thok the Pyromancer"] = "火法师鲁克索克",
	["Tarangos"] = "塔兰苟斯",
	["Xalvic Blackclaw"] = "夏威克·黑爪",
	["Aquitus"] = "阿奎图斯",
    ["Kin'Tozo"] = "金托佐",
    ["Ripjaw"] = "裂口",
	["Firstborn of Arugal"] = "阿鲁高的长子",
	["Letashaz"] = "莱塔沙兹",
	["Margon the Mighty"] = "强大的玛贡",
	["The Wandering Knight"] = "游荡骑士",
	["Stoneshell"] = "石壳",
	["Zareth Terrorblade"] = "吒雷斯·恐怖利刃",
	["Highvale Silverback"] = "高原银背兽",
	["Mallon The Moontouched"] = "月抚犸郎",
	["Blademaster Kargron"] = "剑圣咖哥隆",
	["Professor Lysander"] = "莱桑德·琥珀",
	["Admiral Barean Westwind"] = "海军上将巴利安·西风",
	["Azurebeak"] = "蔚蓝喙",
	["Barkskin Fisher"] = "树皮渔夫",
	["Crusader Larsarius"] = "十字军拉萨里斯",
	["Shadeflayer Goliath"] = "暗灭者歌利亚",
	["Widow of the Woods"] = "树林寡妇",
	["M-0L1Y"] = "M-0L1Y",
	--制造业
	["Blacksmithing"] = "锻造",
	["Leatherworking"] = "制皮",
	["Tailoring"] = "裁缝",
	["Engineering"] = "工程学",
	["Alchemy"] = "炼金术",
	["Cooking"] = "烹饪",
	["First Aid"] = "急救",
	["Enchanting"] = "附魔",
	["Jewelcrafting"] = "珠宝加工",
	["Mining"] = "采矿",
	["Herbalism"] = "|cffffd200草药学",
	["Survival"] = "生存",
	["Gardening"] = "园艺",
	["Poisons"] = "毒药",
    ["Skill:"] = "技能：",
    ["Level:"] = "等级：",
    ["Smelting"] = "熔炼",
	--炼金术
	["Apprentice"] = "学徒",
	["Journeyman"] = "中级",
	["Expert"] = "高级",
	["Artisan"] = "大师",
	["Flasks"] = "药剂",
	["Protection Potions"] = "防护药水",
	["Health and Mana Potions"] = "治疗和法力药水",
	["Transmutes"] = "转化",
	["Defensive Potions and Elixirs"] = "防御药水和药剂",
	["Offensive Potions and Elixirs"] = "攻击药水和药剂",
	--附魔
	["Bracers"] = "护腕",
	["Gloves"] = "手套",
    ["Boots"] = "靴子",
    ["2H Weapon"] = "双手武器",
	["Weapon"] = "武器",
	["Shield"] = "盾牌",
    ["Bracer"] = "护腕",
	--工程学
    ["Equipment"] = "装备",
    ["Trinkets"] = "饰品",
    ["Explosives"] = "炸药",
    ["Parts"] = "零件",
    ["Gnomish Engineering"] = "侏儒工程学",
	["Goblin Engineering"] = "地精工程学",
	--制皮
	["Helm"] = "头盔",
    ["Belt"] = "腰带",
	["Pants"] = "裤子",
	["Bags"] = "背包",
    ["Dragonscale"] = "龙鳞",
    ["Dragonscale Leatherworking"] = "龙鳞制皮",
	["Elemental Leatherworking"] = "元素制皮",
	["Tribal Leatherworking"] = "部族制皮",
    ["Leather"] = "皮甲",
    ["Grifter's Armor"] = "欺诈者的盔甲",
    ["Primalist's Trappings"] = "原始者的装备",
    ["Red Dragon Mail"] = "红龙锁甲",
	--锻造
    ["Belt Buckles"] = "腰带扣",
	["Axes"] = "斧头",
	["Swords"] = "剑",
    ["Maces"] = "锤",
    ["Fist"] = "拳套",
    ["Daggers"] = "匕首",
    ["Armorsmith"] = "护甲锻造师",
	["Weaponsmith"] = "武器锻造师",
	["Master Axesmith"] = "铸斧大师",
	["Master Hammersmith"] = "铸锤大师",
	["Master Swordsmith"] = "铸剑大师",
	["Mail"] = "锁甲",
	["Plate"] = "板甲",
	["Steel Plate"] = "板甲",
	--珠宝加工
    ["Gemstones"] = "宝石",
	["Rings"] = "戒指",
	["Amulets"] = "项链",
	["Staff"] = "法杖",
	["Gemology"] = "宝石学",
    ["Goldsmithing"] = "金匠学",
	--裁缝
	["Augerer's Attire"] = "奥格瑞尔的服装",
	["Shadoweave"] = "暗纹",
    ["Diviner's Garments"] = "占卜师的服饰",
    ["Pillager's Garb"] = "劫掠者的服饰",
    ["Mooncloth Regalia"] = "月布套装",
	--职业
	["Warrior"] = "战士",
	["Paladin"] = "圣骑士",
	["Hunter"] = "猎人",
	["Rogue"] = "盗贼",
	["Priest"] = "牧师",
	["Shaman"] = "萨满祭司",
	["Mage"] = "法师",
	["Warlock"] = "术士",
	["Druid"] = "德鲁伊",
	--PVP奖励
	["Arathi Basin"] = "阿拉希盆地",
	["Alterac Valley"] = "奥特兰克山谷",
	["Warsong Gulch"] = "战歌峡谷",
	["Blood Ring"] = "|CFF00FF00血环",
	["PvP Mounts"] = "|cffffd200PvP坐骑",
	["Rank 11"] = "11 级",
    ["Rank 14 Weapons"] = "|cffffd200R14 武器",
    ["Rank 14"] = "14 级",
    ["Rank 2-9"] = "2-9 级",
    ["Rank 7-13"] = "7-13 级",
    ["Rank"] = "等级",
	["Friendly Reputation Rewards"] = "友善声望奖励",
	["Honored Reputation Rewards"] = "尊敬声望奖励",
	["Revered Reputation Rewards"] = "崇敬声望奖励",
	["Exalted Reputation Rewards"] = "崇拜声望奖励",
	["Korrak the Bloodrager"] = "血怒者科尔拉克",
	["Ivus & Lokholar"] = "伊弗斯 & 洛克霍拉",
	["AB 20-29 Honored Rewards"] = "阿拉希盆地 20-29 尊敬奖励",
    ["AB 20-29 Revered Rewards"] = "阿拉希盆地 20-29 崇敬奖励",
    ["AB 30-39 Honored Rewards"] = "阿拉希盆地 30-39 尊敬奖励",
    ["AB 30-39 Revered Rewards"] = "阿拉希盆地 30-39 崇敬奖励",
    ["AB 40-49 Honored Rewards"] = "阿拉希盆地 40-49 尊敬奖励",
    ["AB 40-49 Revered Rewards"] = "阿拉希盆地 40-49 崇敬奖励",
    ["AB 50-59 Honored Rewards"] = "阿拉希盆地 50-59 尊敬奖励",
    ["AB 50-59 Revered Rewards"] = "阿拉希盆地 50-59 崇敬奖励",
    ["AB Exalted Rewards"] = "阿拉希盆地 崇拜奖励",
    ["AB Friendly Rewards"] = "阿拉希盆地 友善奖励",
	["AV Exalted Rewards"] = "奥特兰克山谷 崇拜奖励",
    ["AV Friendly Rewards"] = "奥特兰克山谷 友善奖励",
    ["AV Honored Rewards"] = "奥特兰克山谷 尊敬奖励",
    ["AV Revered Rewards"] = "奥特兰克山谷 崇敬奖励",
	["WSG 10-19 Honored Rewards"] = "战歌峡谷 10-19 尊敬奖励",
    ["WSG 10-19 Revered Rewards"] = "战歌峡谷 10-19 崇敬奖励",
    ["WSG 20-29 Honored Rewards"] = "战歌峡谷 20-29 尊敬奖励",
    ["WSG 20-29 Revered Rewards"] = "战歌峡谷 20-29 崇敬奖励",
    ["WSG 30-39 Honored Rewards"] = "战歌峡谷 30-39 尊敬奖励",
    ["WSG 30-39 Revered Rewards"] = "战歌峡谷 30-39 崇敬奖励",
    ["WSG 40-49 Exalted Rewards"] = "战歌峡谷 40-49 崇拜奖励",
    ["WSG 40-49 Honored Rewards"] = "战歌峡谷 40-49 尊敬奖励",
    ["WSG 40-49 Revered Rewards"] = "战歌峡谷 40-49 崇敬奖励",
    ["WSG 50-59 Exalted Rewards"] = "战歌峡谷 50-59 崇拜奖励",
    ["WSG 50-59 Honored Rewards"] = "战歌峡谷 50-59 尊敬奖励",
    ["WSG 50-59 Revered Rewards"] = "战歌峡谷 50-59 崇敬奖励",
    ["WSG 60 Exalted Rewards"] = "战歌峡谷 60 崇拜奖励",
    ["WSG Friendly Rewards"] = "战歌峡谷 友善奖励",
	["Blood Ring Exalted Rewards"] = "血环 崇拜奖励",
    ["Blood Ring Friendly Rewards"] = "血环 友善奖励",
    ["Blood Ring Honored Rewards"] = "血环 尊敬奖励",
    ["Blood Ring Revered Rewards"] = "血环 崇敬奖励",
	["Token of Blood Rewards"] = "鲜血令牌奖励",
	--套装/收藏
	["Sets"] = "套装",
	["Karazhan Sets"] = "T3.5卡拉赞之塔套装",
	["Rare Pets"] = "|cffffd200稀有宠物",
	["Tier 1 Sets"] = "T1套装",
	["Tier 2 Sets"] = "T2套装",
	["Tier 3 Sets"] = "T3套装",
        ["Tier 3.5"] = "T3.5",
	--阵营
	["Argent Dawn"] = "银色黎明",
    ["Bloodsail Buccaneers"] = "血帆海盗",
	["Brood of Nozdormu"] = "诺兹多姆的子嗣",
	["Cenarion Circle"] = "塞纳里奥议会",
	["Darkmoon Faire"] = "暗月马戏团",
    ["Gelkis Clan Centaur"] = "吉尔吉斯半人马",
	["Hydraxian Waterlords"] = "海达希亚水元素",
    ["Magram Clan Centaur"] = "玛格拉姆半人马",
    ["Thorium Brotherhood"] = "瑟银兄弟会",
	["Timbermaw Hold"] = "木喉要塞",
    ["Shen'dralar"] = "辛德拉",
    ["Zandalar Tribe"] = "赞达拉部族",
	["Wintersaber Trainers"] = "|cff2773ff冬刃豹训练师",
    ["Wardens of Time"] = "时间守护者",
    ["Wildhammer Clan"] = "|cff2773ff蛮锤部落",
    ["Stormpike Guard"] = "雷矛卫队",
    ["Frostwolf Clan"] = "霜狼氏族",
    ["Dalaran"] = "|CFF00FF00达拉然",
    ["Darkspear Trolls"] = "|cffff0000暗矛巨魔",
    ["Silvermoon Remnant"] = "|cff2773ff银月幸存者",
    ["Revantusk Trolls"] = "|cffff0000恶齿巨魔",
    ["Ironforge"] = "|cff2773ff铁炉堡",
    ["Durotar Labor Union"] = "|cffff0000杜隆塔尔工会",
    ["Darnassus"] = "|cff2773ff达纳苏斯",
    ["Undercity"] = "|cffff0000幽暗城",
    ["Gnomeregan Exiles"] = "|cff2773ff诺莫瑞根复兴城",
    ["Thunder Bluff"] = "|cffff0000雷霆崖",
	--副本boss名称
	--怒焰裂谷
	["Jergosh the Invoker"] = "祈求者耶戈什",
	["Bazzalan"] = "巴扎兰",
	["Taragaman the Hungerer"] = "饥饿者塔拉加曼",
	["Oggleflint"] = "奥格弗林特",
	--哀嚎洞穴
    ["Lord Cobrahn"] = "考布莱恩领主",
    ["Lady Anacondra"] = "安娜科德拉女士",
    ["Kresh"] = "克雷什",
    ["Deviate Faerie Dragon"] = "变异精灵龙",
    ["Zandara Windhoof"] = "赞达拉·风蹄",
    ["Lord Pythas"] = "皮萨斯领主",
    ["Skum"] = "斯卡姆",
    ["Vangros"] = "范格罗斯",
    ["Lord Serpentis"] = "瑟芬迪斯领主",
    ["Verdan the Everliving"] = "永生者沃尔丹",
    ["Mutanus the Devourer"] = "吞噬者穆坦努斯",
	--死亡矿井
    ["Jared Voss"] = "杰里德·维斯",
    ["Rhahk'Zor"] = "拉克佐",
    ["Miner Johnson"] = "矿工约翰森",
    ["Sneed"] = "斯尼德",
    ["Sneed's Shredder"] = "斯尼德的伐木机",
    ["Gilnid"] = "基尔尼格",
    ["Masterpiece Harvester"] = "收割者-最后杰作",
    ["Mr. Smite"] = "重拳先生",
	["Cookie"] = "曲奇",
    ["Captain Greenskin"] = "绿皮队长",
    ["Edwin VanCleef"] = "艾德温·范克里夫",
	--霜鬃谷
	["Nimble Tansha"] = "灵动的坦莎",
	["Ubbukaz Battlefield Overlord"] = "战场主宰乌布卡兹",
	["Prophet Kanza"] = "先知坎扎",
	["Cold Haile"] = "冰冷的海乐",
	--影牙城堡
	["Fel Steed"] = "地狱战马",
    ["Razorclaw the Butcher"] = "屠夫拉佐克劳",
    ["Baron Silverlaine"] = "席瓦莱恩男爵",
    ["Commander Springvale"] = "指挥官斯普林瓦尔",
    ["Odo the Blindwatcher"] = "盲眼守卫奥杜",
    ["Deathsworn Captain"] = "死亡之誓队长",
    ["Fenrus the Devourer"] = "吞噬者芬鲁斯",
	["Arugal's Voidwalker"] = "阿鲁高的虚空行者",
    ["Wolf Master Nandos"] = "狼王南杜斯",
    ["Archmage Arugal"] = "大法师阿鲁高",
    ["Prelate Ironmane"] = "艾隆迈恩主教",
	--黑暗深渊
	["Ghamoo-ra"] = "加摩拉",
	["Lady Sarevess"] = "萨利维丝女士",
	["Gelihast"] = "格里哈斯特",
    ["Baron Aquanis"] = "阿奎尼斯男爵",
    ["Velthelaxx the Defiler"] = "污染者维尔瑟拉克斯",
    ["Twilight Lord Kelris"] = "梦游者克尔里斯",
	["Old Serra'kis"] = "老瑟拉吉斯",
    ["Aku'mai"] = "阿库麦尔",
	--监狱
	["Targorr the Dread"] = "可怕的塔格尔",
    ["Kam Deepfury"] = "卡姆·深怒",
    ["Hamhock"] = "哈姆霍克",
    ["Dextren Ward"] = "迪克斯特·瓦德",
    ["Bazil Thredd"] = "巴基尔·斯瑞德",
    ["Bruegal Ironknuckle"] = "布鲁高·铁拳",
	--风角峡谷
	["Pathun Buskhide"] = "帕斯恩·暮皮",
    ["Ahgk'tos the Pure"] = "纯洁者阿格图斯",
    ["Ambassador Vortalus"] = "沃尔塔卢斯大使",
    ["Walgan Bloodcaller"] = "瓦尔根·唤血者",
    ["Bonespeaker Narlgom"] = "骨语者纳尔格姆",
    ["Prophet Stormhoof"] = "雷角",
    ["Chieftain Shalk Blackwind"] = "酋长沙尔克·黑风",
	--龙吼居所
    ["Gowlfang"] = "孤峰",
	["Cavernweb Broodmother"] = "穴织女王",
    ["Web Master Torkon"] = "织网大师托尔康",
    ["Garlok Flamekeeper"] = "卡洛克·护火者",
    ["Halgan Redbrand"] = "哈尔甘·红标",
    ["Slagfist Destroyer"] = "渣拳毁灭者",
    ["Overlord Blackheart"] = "黑心大王",
    ["Elder Hollowblood"] = "长者空血",
    ["Searistrasz"] = "希瑞斯塔萨",
    ["Zuluhed the Whacked"] = "疲惫的祖鲁希德",
	--诺莫瑞根
    ["Grubbis"] = "格鲁比斯",
	["Viscous Fallout"] = "粘性辐射尘",
    ["Electrocutioner 6000"] = "电刑器6000型",
    ["Crowd Pummeler 9-60"] = "群体打击者9-60",
    ["Dark Iron Agent"] = "黑铁密探",
    ["Mekgineer Thermaplugg"] = "机械师瑟玛普拉格",
	--剃刀沼泽
	["Death Speaker Jargba"] = "亡语者贾格巴",
    ["Overlord Ramtusk"] = "主宰拉姆塔斯",
    ["Agathelos the Raging"] = "暴怒的阿迦赛罗斯",
    ["Blind Hunter"] = "盲眼猎手",
    ["Charlga Razorflank"] = "卡尔加·刺肋",
    ["Earthcaller Halmgar"] = "唤地者哈穆加",
    ["Rotthorn"] = "罗特索恩",
	--新月林地
    ["Grovetender Engryss"] = "护林员恩格里斯",
    ["Keeper Ranathos"] = "守护者拉纳苏斯",
    ["High Priestess A'lathea"] = "高阶女祭司阿勒西",
    ["Fenektis the Deceiver"] = "欺诈者弗纳克提斯",
    ["Master Raxxieth"] = "拉克西斯大师",
	--血色修道院
    ["Interrogator Vishas"] = "审讯员韦沙斯",
    ["Duke Dreadmoore"] = "达德摩尔公爵",
    ["Ironspine"] = "铁脊死灵",
    ["Azshir the Sleepless"] = "永醒的艾希尔",
    ["Fallen Champion"] = "亡灵勇士",
    ["Bloodmage Thalnos"] = "血法师萨尔诺斯",
    ["Houndmaster Loksey"] = "驯犬者洛克希",
    ["Brother Wystan"] = "威斯坦修士",
    ["Arcanist Doan"] = "秘法师杜安",
    ["Herod"] = "赫洛德",
    ["Armory Quartermaster Daghelm"] = "军械库军需官达格海姆",
    ["High Inquisitor Fairbanks"] = "大检察官法尔班克斯",
    ["Scarlet Commander Mograine"] = "血色十字军指挥官莫格莱尼",
    ["High Inquisitor Whitemane"] = "大检察官怀特迈恩",
	--风暴废墟
    ["Oronok Torn-Heart"] = "欧鲁诺克·裂心",
    ["Dagar the Glutton"] = "暴食的达加尔",
    ["Duke Balor the IV"] = "巴洛公爵四世",
    ["Librarian Theodorus"] = "图书管理员西奥多鲁斯",
    ["Chieftain Stormsong"] = "雷歌酋长",
    ["Deathlord Tidebane"] = "死亡领主泰德贝恩",
    ["Subjugator Halthas Shadecrest"] = "征服者哈尔萨斯·暗影之冠",
    ["Mycellakos"] = "麦塞拉克斯",
    ["Eldermaw the Primordial"] = "原始的老喉巨鳄",
    ["Lady Drazare"] = "德拉扎尔女士",
    ["Remains of the Innocent"] = "无辜者的遗骸",
    ["Mergothid"] = "墨苟斯",
	--剃刀高地
    ["Tuten'kash"] = "图特卡什",
    ["Plaguemaw the Rotting"] = "腐烂的普雷莫尔",
    ["Mordresh Fire Eye"] = "火眼莫德雷斯",
    ["Glutton"] = "暴食者",
    ["Death Prophet Rakameg"] = "死亡预言者拉卡梅格",
    ["Ragglesnout"] = "拉戈斯诺特",
    ["Amnennar the Coldbringer"] = "寒冰之王亚门纳尔",
	--奥达曼
    ["Baelog"] = "巴尔洛戈",
    ["Revelosh"] = "鲁维罗什",
    ["Ironaya"] = "艾隆纳亚",
    ["Ancient Stone Keeper"] = "远古巨石卫士",
    ["Galgann Firehammer"] = "加加恩·火锤",
    ["Grimlok"] = "格瑞姆洛克",
    ["Archaedas"] = "阿扎达斯",
	--吉尔尼斯城
    ["Matthias Holtz"] = "马蒂亚斯·霍尔茨",
    ["Packmaster Ragetooth"] = "兽群首领怒牙",
    ["Judge Sutherland"] = "萨瑟兰法官",
    ["Dustivan Blackcowl"] = "达斯蒂万·布莱克考尔",
    ["Marshal Magnus Greystone"] = "元帅马格努斯·格雷斯通",
    ["Horsemaster Levvin"] = "御马司莱文",
    ["Harlow Family Chest"] = "哈洛家庭宝箱",
    ["Genn Greymane"] = "吉恩·格雷迈恩",
	--玛拉顿
	["Noxxion"] = "诺克赛恩",
    ["Razorlash"] = "锐刺鞭笞者",
    ["Lord Vyletongue"] = "维利塔恩",
    ["Meshlok the Harvester"] = "收割者麦什洛克",
    ["Celebras the Cursed"] = "被诅咒的塞雷布拉斯",
    ["Landslide"] = "兰斯利德",
    ["Tinkerer Gizlock"] = "工匠吉兹洛克",
    ["Rotgrip"] = "洛特格里普",
    ["Princess Theradras"] = "瑟莱德丝公主",
	--祖尔法拉克
    ["Antu'sul"] = "安图苏尔",
    ["Witch Doctor Zum'rah"] = "巫医祖穆拉恩",
    ["Shadowpriest Sezz'ziz"] = "暗影祭司塞瑟斯",
    ["Dustwraith"] = "灰尘怨灵",
    ["Zerillis"] = "泽雷利斯",
    ["Gahz'rilla"] = "加兹瑞拉",
    ["Chief Ukorz Sandscalp"] = "乌克兹·沙顶",
    ["Zel'jeb the Ancient"] = "年迈的泽尔杰布",
    ["Champion Razjal the Quick"] = "迅捷的拉扎尔",
	--沉没的神庙
    ["Balcony Minibosses"] = "阳台小首领",
    ["Atal'alarion"] = "阿塔拉利恩",
    ["Spawn of Hakkar"] = "哈卡之子",
    ["Avatar of Hakkar"] = "哈卡的化身",
    ["Jammal'an the Prophet"] = "预言者迦玛兰",
    ["Ogom the Wretched"] = "可悲的奥戈姆",
    ["Dreamscythe"] = "德姆塞卡尔",
    ["Weaver"] = "德拉维沃尔",
    ["Morphaz"] = "摩弗拉斯",
    ["Hazzas"] = "哈扎斯",
    ["Shade of Eranikus"] = "伊兰尼库斯的阴影",
    ["Gasher"] = "加什尔",
    ["Zul'Lor"] = "祖罗尔",
    ["Hukku"] = "胡库",
    ["Mijan"] = "米杉",
    ["Loro"] = "洛若尔",
    ["Zolo"] = "祖罗",
	--仇恨熔炉采石场
    ["High Foreman Bargul Blackhammer"] = "高级工头巴古·黑锤",
    ["Engineer Figgles"] = "工程师菲格尔斯",
    ["Corrosis"] = "腐蚀西斯",
    ["Hatereaver Annihilator"] = "憎恨歼灭者",
    ["Har'gesh Doomcaller"] = "哈格什·末日召唤者",
	--黑石深渊
    ["Lord Roccor"] = "洛考尔",
    ["High Interrogator Gerstahn"] = "审讯官格斯塔恩",
    ["Anub'shiah"] = "阿努希尔",
    ["Eviscerator"] = "剔骨",
    ["Gorosh the Dervish"] = "修行者高罗什",
    ["Grizzle"] = "格里兹尔",
    ["Hedrum the Creeper"] = "爬行者赫杜姆",
    ["Ok'thor the Breaker"] = "破坏者奥科索尔",
    ["Houndmaster Grebmar"] = "驯犬者格雷布玛尔",
    ["Pyromancer Loregrain"] = "控火师罗格雷恩",
    ["Warder Stilgiss"] = "典狱官斯迪尔基斯",
    ["Verek"] = "维雷克",
    ["Fineous Darkvire"] = "弗诺斯·达克维尔",
    ["Lord Incendius"] = "伊森迪奥斯",
    ["Bael'Gar"] = "贝尔加",
    ["General Angerforge"] = "安格弗将军",
    ["Golem Lord Argelmach"] = "傀儡统帅阿格曼奇",
    ["The Grim Guzzler"] = "黑铁酒吧",
    ["Ambassador Flamelash"] = "弗莱拉斯大使",
    ["Panzor the Invincible"] = "无敌的潘佐尔",
    ["Summoner's Tomb"] = "召唤者之墓",
    ["Magmus"] = "玛格姆斯",
    ["Princess Moira Bronzebeard"] = "铁炉堡公主茉艾拉·铜须",
    ["Emperor Dagran Thaurissan"] = "达格兰·索瑞森大帝",
    ["Hammered Patron"] = "喝醉的顾客",
    ["Blacksmithing Plans"] = "铁匠锻造图纸",
    ["Schematic: Field Repair Bot 74A"] = "战地修理机器人74A型",
    ["Lockpicking"] = "开锁",
	--厄运之槌
    ["Pusillin"] = "普希林",
    ["Zevrim Thornhoof"] = "瑟雷姆·刺蹄",
    ["Hydrospawn"] = "海多斯博恩",
    ["Lethtendris"] = "蕾瑟塔蒂丝",
    ["Pimgib"] = "匹姆吉布",
    ["Isalien"] = "伊萨利恩",
    ["Alzzin the Wildshaper"] = "奥兹恩",
    ["Tendris Warpwood"] = "特迪斯·扭木",
    ["Illyanna Ravenoak"] = "伊琳娜·暗木",
    ["Magister Kalendris"] = "卡雷迪斯镇长",
    ["Tsu'zee"] = "苏斯",
    ["Immol'thar"] = "伊莫塔尔",
    ["Lord Hel'nurath"] = "赫尔努拉斯",
    ["Prince Tortheldrin"] = "托塞德林王子",
    ["Guard Mol'dar"] = "卫兵摩尔达",
    ["Stomper Kreeg"] = "践踏者克雷格",
    ["Guard Fengus"] = "卫兵芬古斯",
    ["Guard Slip'kik"] = "卫兵斯里基克",
    ["Captain Kromcrush"] = "克罗卡斯",
    ["Cho'Rush the Observer"] = "观察者克鲁什",
    ["King Gordok"] = "戈多克大王",
    ["Warpwood Pod"] = "扭木茧",
	--通灵学院
    ["Kirtonos the Herald"] = "传令官基尔图诺斯",
    ["Jandice Barov"] = "詹迪斯·巴罗夫",
    ["Rattlegore"] = "血骨傀儡",
    ["Death Knight Darkreaver"] = "死亡骑士达克雷尔",
    ["Marduk Blackpool"] = "马杜克·布莱克波尔",
    ["Vectus"] = "维克图斯",
    ["Ras Frostwhisper"] = "莱斯·霜语",
    ["Kormok"] = "库尔莫克",
    ["Instructor Malicia"] = "讲师玛丽希亚",
    ["Doctor Theolen Krastinov"] = "瑟尔林·卡斯迪诺夫教授",
    ["Lorekeeper Polkelt"] = "博学者普克尔特",
    ["The Ravenian"] = "拉文尼亚",
    ["Lord Alexei Barov"] = "阿雷克斯·巴罗夫领主",
    ["Lady Illucia Barov"] = "伊露希亚·巴罗夫",
    ["Darkmaster Gandling"] = "黑暗院长加丁",
    ["Plagued Hatchling"] = "天灾龙崽",
	--斯坦索姆
    ["Skul"] = "斯库尔",
    ["Stratholme Courier"] = "斯坦索姆信使",
    ["Postmaster Malown"] = "邮差马龙",
    ["Fras Siabi"] = "弗拉斯·希亚比",
    ["Atiesh"] = "埃提耶什",
    ["Hearthsinger Forresten"] = "弗雷斯特恩",
    ["The Unforgiven"] = "不可宽恕者",
    ["Timmy the Cruel"] = "悲惨的提米",
    ["Malor the Zealous"] = "指挥官玛洛尔",
    ["Crimson Hammersmith"] = "红衣铸锤师",
    ["Cannon Master Willey"] = "炮手威利",
    ["Archivist Galford"] = "档案管理员加尔福特",
    ["Balnazzar"] = "巴纳扎尔",
	["Sothos"] = "索托斯",
    ["Jarien"] = "亚雷恩",
    ["Stonespine"] = "石脊",
    ["Baroness Anastari"] = "安娜丝塔丽男爵夫人",
    ["Black Guard Swordsmith"] = "黑衣守卫铸剑师",
    ["Nerub'enkan"] = "奈鲁布恩坎",
    ["Maleki the Pallid"] = "苍白的玛勒基",
    ["Magistrate Barthilas"] = "巴瑟拉斯镇长",
    ["Ramstein the Gorger"] = "吞咽者拉姆斯登",
    ["Baron Rivendare"] = "瑞文戴尔男爵",
	--黑下
    ["Spirestone Butcher"] = "尖石屠夫",
    ["Spirestone Battle Lord"] = "尖石统帅",
    ["Spirestone Lord Magus"] = "尖石首席法师",
    ["Highlord Omokk"] = "欧莫克大王",
    ["Shadow Hunter Vosh'gajin"] = "暗影猎手沃什加斯",
    ["War Master Voone"] = "指挥官沃恩",
    ["Burning Felguard"] = "燃烧恶魔卫士",
    ["Mor Grayhoof"] = "莫尔·灰蹄",
    ["Bannok Grimaxe"] = "班诺克·巨斧",
    ["Mother Smolderweb"] = "烟网蛛后",
    ["Crystal Fang"] = "水晶之牙",
    ["Urok Doomhowl"] = "乌洛克",
    ["Quartermaster Zigris"] = "军需官兹格雷斯",
    ["Halycon"] = "哈雷肯",
    ["Gizrul the Slavener"] = "奴役者基兹鲁尔",
    ["Ghok Bashguud"] = "霍克·巴什古德",
    ["Overlord Wyrmthalak"] = "维姆萨拉克",
    ["Pyroguard Emberseer"] = "烈焰卫士艾博希尔",
    ["Solakar Flamewreath"] = "索拉卡·火冠",
    ["Jed Runewatcher"] = "杰德",
    ["Goraluk Anvilcrack"] = "古拉鲁克",
    ["Warchief Rend Blackhand"] = "大酋长雷德·黑手",
    ["Gyth"] = "盖斯",
    ["The Beast"] = "比斯巨兽",
    ["Lord Valthalak"] = "瓦萨拉克",
    ["General Drakkisath"] = "达基萨斯将军",
    ["Scarshield Warlock"] = "裂盾术士",
    ["Spire Spider/Spiderling"] = "尖塔蜘蛛/小蜘蛛",
	--卡拉赞墓穴
    ["Marrowspike"] = "髓刺",
    ["Hivaxxis"] = "西瓦克西斯",
    ["Corpsemuncher"] = "嚼尸鬼",
    ["Guard Captain Gort"] = "卫兵长高尔特",
    ["Archlich Enkhraz"] = "大巫妖安克瑞兹",
    ["Commander Andreon"] = "指挥官安德里昂",
    ["Alarus"] = "阿拉鲁斯",
    ["Half-Buried Treasure Chest"] = "半埋宝箱",
	--黑色沼泽
    ["Chronar"] = "克罗纳",
    ["Epidamu"] = "埃皮达姆",
    ["Drifting Avatar of Sand"] = "沙之漂流化身",
    ["Time-Lord Epochronos"] = "时间领主伊波克洛诺斯",
    ["Mossheart"] = "莫斯哈特",
    ["Rotmaw"] = "腐烂之喉",
    ["Antnormi"] = "安蒂诺米",
	--暴风城地牢
    ["Aszosh Grimflame"] = "艾丝卓仕·格瑞姆弗雷姆",
    ["Tham'Grarr"] = "塞姆格拉",
    ["Black Bride"] = "黑色新娘",
    ["Damian"] = "戴米安",
    ["Volkan Cruelblade"] = "沃肯·冷刃",
    ["Arc'tiras"] = "阿尔克提拉斯",
	--祖尔格拉布
    ["High Priestess Jeklik"] = "高阶女祭司耶克里克",
    ["High Priest Venoxis"] = "高阶祭司温诺希斯",
    ["High Priestess Mar'li"] = "高阶女祭司玛尔里",
    ["Bloodlord Mandokir"] = "血领主曼多基尔",
    ["Gri'lek"] = "格里雷克",
    ["Hazza'rah"] = "哈札拉尔",
    ["Renataki"] = "雷纳塔基",
    ["Wushoolay"] = "乌苏雷",
    ["Gahz'ranka"] = "加兹兰卡",
    ["High Priest Thekal"] = "古拉巴什食腐者",
    ["High Priestess Arlokk"] = "高阶女祭司娅尔罗",
    ["Jin'do the Hexxer"] = "妖术师金度",
    ["Hakkar"] = "哈卡",
    --安其拉废墟
    ["Kurinnaxx"] = "库林纳克斯",
    ["Lieutenant General Andorov"] = "安多洛夫中将",
    ["General Rajaxx"] = "拉贾克斯将军",
    ["Moam"] = "莫阿姆",
    ["Buru the Gorger"] = "吞咽者布鲁",
    ["Ayamiss the Hunter"] = "狩猎者阿亚米斯",
    ["Ossirian the Unscarred"] = "无疤者奥斯里安",
	--熔火之心
    ["Incindis"] = "因辛迪斯",
    ["Lucifron"] = "鲁西弗隆",
    ["Magmadar"] = "玛格曼达",
    ["Garr"] = "加尔",
    ["Shazzrah"] = "沙斯拉尔",
    ["Baron Geddon"] = "迦顿男爵",
    ["Golemagg the Incinerator"] = "焚化者古雷曼格",
    ["Basalthar & Smoldaris"] = "巴萨尔萨和斯摩达利斯",
    ["Sorcerer-Thane Thaurissan"] = "巫王索瑞森",
    ["Sulfuron Harbinger"] = "萨弗隆先驱者",
    ["Majordomo Executus"] = "管理者埃克索图斯",
    ["Ragnaros"] = "拉格纳罗斯",
	["Basalthar"] = "巴萨尔萨",
    ["Smoldaris"] = "斯摩达利斯",
    ["Molten Destroyer"] = "熔核摧毁者",
	--奥妮克希亚的巢穴
    ["Onyxia's Lair"] = "奥妮克希亚的巢穴",
    ["Onyxia"] = "奥妮克希亚",
    ["Broodcommander Axelus"] = "巢群指挥官阿克塞勒斯",--1.81新增boss
	
	--卡拉赞下层大厅
    ["Master Blacksmith Rolfen"] = "锻造大师罗尔芬",
    ["Brood Queen Araxxna"] = "巢穴女王阿拉克萨娜",
    ["Grizikil"] = "格里齐基尔",
    ["Clawlord Howlfang"] = "爪王嚎牙",
    ["Lord Blackwald II"] = "布莱克沃尔德勋爵二世",
    ["Moroes"] = "莫罗斯",
    ["LKH Enchants"] = "卡拉赞下层附魔",
	--黑翼之巢
    ["Razorgore the Untamed"] = "狂野的拉佐格尔",
    ["Vaelastrasz the Corrupt"] = "堕落的瓦拉斯塔兹",
    ["Broodlord Lashlayer"] = "勒什雷尔",
    ["Firemaw"] = "费尔默",
    ["Ebonroc"] = "埃博诺克",
    ["Flamegor"] = "弗莱格尔",
	["Chromaggus"] = "克洛玛古斯",
	["Nefarian"] = "奈法利安",
	["Ezzel Darkbrewer"] = "伊泽尔·黑酿",--1.81新增boss
	--翡翠圣殿
    ["Erennius"] = "埃伦纽斯",
    ["Solnius the Awakener"] = "索尔纽斯",
	["Favor of Erennius (ES Hard Mode)"] = "埃伦纽斯的青睐（翡翠圣殿 困难模式）",
	--安其拉神殿
    ["The Prophet Skeram"] = "预言者斯克拉姆",
    ["The Bug Family"] = "吉祥三宝",
    ["Battleguard Sartura"] = "沙尔图拉",
    ["Fankriss the Unyielding"] = "顽强的范克瑞斯",
    ["Viscidus"] = "维希度斯",
    ["Princess Huhuran"] = "哈霍兰公主",
    ["The Twin Emperors"] = "双子皇帝",
	["Ouro"] = "奥罗",
	["C'Thun"] = "克苏恩",
    ["Emperor Vek'lor"] = "维克洛尔大帝",
    ["Emperor Vek'nilash"] = "维克尼拉斯大帝",
    ["Lord Kri"] = "克里勋爵",
    ["Vem"] = "维姆",
    ["Princess Yauj"] = "亚尔基公主",
    ["Greater Scarab Coffer"] = "巨型圣甲虫宝箱",
	--纳克萨玛斯
    ["Patchwerk"] = "帕奇维克",
    ["Grobbulus"] = "格罗布鲁斯",
    ["Gluth"] = "格拉斯",
    ["Thaddius"] = "塔迪乌斯",
    ["Anub'Rekhan"] = "阿努布雷坎",
    ["Grand Widow Faerlina"] = "黑女巫法琳娜",
    ["Maexxna"] = "迈克斯纳",
    ["Noth the Plaguebringer"] = "瘟疫使者诺斯",
    ["Heigan the Unclean"] = "肮脏的希尔盖",
    ["Loatheb"] = "洛欧塞布",
    ["Instructor Razuvious"] = "教官拉苏维奥斯",
    ["Gothik the Harvester"] = "收割者戈提克",
    ["The Four Horsemen"] = "四骑士",
    ["Sapphiron"] = "萨菲隆",
    ["Kel'Thuzad"] = "克尔苏加德",
	-- 卡拉赞之塔
	["King (Chess fight)"] = "国王议会",
	["Sanv Tas'dal"] = "桑夫·塔斯达尔",
	["Kruul"] = "库鲁尔",
	["Ley-Watcher Incantagos"] = "魔网观察者因塔苟斯",
	["Anomalus"] = "阿诺玛鲁斯",
	["Echo of Medivh"] = "麦迪文的回响",
	["Keeper Gnarlmoon"] = "守护者纳尔穆恩", 
	["Mephistroth"] = "孟菲斯托斯", 
	["Rupturan the Broken"] = "破碎的鲁普图兰",
	-- 木喉要塞
	["Karrsh the Sentinel"] = "哨兵卡什",
	["Rotgrowl"] = "腐嚎",
	["Trioch the Devourer"] = "吞噬者提里奥克",
	["Loktanag the Vile"] = "落潭",
	["Archdruid Kronn"] = "大德鲁伊科罗恩",
	["Ormanos the Cracked"] = "裂地者欧曼诺斯",
	["Chieftain Partath"] = "大酋长帕萨斯",
	["Selenaxx Foulheart"] = "塞雷纳克斯·腐心",
	["Ursol"] = "乌索尔",
	--杂项
    ["Cloth"] = "布甲",
    ["Ring"] = "戒指",
    ["Gun"] = "枪械",
    ["Axe"] = "斧",
    ["Mace"] = "锤",
    ["Sword"] = "剑",
    ["Dagger"] = "匕首",
    ["Polearm"] = "长柄武器",
    ["Champion"] = "勇士",
    ["Centurion"] = "百夫长",
    ["Defender"] = "防御者",
    ["Myrmidon"] = "仆从",
    ["Level One Lunatic Challenge"] = "一级疯子挑战",
    ["Wand"] = "魔杖",
    ["Fist Weapon"] = "拳套",
    ["Defias Pirate"] = "迪菲亚海盗",
    ["Libram"] = "圣契",
    ["Totem"] = "图腾",
    ["Idol"] = "神像",
    ["Prisoner/Insurgent/Convict"] = "囚犯/叛军/罪犯",
    ["Bow"] = "弓",
    ["Crossbow"] = "弩",
    ["Fishing Pole"] = "钓鱼竿",
    ["Doll"] = "人偶",
    ["Coin"] = "硬币",
    ["Bijou"] = "宝石",
	["AtlasLoot Enhanced"] = "AtlasLoot 副本掉落",
	["Left-click to open AtlasLoot.\nMiddle-click for AtlasLoot options.\nRight-click and drag to move this button."] = "左键点击打开AtlasLoot。\n中键点击打开AtlasLoot选项。\n右键点击并拖动以移动此按钮。",
    ["Show Source on Tooltips"] = "在工具提示中显示来源",
    ["Show IDs at all times"] = "永远显示物品ID",
    ["Button Position"] = "按钮位置",
    ["Button Radius"] = "按钮半径",
    ["Default Settings"] = "默认设置",
    ["Reset Position"] = "重置位置",
    ["SpellID:"] = "法术ID:",
    ["ItemID:"] = "物品ID:",
    ["Back1"]= "返回",
    ["Container"] = "容器",
    ["Consumable"] = "消耗品",
    ["Arrow"] = "箭矢",
    ["Food"] = "食物",
    ["Quiver"] = "箭袋",
    ["Ammo Pouch"] = "弹药包",
    ["Drink"] = "饮料",
    ["Key"] = "钥匙",
    ["Reagent"] = "材料",
    ["Alliance"] = "联盟",
    ["Horde"] = "部落",
    ["Thrown"] = "投掷武器",
    ["Moomoo Grove"] = "哞哞林",
    ["Prince Skaldrenox"] = "斯卡德诺克斯王子",
    ["Lord Skwol"] = "斯古恩男爵",
    ["High Marshal Whirlaxis"] = "大元帅维拉希斯",
    ["Baron Kazum"] = "卡苏姆男爵",
    ["Air"] = "空气",
    ["Baron Charr"] = "火焰男爵查尔",
    ["Avalanchion"] = "阿瓦兰奇奥",
    ["Princess Tempestria"] = "泰比斯蒂亚公主",
    ["The Windreaver"] = "烈风掠夺者",
    ["Scourge Invasion Bosses"] = "天灾入侵的首领",
    ["Needed to purchase rewards"] = "需要购买奖励所需的物品/货币",
    ["Toy"] = "玩具",
    ["ALT+Click on item to add or remove it from WishList"] = "ALT+单击物品以将其添加或从收藏中删除",
    ["AL Panel"] = "AL 面板",
    ["Bandage"] = "绷带",
    ["Potion"] = "药水",
    ["Bullet"] = "子弹",
    ["The League of Arathor"] = "阿拉索联军",
    ["The Defilers"] = "污染者",
} end)