...
local isWestern = GetLocale() == "enUS" or GetLocale() == "zhCN" or GetLocale() == "deDE" or GetLocale() == "frFR" or GetLocale() == "esES"
...
local function activate(self, oldLib, oldDeactivate)
	...
	local STORMWIND_AUBERDINE_BOAT = string.format(X_Y_BOAT, Z["Stormwind Harbor"], Z["Auberdine"])
	local AUBERDINE_ALAH_THALAS_BOAT = string.format(X_Y_BOAT, Z["Auberdine"], Z["Alah'Thalas"])
	local SPARKWATER_REVANTUSK_BOAT = string.format(X_Y_BOAT, Z["Sparkwater Port"], Z["Revantusk Village"])
	local SHADOWPREY_MOONHOOF_BOAT = string.format(X_Y_BOAT, Z["Shadowprey Village"], Z["Moonhoof Village"])
	local DARKSHORE_ICEPOINT_CANOE = string.format(X_Y_CANOE, Z["Darkshore"], Z["Icepoint Rock"])
	...
	local ORGRIMMAR_THUNDERBLUFF_ZEPPELIN = string.format(X_Y_ZEPPELIN, Z["Orgrimmar"], Z["Thunder Bluff"])
	local ORGRIMMAR_KARGATH_ZEPPELIN = string.format(X_Y_ZEPPELIN, Z["Orgrimmar"], Z["Kargath"])
	...
	zones[AUBERDINE_ALAH_THALAS_BOAT] = {
		paths = {
			[Z["Darkshore"]] = true,
			[Z["Thalassian Highlands"]] = true,
		},
		type = "Transport",
	}

	zones[SPARKWATER_REVANTUSK_BOAT] = {
		paths = {
			[Z["Durotar"]] = true,
	...
	zones[SHADOWPREY_MOONHOOF_BOAT] = {
		paths = {
			[Z["Desolace"]] = true,
			[Z["Moonwhisper Coast"]] = true,
		},
		type = "Transport",
	}

	zones[DARKSHORE_ICEPOINT_CANOE] = {
		paths = {
			[Z["Darkshore"]] = true,
			[Z["Icepoint Rock"]] = true,
		},
		type = "Transport",
	}
	...
	zones[ORGRIMMAR_KARGATH_ZEPPELIN] = {
		paths = {
			[Z["Durotar"]] = true,
			[Z["Badlands"]] = true,
		},
	...
	zones[Z["Badlands"]] = {
		...
		paths = {
			...
			[Z["Loch Modan"]] = true,
			[ORGRIMMAR_KARGATH_ZEPPELIN] = true,
		},
	}
	...
	zones[Z["The Hinterlands"]] = {
		...
		paths = {
			[Z["Hillsbrad Foothills"]] = true,
			[Z["Western Plaguelands"]] = true,
			[SPARKWATER_REVANTUSK_BOAT] = true,
		},
	}
	...
	zones[Z["Thalassian Highlands"]] = {
		continent = Eastern_Kingdoms,
		paths = {
			[AUBERDINE_ALAH_THALAS_BOAT] = true,
		},
	}
	...
	zones[Z["Moonwhisper Coast"]] = {
		continent = Kalimdor,
		paths = {
			[Z["Icepoint Rock"]] = true,
			[SHADOWPREY_MOONHOOF_BOAT] = true,
		},
	}
