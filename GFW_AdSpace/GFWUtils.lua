...
function GFWUtils_temp_TextGSC(money, noRound)
	...
	local gsc = "";
	if (g > 0) then
		gsc = format(GSC_START, GSC_GOLD, g, "金");     
		if (s > 0) then
			gsc = gsc..format(GSC_PART, GSC_SILVER, s, "银");
			if (noRound and c > 0) then
				gsc = gsc..format(GSC_PART, GSC_COPPER, c, "铜");
			end
		end
	elseif (s > 0) then
		gsc = format(GSC_START, GSC_SILVER, s, "银");
		if (c > 0) then
			gsc = gsc..format(GSC_PART, GSC_COPPER, c, "铜");
		end
	elseif (c > 0) then
		gsc = gsc..format(GSC_START, GSC_COPPER, c, "铜");
	else
		gsc = GSC_NONE;
	end
