...
--[[local function LoadItemInfo(type, sid)
    ...
    local itemInfo = tab and tab[tonumber(sid)]
    if not itemInfo then
        local validSid = tonumber(sid)
        if validSid then
            local itemName, _, _ = GetItemInfo(validSid)
			--local itemName, _, _, _, _, _, _, _, _, itemPrice = GetItemInfo(validSid)
            if itemName then
                for _, subTab in pairs(itemDB[type] or {}) do
                    if subTab[IDX_enUS] == itemName then
                        itemInfo = subTab
                        break
                    end
                end
...
local function LoadItemInfo(type, sid)
	...
	local itemLevel = staticInfo and staticInfo[IDX_LV] or 0
	local sellPrice = staticInfo and staticInfo[IDX_PRICE] or 0
	
	if englishName == "" or sellPrice == 0 then
		local nameFromServer, _, _, _, _, _, _, _, _, priceFromServer = GetItemInfo(validSid)
		if nameFromServer then
			if englishName == "" then
				englishName = nameFromServer
			end
			if sellPrice == 0 and priceFromServer then
