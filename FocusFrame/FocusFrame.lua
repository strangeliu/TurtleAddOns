...
FocusFrameDB = FocusFrameDB or { tot = true, unlock = true, scale = 1 }
...
local function OnFocusTargetUpdated(event, name, isDead, unitID)
	...
	FocusFrameTargetofTargetHealthBar:SetStatusBarColor(UnitHealthBarColor(unitID))
	...
	name = Focus:ConvertName(name)
	FocusFrameTargetofTargetName:SetText(name)
	SetPortraitTexture(FocusFrameTargetofTargetPortrait, unitID)

	--快捷设置焦点目标的目标为焦点
	for _, Focu_TargetFrame in ipairs({FocusFrameTargetofTargetFrame}) do
		Focu_TargetFrame:SetScript('OnMouseDown', function()
			if IsControlKeyDown() and IsAltKeyDown() then
				Focus:SetFocus(Focus:org_UnitName(unitID))
			end
		end)
	end
end
...
local function OnFocusSat(event, unit)
	local fontName, fontSize = TargetName:GetFont()
	...
	FocusFrame:SetScript("OnUpdate", function()
		FocusFrame_CastingBarOnUpdate()
	end)
	
	FocusFrame:Show()
	FocusFrameHealthBar:SetStatusBarColor(UnitHealthBarColor(unit))
end
...
local function HealthUpdate()
    ...
    if health == 0 then
        ...
        FocusFrameHealthBarText:SetText(health .. " / " .. maxHealth)
...
local function CheckFaction()
	...
	-- if Focus:GetData("unitIsPVPFreeForAll") == 1 then
		-- FocusPVPIcon:SetTexture("Interface\\AddOns\\FocusFrame\\Media\\TargetingFrame\\UI-PVP-FFA")
		-- FocusPVPIcon:Show()
	-- elseif factionGroup and Focus:GetData("unitIsPVP") == 1 then
		-- FocusPVPIcon:SetTexture("Interface\\AddOns\\FocusFrame\\Media\\TargetingFrame\\UI-PVP-" .. factionGroup)
		-- FocusPVPIcon:Show()
	-- else
		FocusPVPIcon:Hide()
	-- end
...
local function CheckClassification()
	...
	if classification == "worldboss" or classification == "rareelite" or classification == "elite" then
		FocusFrameTexture:SetTexture("Interface\\AddOns\\FocusFrame\\Media\\TargetingFrame\\UI-TargetingFrame-Elite")
	elseif classification == "rare" then
		FocusFrameTexture:SetTexture("Interface\\AddOns\\FocusFrame\\Media\\TargetingFrame\\UI-TargetingFrame-Rare")
	else
		FocusFrameTexture:SetTexture("Interface\\AddOns\\FocusFrame\\Media\\TargetingFrame\\UI-TargetingFrame")
	end
end
...
FocusFrame.cast:SetStatusBarTexture("Interface\\AddOns\\FocusFrame\\Media\\TargetingFrame\\UI-StatusBar")
...
FocusFrame.cast.timer:SetPoint("LEFT", FocusFrame.cast, "RIGHT", 3, 0)
...
SlashCmdList.FOCUSOPTIONS = function(msg)
	...
	elseif cmd == "reset" then
		...
		print("    target -|cff00FF7F 打开焦点目标的目标")
		print("    statustext - |cff00FF7F 显示血量/蓝量数值")
		print("    tarcycles -|cff00FF7F 设置循环执行/fmcast的周期")
