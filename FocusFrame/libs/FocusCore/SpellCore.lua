...
local CastCraftPerform = function()
	...
	local perform 	= '(.+)使用(.+)。'
	local bperform 	= '(.+)开始施展(.+)。'
	local performOn = '(.+)对(.+)使用(.+)。'
	local pcastFin 	= '你对(.+)施放了(.+)。'
	local castFin 	= '(.+)对(.+)施放了(.+)。'
	...
	elseif fperform or fbperform or fperformOn then
        ...
        if fperform then
            m = perform
        elseif fbperform then
            m = bperform
        elseif fperformOn then
            m = performOn
        end
		local c = gsub(arg1, m, '%1')
		local s = gsub(arg1, m, '%2')
...
local DirectInterrupt = function()
	local pintrr 	= '你打断了(.+)的(.+)。'			local fpintrr  	= strfind(arg1, pintrr)
	local intrr 	= '(.+)打断了(.+)的(.+)。'		local fintrr  	= strfind(arg1, intrr)
...
local GainAfflict = function()
	local gain 		= '(.+)获得了(.+)的效果。'								local fgain = strfind(arg1, gain)
	local pgain 	= '你获得了(.+)的效果。'								local fpgain = strfind(arg1, pgain)
	local afflict 	= '(.+)受到了(.+)效果的影响。' 					local fafflict = strfind(arg1, afflict)
	local pafflict 	= '你受到了(.+)效果的影响。' 					local fpafflict = strfind(arg1, pafflict)
...
local FadeRem = function()
	local fade 		= '(.+)效果从(.+)身上消失。'					local ffade = strfind(arg1, fade)
	local rem 		= '(.+)的(.+)被移除了。'						local frem = strfind(arg1, rem)
	local prem 		= '你的(.+)被移除了。'							local fprem = strfind(arg1, prem)
...
local HitsCrits = function()
	local hits = '(.+)的(.+)击中(.+)造成(.+)点' 			local fhits = strfind(arg1, hits)
	local crits = '(.+)的(.+)致命一击对(.+)造成(.+)点' 		local fcrits = strfind(arg1, crits)
	local absb = '(.+)的(.+)被(.+)吸收了。'					local fabsb = strfind(arg1, absb)

	local phits = '你的(.+)击中(.+)造成(.+)点' 				local fphits = strfind(arg1, phits)
	local pcrits = '你的(.+)对(.+)造成(.+)的致命一击伤害' 	local fpcrits = strfind(arg1, pcrits)
	local pabsb = '你的(.+)被(.+)吸收了。'					local fpabsb = strfind(arg1, pabsb)
	--local yphits = 'You hit (.+) for (.+).'				
	local channelDotRes = "(.+)的(.+)被(.+)抵抗了。"		local fchannelDotRes = strfind(arg1, channelDotRes)
	local pchannelDotRes = "(.+)的(.+)被抵抗了"				local fpchannelDotRes = strfind(arg1, pchannelDotRes)
...
local channelDot = function()
	local channelDot 	= "(.+)的(.+)使(.+)受到了(.+)点伤害。"		local fchannelDot = strfind(arg1, channelDot)
	local channelpDot 	= '你的(.+)使(.+)受到了(.+)点伤害。'		local fchannelpDot	= strfind(arg1, channelpDot)
	local pchannelDot 	= "你受到(.+)点伤害（(.+)的(.+)）。"		local fpchannelDot = strfind(arg1, pchannelDot)

	local MDrain = '(.+)的(.+)从(.+)身上吸取了(.+)点法力值。' 		local fMDrain = strfind(arg1, MDrain)
