...
local GetTime, UnitIsPlayer, type, strfind, tgetn =
      GetTime, UnitIsPlayer, type, string.find, table.getn

function Focus:org_GetUnitName(unit)
    if not SetAutoloot then
        return GetUnitName(unit)
    else
        local _, name = UnitExists(unit)
        return name
    end
end

function Focus:org_UnitName(unit)
    if not SetAutoloot then
...
function Focus:org_TargetByName(unit, exactMatch)
	if not SetAutoloot then
		if exactMatch == nil then
			TargetByName(unit)
		else
			TargetByName(unit, exactMatch)
		end
	else
		TargetUnit(unit)
	end
...
function Focus:ConverUnit(unit)
    if not SetAutoloot then
        return unit
    else
        local _, name = UnitExists(unit)
...
function Focus:ConvertName(name)
	if name and string.find(name, "0x") then
		name = UnitName(name)
	end
	return name
end
...
local function IsPlayerWithSamePetName(unit)
	if rawData.unitName and rawData.unitName == Focus:org_UnitName(unit) then
...
local function SetFocusHealth(unit, isDead, hasPetFixRan)
    ...
    if MobHealthFrame then
        local min,max = MobHealth3:GetUnitHealth(unit, UnitHealth(unit), UnitHealthMax(unit))
        data.health = isDead and 0 or min
        data.maxHealth = isDead and 0 or max
    else
...
local function SetFocusTargetInfo(event, unit)
	...
	if unit == "player" then
        ...
        if unit then
            ...
            tot = nil -- 如果 unit 为 nil，则 tot 可以赋值为 nil 或其他默认值
        end
    end
	...
	if unit and UnitExists(tot) then
		...
		rawData.targetName = Focus:org_UnitName(tot)
...
local function SetFocusInfo(unit, resetRefresh, test)
	...
	-- 焦点名字/GUID
	rawData.unitName = Focus:org_GetUnitName(unit)
	-- 是否是玩家
...
do
	...
	function Focus:ShowError(msg)
		UIErrorsFrame:AddMessage("|cffFF003F " .. (msg or "你没有设置焦点目标") .. "|r")
	end
	...
	function Focus:UnitIsFocus(unit, checkName)
		if not checkName then
			return focusTargetName and Focus:org_UnitName(unit) == focusTargetName
		else
	...
	-- @private
	-- only used for npcs/hunters 只用于NPC / 猎人使用 ？
	function Focus:TargetWithFixes(name)
		...
		-- 如果name不为空 返回 去掉最后一个字符后的name，否则返回 去掉最后一个字符后的focusTargetName
		local _name = strsub(name or focusTargetName, 1, -2)
		Focus:org_TargetByName(_name, false)
		...
		if UnitIsDead("target") == 1 or (isPlayer and isPlayer ~= UnitIsPlayer("target")) or UnitIsUnit("target", "player") then
			...
			-- 试试区分大小写搜索
			Focus:org_TargetByName(name or focusTargetName, true)
		end
    ...
    ---选中焦点为目标
    ---@param name string 名字或者nil
    ---@param setFocusName boolean 布尔值或者nil 如果是true，则更新焦点单位的名字
	...
	function Focus:TargetFocus(name, setFocusName)
		if not setFocusName and not self:FocusExists() then
            ...
            self.oldTarget = Focus:org_UnitName("target")
            if not self.oldTarget or self.oldTarget ~= focusTargetName or rawData.IsPlayerWithSamePetName then
                ------------------------------修改-----------------------------------
                if SetAutoloot and focusTargetName then
					TargetUnit(focusTargetName)
				else
					------------------------------修改-----------------------------------
					if rawData.unitIsPlayer ~= 1 then
						...
						if rawData.IsPlayerWithSamePetName then
							...
							if rawData.playerCanAttack and rawData.unitIsPlayer and not UnitIsPlayer("target") then
								...
								if Focus:org_UnitName("target") ~= rawData.unitName then
									ClearTarget()
								end
							end
						else
							local willSeleteName = focusTargetName
							if focusTargetName and string.find(focusTargetName, "0x") then
								willSeleteName = UnitName(focusTargetName)
							end
							Focus:org_TargetByName(name or willSeleteName, true)
						end
					end
				end
                self.needRetarget = true
		...
		if setFocusName then
			...
			-- /focus <name>
			focusTargetName = Focus:org_UnitName("target")
	...
	--修正焦点目标的目标不准确的问题（by狗血编剧男20201207）
	function Focus:TargetFocusTarget()
		if rawData.targetName ~= Focus:org_GetUnitName("player") then
			Focus:org_TargetByName(Focus:GetName())
			TargetofTargetFrame:Click()
		else
			Focus:org_TargetByName(rawData.targetName, true)
		end
	end
	...
	function Focus:TargetPrevious()
		if self.oldTarget and self.needRetarget then
			...
			if Focus:org_UnitName("target") ~= self.oldTarget then
	...
	function Focus:SetFocus(name)
		if not name or name == "" then
			name = Focus:org_UnitName("target")
		end
	...
	do
		...
		function Focus:GetCast(name)
			...
			if getTime < timeEnd then
				local t = getTime - timeStart
                local totaltime = timeEnd - timeStart
				local timer = (string.format("%0.1f", t) .. "/" .. string.format("%0.1f", totaltime))    -- Round(t)
...
do
	...
	local playerName = Focus:org_UnitName("player")
	local refresh = 0
	...
	local function ParseCombatDeath()
		if not Focus:FocusExists() then return end
		if SetAutoloot and string.find(focusTargetName, "0x") then
			if UnitHealth(focusTargetName) == 0 then
				SetFocusHealth(nil, true)
				SetFocusTargetInfo(nil, nil)
			end
		else
			local pdie 		= '你死了。'					local fpdie		= strfind(arg1, pdie)
			local dies		= '(.+)死亡了。'				local fdies		= strfind(arg1, dies)
			local slain 	= '(.+)被(.+)干掉了！'			local fslain 	= strfind(arg1, slain)
			local pslain 	= '你杀死了(.+)！'				local fpslain 	= strfind(arg1, pslain)
			...
			if fpdie or fdies or fslain or fpslain then
				...
				local m,c
				if fdies and dies then
					m = dies
				elseif fslain and slain then
					m = slain
				elseif fpslain and pslain then
					m = pslain
				else
					m = ""
				end
				
				if fpdie and playerName then
					c = playerName
				else
					c = gsub(arg1, m, "%1")
				end
