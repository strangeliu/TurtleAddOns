...
local function isValidTooltipOwner(owner)
	if (not owner) then
		return nil;
	end
...
function Hook.GameTooltip_SetOwner(this, owner, anchor, x, y)
	local tooltip = this;
	...
	-- Some code paths call GameTooltip.SetOwner(owner, anchor, x, y) using dot syntax.
	...
	if (not isValidTooltipOwner(tooltipOwner)) then
	...
	Orig.GameTooltip_SetOwner(tooltip, tooltipOwner, tooltipAnchor, tooltipX, tooltipY);
	gameToolTipOwner = tooltipOwner;
end
