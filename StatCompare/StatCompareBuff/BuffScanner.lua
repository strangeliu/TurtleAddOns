		...
		if(SC_BuffScanner_bonuses[e.effect]) then
			if(bonus[e.effect]) then
				if e.effect == "HASTE" then
					bonus[e.effect] = ((bonus[e.effect] / 100 + 1) * (SC_BuffScanner_bonuses[e.effect] / 100 + 1) - 1 ) * 100
				else
...
function SC_BuffScanner_AddValue(effect, value, v1, v2, v3, v4)
	...
	if(type(effect) == "string") then
		if effect ~= "HASTE" then 
			...
			if(SC_BuffScanner_bonuses[effect]) then
				SC_BuffScanner_bonuses[effect] = ((SC_BuffScanner_bonuses[effect] /100 +1 ) * (value / 100 + 1) - 1)*100;
			else
				SC_BuffScanner_bonuses[effect] = value;
			end
		end
