...
function StatCompare_CheckPlayerInfo()
	local tabs = { "Settings", "FramePositions" };
...
function StatCompare_SaveConfigInfo()
	if ( StatCompare_Info[realmName] and
		...
		local tabs = { "Settings", "FramePositions" };
		for _,tab in pairs(tabs) do
			if StatCompare_Player[tab] then
...
function StatCompare_CheckRealm()
	if ( StatCompare_Info["Settings"] ) then
		...
		if ( old ) then
			...
			StatCompare_Info[realmName]["Settings"][playerName] = { };
			for k, v in pairs(old) do
				StatCompare_Info[realmName]["Settings"][playerName][k] = v;
			end
			StatCompare_Info["Settings"][playerName] = nil;
		end
